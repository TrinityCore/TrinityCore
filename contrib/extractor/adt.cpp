        ...
        if(fourcc == 0x4d48324f)                            // MH2O new in WotLK
        {
            ...
            for(chunk_num = 0; chunk_num < 256; ++chunk_num)
            {
                ...
                if(LiqOffsData->offsData1 != 0)             // если данные ?Data1 ?воде есть, то их надо конвертировать
                {
                    ...
                    // предварительно заполн€ем весь кусо?данным?- не?воды
                    for(int j = 0; j < 81; ++j)
                    {
                        ChunkLiqHeight[j] = -999999;        // no liquid/water
                    }
                    // теперь вычисл€ем те чт??водо??перезаписываем их ?куск?
                    ...
                    mf.seek(header_pos);                    // ?не забыть вернутьс€ на исходную позици?именно ?’»ƒ≈–≈
                }
                ...
                k = m + (chunk_num % 16) * 8;               // устанавливаемс€ на начальны?индекс дл€ заполнен? ?да
                // заноси?данные куск??массив дл€ карт? ?перекрытие??обрезанием кусков тк данных 81
                // эт?аналог старог?обрезани€ граничны?правых-боковы??нижних данных
                for(int p = 0; p < 72; p += 9)              // нижние 8 не заноси?тк он?дублируетс€ след куском
                {
                    for(int s = 0; s < 8; ++s)              // 9 значение ?строке не заноси?тк он?дублируетс€ след куском, ??правых-боковы?обрезает? дл€ 128?28
                    {
