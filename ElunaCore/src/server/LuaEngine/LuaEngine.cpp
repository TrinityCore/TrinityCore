...
{
    if (sLuaGameObjectScript->GetGameObjectBindingForId(gameObject->GetEntry()))
...
void Eluna::Restart()
{
    ...
    for (vector<GameObjectBind*>::iterator itr = get()->_gameObjectEventBindings.begin(); itr != get()->_gameObjectEventBindings.end(); ++itr)
    {
