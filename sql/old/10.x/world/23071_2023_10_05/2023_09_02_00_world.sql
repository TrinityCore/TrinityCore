...
UPDATE `creature_template` SET `faction`=2843, `BaseAttackTime`=1000, `unit_flags`=32768, `unit_flags2`=33556480 WHERE `entry`=99919; -- Sevis Brightflame
UPDATE `creature_template` SET `faction`=2189, `speed_walk`=4, `speed_run`=1.428571462631225585, `BaseAttackTime`=2000, `unit_flags`=33554432, `unit_flags2`=1073743872, `unit_flags3`=524288 WHERE `entry`=100931; -- Fel Cannon

UPDATE `gameobject_template` SET `ContentTuningId`=699, `VerifiedBuild`=50791 WHERE `entry`=242987; -- Jailer Cage
UPDATE `gameobject_template` SET `ContentTuningId`=699, `VerifiedBuild`=50791 WHERE `entry`=241751; -- Legion Gateway Activator
UPDATE `gameobject_template` SET `ContentTuningId`=699, `VerifiedBuild`=50791 WHERE `entry`=259043; -- Legion Banner 02
UPDATE `gameobject_template` SET `ContentTuningId`=699, `VerifiedBuild`=50791 WHERE `entry`=250560; -- Legion Banner
UPDATE `gameobject_template` SET `ContentTuningId`=699, `VerifiedBuild`=50791 WHERE `entry`=244439; -- Legion Communicator

-- Creature Difficulty
DELETE FROM `creature_template_difficulty` WHERE (`Entry`=100931 AND `DifficultyID`=0);
...
(100931, 0, 0, 0, 699, 6, 2, 1, 101449, 1024, 16, 50791); -- Fel Cannon

-- Creature Sparring data
DELETE FROM `creature_template_sparring` WHERE `Entry` IN (99919, 98458, 98459, 98460, 98456, 98457);
...
-- Scene
DELETE FROM `scene_template` WHERE `SceneId`=1116;
INSERT INTO `scene_template` (`SceneId`, `Flags`, `ScriptPackageID`, `Encrypted`) VALUES
(1116, 27, 1493, 0);

-- Phasing
DELETE FROM `phase_area` WHERE `PhaseId` = 5310 AND `AreaId` = 7741;
INSERT INTO `phase_area` (`AreaId`, `PhaseId`, `Comment`) VALUES
(7741, 5310, 'Cosmetic - The Invasion Begins - Dispair Ridge Spawns');

-- Conditions
DELETE FROM `conditions` WHERE (`SourceTypeOrReferenceId`=26 AND `SourceGroup` = 5310 AND `SourceEntry` = 0) OR (`SourceTypeOrReferenceId`=26 AND `SourceGroup` = 5311 AND `SourceEntry` = 0);
INSERT INTO `conditions` (`SourceTypeOrReferenceId`, `SourceGroup`, `SourceEntry`, `SourceId`, `ElseGroup`, `ConditionTypeOrReference`, `ConditionTarget`, `ConditionValue1`, `ConditionValue2`, `ConditionValue3`, `NegativeCondition`, `Comment`) VALUES
(26, 5310, 0, 0, 0, 47, 0, 40077, 8, 0, 0, 'Apply Phase 5310 if Quest 40077 is in progress'),
(26, 5311, 0, 0, 0, 47, 0, 40077, 2|8|64, 0, 1, 'Apply Phase 5311 if Quest 40077 is not complete | in progress | rewarded');

 -- Legion Banner smart ai
SET @ENTRY := 250560;
...
(@ENTRY, 1, 0, 0, 8, 0, 100, 0, 6477, 0, 0, 0, 85, 191677, 0, 0, 0, 0, 0, 7, 0, 0, 0, 0, 0, 0, 0, 'On spell  6477 hit - Caster: Cast spell 191677 on self'),
(@ENTRY, 1, 1, 0, 8, 0, 100, 0, 6477, 0, 0, 0, 85, 82238, 0, 0, 0, 0, 0, 7, 0, 0, 0, 0, 0, 0, 0, 'On spell  6477 hit - Caster: Cast spell 82238 on self');
