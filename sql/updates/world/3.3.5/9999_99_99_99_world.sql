-- Zephyr
UPDATE `creature_template` SET `AIName` = 'SmartAI', `ScriptName` = '' WHERE `entry` = 25967;
...
DELETE FROM `conditions` WHERE `SourceTypeOrReferenceId`=15 AND `SourceGroup`=9205 AND `SourceEntry`=0 AND `SourceId`=0;
...
-- Flask vendors
UPDATE `creature_template` SET `AIName` = 'SmartAI', `ScriptName` = '' WHERE `entry` IN (23483,23484);

-- Fix 8752 second gossip
INSERT INTO `gossip_menu` (`MenuID`, `TextID`, `VerifiedBuild`) VALUES
(8752,11085,0);

-- TODO: need gossip_menu_option BroadcastTextID for these NPC's - check against live
