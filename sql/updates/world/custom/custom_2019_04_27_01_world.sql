-- Missing Twilight Proveditor Spawn Controller Bunny 02
SET @GUID := 252467;
...
(@GUID,39600,1,0,0,1,1,169,0,0,0,5296.32,-2313.46,1433.643,0.1745329,120,0,0,0,0,0,0,0,0,'',0),
(@GUID+1,39600,1,0,0,1,1,169,0,0,0,5239.36,-2372.2,1432.823,0.9250245,120,0,0,0,0,0,0,0,0,'',0);

UPDATE `quest_template_addon` SET `SpecialFlags`=2 WHERE `ID`=25325;
UPDATE `creature_addon` SET `emote`=333 WHERE `guid` IN (384409,384410);

DELETE FROM `creature_template_movement` WHERE `CreatureId` IN (35592, 39600, 40240, 40650, 40250, 40190);
...
UPDATE `creature_template` SET `flags_extra`=128 WHERE `entry` IN (39600);
UPDATE `creature_template` SET `npcflag`=16777216,`unit_flags`=512,`unit_flags2`=67110912  WHERE `entry` IN (40190);
...
DELETE FROM `quest_template_addon` WHERE `ID` IN (25233,25234,25255,25269,25268);
