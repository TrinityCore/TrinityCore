...
DELETE FROM `smart_scripts` WHERE `entryorguid`=30210;
...
SET @GUID_GO                  := XXXXX; -- set by TDB
SET @GUID_NPC                 := XXXXX; -- set by TDB (need 2)
...
UPDATE `gameobject_template` SET `flags`=`flags`|32|4 WHERE `entry`=@HELM;
...
(@SPELL_READ_PRONOUNCEMENT,'spell_gen_read_pronouncement');
