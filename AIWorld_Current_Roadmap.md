...
**Runtime evidence (2026-09-21) — první živý kill test, RUNTIME PASS s jedním nálezem:** zabití NPC skutečně prochází celou cestou `KillRewarder::_RewardPlayer()` → `Player::RewardReputation()` → `ReputationMgr::ModifyReputation()` → client update → DB persistence, potvrzeno pro správnou faction (1201-1204) a okamžitou změnu v UI. Test ale odhalil sémantickou chybu v hodnotě: `2026_09_16_02_world.sql` mělo `RewOnKillRepValue1 = 1` (podle `onkill_rep_value = 1` v CSV) - zabití člena WorldFaction tak reputaci s tou samou faction zvyšovalo, místo aby ji snižovalo (zabil jsi jednoho z nich - to je nepřátelský čin vůči nim, ne laskavost). Opraveno: `onkill_rep_value` v `world_faction_assignments.csv` je teď `-1` pro všech 32 `CLONED` řádků; nová korektivní migrace `sql/updates/world/3.3.5/2026_09_21_00_world.sql` (`UPDATE ... SET RewOnKillRepValue1 = -1`, cross-checked skriptem proti CSV) - **ne** přepsání `2026_09_16_02_world.sql`, protože to je na nasazeném serveru už aplikované a DB updater migraci se stejným názvem znovu neaplikuje. V1 zůstává úmyslně jednoduché: zabití člena Defias/Riverpaw/Kobolds/Murlocs sníží reputaci jen u té samé faction - žádný automatický `+1` nepřátelské frakci (např. Stormwind), to je samostatná, zatím nenavržená reputation/diplomacy reward policy.
...
opravená hodnota re-test            PENDING       (další kill po opravě ještě neproběhl)
```
