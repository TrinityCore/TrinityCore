...
**Runtime evidence (2026-09-16) — `creature_onkill_reputation`:** `sql/updates/world/3.3.5/2026_09_16_02_world.sql` přidává reputation reward za zabití pro přesně těch 32 entries, které už mají `FactionTemplate` klon (`tc_faction_action = CLONED` v `world_faction_assignments.csv`, nový sloupec `onkill_rep_value`) - cross-checked skriptem proti CSV stejným způsobem jako 2026_09_16_01. `RewOnKillRepFaction1` je přímo `factions.csv`'s `id` (1201-1204); `MaxStanding1 = 8` (jedna hodnota nad `REP_EXALTED`), protože `Player::RewardReputation()` by s `MaxStanding1 = 0` blokoval zisk reputace hned od prvního killu (tyto frakce startují na `REP_NEUTRAL`, což je už `> 0`). V1 flat baseline: stejná hodnota za kill pro common trash i unique named NPC - tiering podle vzácnosti je vědomě odložený.

**Runtime evidence (2026-09-21) — první živý kill test, RUNTIME PASS s jedním nálezem:** zabití NPC skutečně prochází celou cestou `KillRewarder::_RewardPlayer()` → `Player::RewardReputation()` → `ReputationMgr::ModifyReputation()` → client update → DB persistence, potvrzeno pro správnou faction (1201-1204) a okamžitou změnu v UI. Test ale odhalil sémantickou chybu v hodnotě: `2026_09_16_02_world.sql` mělo `RewOnKillRepValue1 = 1` (podle `onkill_rep_value = 1` v CSV) - zabití člena WorldFaction tak reputaci s tou samou faction zvyšovalo, místo aby ji snižovalo (zabil jsi jednoho z nich - to je nepřátelský čin vůči nim, ne laskavost). Opraveno na `-1` v `2026_09_21_00_world.sql`.

**Druhé kolo (2026-09-21) — `-1` samo o sobě je fragilní kvůli zaokrouhlování:** `Player::CalculateReputationGain()` aplikuje faction/racial modifiery jako procenta a `CalculatePct()` (`src/common/Utilities/Util.h`) výsledek do `int32` ořezává směrem k nule, nezaokrouhluje. Jakýkoliv modifier jiný než přesně 100 % (např. Human racial "Diplomacy", `+10 %` k `SPELL_AURA_MOD_REPUTATION_GAIN`) tak dokáže hodnotu `-1` ztratit úplně na `0`: `-1 × 90 % = -0.9`, `int32(-0.9f) = 0` - `ModifyReputation()` pak dostane nulu a standing se nezmění, i když celá cesta jinak proběhne správně. Ověřeno staticky v kódu (`Util.h:71-74`, `T(base * pct / 100.0f)`), živý test s Human postavou to potvrdil pozorovaným chováním. Opraveno: `onkill_rep_value` je teď `-10` pro všech 32 `CLONED` řádků (`-10 × 90 % = -9.0`, žádná ztráta zlomku) - nová korektivní migrace `sql/updates/world/3.3.5/2026_09_21_01_world.sql` (na `2026_09_21_00_world.sql`, ne místo něj). `-10` je zároveň rozumnější V1 gameplay magnituda - `-1`/kill by potřeboval tisíce killů na posun o jeden standing tier.
...
**Stav (2026-09-19) — participation → ControlMode enforcement (follow-up code review fix):** druhé kolo code review odhalilo, že `VanillaOnly`-detekce z předchozího kroku byla jen `TC_LOG_ERROR`, ale `record->ControlMode` se nikdy neopravil - `OwnsSpawn()` je autoritativní jen podle `AgentRecord::ControlMode`, takže deklarovaný invariant ("VanillaOnly nesmí být AIWorldControlled") nebyl reálně vynucený. Zároveň celá participation-safety (`ExcludedButBound` quarantine i VanillaOnly kontrola) žila jen uvnitř `RunSpawnReconciliation()`, tedy závisela na `AIWorld.EnableSpawnReconciliation` - bezpečnostní invariant neměl záviset na volitelném data-reconciliation feature flagu. A `RunZoneControlActivation()` pořád pracoval s raw zone census bez jakéhokoliv ohledu na participation, takže: (a) po zavedení `ApplyParticipationControlPolicy` (níže) by chybějící `AgentRecord` u `Excluded` spawnu způsobil trvalé `notYetReconciled > 0` a **trvalé odmítnutí celé zone activation**, (b) `VanillaOnly` mohl být mezi promotion kandidáty.

Oprava:
- Nová `AIWorldMgr::ApplyParticipationControlPolicy()` - volaná bezpodmínečně v `Initialize()` hned za `_spawnParticipationCatalog.Load()`, PŘED `AIWorld.EnableSpawnReconciliation` gate. Prochází celý `AgentRegistry` (ne jen jednu zónu) přes nový `SpawnParticipationCatalog::TryResolve()` (na rozdíl od `Resolve()` rozlišuje "explicitně Excluded" od "mimo scope katalogu" - důležité, protože scan celého registry jinak přes `Resolve()`'s fail-closed-Excluded fallback chybně quarantinuje úplně každého agenta mimo Elwynn).
- `Excluded` → quarantine z `_registry` (stejná politika jako dřív, `ai_agents`/`control_mode` beze změny).
- `VanillaOnly` + `AIWorldControlled` → **skutečná persistovaná demotion** přes nový `AgentPersistence::DemoteControlModeBatch()` (mirror `PromoteControlModeBatch`, batch + read-back confirm) - na rozdíl od `Excluded` tady DB zápis dává smysl (jen 4 spawny, jasně a natrvalo špatný stav, ne nejistá historická populace).
- `RunSpawnReconciliation()`'s vlastní `ExcludedButBound`/`vanillaOnlyControlModeViolations` pass zůstávají jako sekundární potvrzovací kontrola, když reconciliation běží (`ExcludedButBound` je teď typicky no-op, protože `ApplyParticipationControlPolicy` už agenta odstranila; `vanillaOnlyControlModeViolations` by mělo být vždy `0`, nenulová hodnota znamená, že demotion selhala na read-back).
- `RunZoneControlActivation()` teď `Excluded`/`VanillaOnly` identity úplně přeskočí (nepočítají se do `scopedEligibleCount`, nikdy nejsou `notYetReconciled` ani candidate) - `LightweightBackground` zůstává eligible stejně jako `FullAgent` (viz níže).
- CI: `vanillaOnlyControlModeViolations=0` je teď TVRDÝ assert (na rozdíl od `excludedEventQuarantined`, tohle se aktivně a persistentně opravuje, takže nenulová hodnota je regrese, ne historický drift).
...
**Stav (2026-09-21) — GM/debug interface pro runtime test membership:** `PlayerWorldFactionPersistence` byla dosud bez callera - `LoadMembership()`/`Join()`/`Leave()` se v runtime nedaly vůbec vyzkoušet. Přidán `.aiworld faction status|join <faction>|leave` (`src/server/scripts/Commands/cs_aiworld_faction.cpp`) - tenký wrapper nad existující persistencí (žádné přímé SQL v commandu), operuje na vlastní postavě GMa (stejná konvence jako `.gm on/off`), `RBAC_PERM_COMMAND_DEBUG` (existující base auth permission, žádná nová RBAC migrace). `join` má šest doslovných subcommand větví (stormwind/defias/riverpaw/kobolds/murlocs/wolves) - žádná joinable-policy validace na téhle vrstvě (to je vědomě odložené, viz `Join()`'s vlastní komentář), takže command dnes umožní i nesmyslné joiny (např. wolves) - to je pro debug účel žádoucí, ne bug. Dlouhodobě použitelný nástroj, ne dočasný startup hack. `WorldFactionRelationCatalog` zůstává i po tomhle kroku bez runtime callera - tenhle command na ni vůbec nesahá, pouze na `PlayerWorldFactionPersistence`.

**Oprava (2026-09-21, code review):** první verze commandu volala synchronní `LoadMembership()`/`Join()`/`Leave()` (`CONNECTION_SYNCH`/`DirectExecute()`) přímo z chat command handleru - `CMSG_MESSAGECHAT` je ale `PROCESS_THREADUNSAFE` (`Opcodes.cpp`) a zpracovává se inline na `World::UpdateSessions()`, tedy přímo na world threadu. To je přesně ten "world update loop", který `PlayerWorldFactionPersistence.h`'s vlastní komentář zakazuje - `Join()` navíc dělal až tři synchronní DB round-tripy (load → write → confirm), takže pomalá DB mohla zastavit celý world tick. Přidány asynchronní `LoadMembershipAsync()`/`JoinAsync()`/`LeaveAsync()` (`QueryCallback`, chaining přes `WithChainingPreparedCallback()`/`SetNextQuery()`, mirror `WorldSession::HandleCharCreateOpcode()`'s vlastního multi-step chain v `CharacterHandler.cpp`) - command teď registruje výsledek přes `session->GetQueryProcessor().AddCallback(...)`, DB práce běží na DB worker threadu, výsledek se aplikuje (`SendNotification`) až později na world threadu, bez blokování. Zároveň opraveno: `WorldFactionDisplayName()` už neskrývá neznámé/corrupt `world_faction_id` jako "Unaffiliated" - vrací `UNKNOWN(<id>)` pro cokoliv mimo známou šestici.

```text
.aiworld faction command       STATIC PASS (kompilace ověřena, registrováno v cs_script_loader.cpp, async DB dle QueryCallback)
