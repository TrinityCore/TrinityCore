...
#        重要:对于这个变量重要事项注释.
#        例子:  "如果值是一个字符串"
#        默认:   10 - (Enabled(启用）|Comment(注释)|在分别配置选项的情况下变量)
#                0  - (Enabled(启用）|Comment(注释)|在分别配置选项的情况下变量
#
# Note to developers:
...
#    服务器编号
#        描述:使用本配置的服务器编号.
#        重要:登陆服务器设置，服务器编号必须匹配在auth数据库中的Realmlist表.
#        默认: 1
#
...
DataDir ="D:\Program Files (x86)\World of Warcraft\Data"
#DataDir ="..\Data" (曾经使用的目录)
...
WorldDatabaseInfo     =127.0.0.1;3306;trinity;trinity;tc-world
...
#        Default:     8085
#        世界服务器worldserver占用的系统端口，默认：8085
...
#
#    UseProcessors
#       在多线程系统 使用线程MASK(只在Windows系统下使用)，默认：0(自动选择)，数值(设置处理器的线程值)
#
...
#
#    Compression
#        发送到客户端的更新数据包的压缩级别(1到9)，默认: 1 (速度快,不压缩)，9 (速度慢，最好的压缩)
...
#
#    MaxOverspeedPings
#        在玩家被踢出之前，最大的超速限制，(最小值是2, 0为取消检测)，默认：2
...
#        Grids就是地图上刷出来的东西,玩家经过一个地方 就会加载该地方的环境,离开的时候 就卸载该地方的环境
...
#
#    SocketTimeOutTime
#        当玩家在选择人物界面超过多少时间与客户端断开连接，默认：900000 (15分钟)
...
#
#    SessionAddDelay
#        在多少微秒后网络线程验证身份后将休眠，协议将添加一个连接到世界会话上  默认: 10000 (10毫秒，0.01秒)
...
#
#    GridCleanUpDelay
#        默认地图网格清理延迟(单位毫秒)，默认: 300000 (5 min)
...
#         默认天气更新间隔 (单位毫秒)，默认: 600000 (10 min)
...
#
#    PlayerSaveInterval
#        默认玩家数据保存间隔(单位毫秒)，默认: 600000 (10 分钟)
...
#        启用在玩家退出游戏时保存玩家数据，默认：1(仅在玩家退出游戏时)，0(使用时间循环保存数据)
...
#        开启/关闭mmap寻路系统支持，开启后需要,mmaps文件支持-依然处在开发中的功能
...
#        开启/关闭VMmap室内检查，开启后需要Vmaps文件支持，默认:1 (开启) 0 (关闭) 
...
#
#    CleanCharacterDB
#        从数据库清理过期的法术，技能和天赋 默认: 0 (关闭)1 、(允许)
...
#        确定是否彻底清理干净，默认: 0 (所有的清理方法禁用)14、(清理的技能，天赋和技能将继续启用)
...
#
#    PidFile
#        开启否创建登录控制器trinityCore系统进程档案记录PID文件世界服务器保护进程名称，默认""
...
#
#    PacketLogFile
#        世界服务器二进制数据包的日志文件为，默认""，world.bin开启
...
#
#        服务登录区域设置 (设置服务器支持的区域字符表/服务器名称列表等的正确显示)，默认：1(全地区)
#                     1  - (任何语言)
#                     2  - (美国扩展拉丁语)
#                     3  - (海洋扩展拉丁语)
#                     4  - (拉丁美洲的美国-扩展拉丁语)
...
#        Default:     0 - (可以限制服务器的时区，从客户端检查)
#                     1 - (启用，严格基本拉丁字符)
#                     2 - (realmzone设置自定义字体需要 放置在clientdir /字体。)
#                     3 - (启用，基本拉丁字符+特定服务器的时区)

StrictPlayerNames = 0

#
#    StrictCharterNames
#        公会/竞技团队名称文字规定，默认：0(全符号支持) 参照角色命名规定
...
#
#    Expansion
#        开放内容选择，0(普通)，1(燃烧的远程)，2(巫妖王之怒),3(大灾变),4(熊猫人之谜),5(德拉诺之王),6(军团再临),7(争霸艾泽拉斯),8(暗影国度),9(巨龙时代)
...
#        禁用某个阵营的玩家角色创造，默认：0 (所有阵营都允许) 1(禁用联盟)2(禁用部落)3(禁用俩阵营)
...
#        禁用某个阵营的玩家角色创造，默认：0 (所有种族都允许)，1(禁用人类)，2(禁用兽人)，4(禁用矮人)，8(禁用暗夜精灵)，16(禁用不死)，32(禁用牛头人),64(禁用巨魔)，128(禁用侏儒)，512(禁用血精灵),1024(禁用德莱尼)
#        举例 1536 - (1024 + 512, 血精灵和德莱尼禁止创建)
...
#        禁用某个阵营的玩家角色创造，默认：0 (所有种族都允许)，1(禁用战士)，2(禁用圣骑士)，4(禁用猎人)，8(禁用盗贼)，16(禁用牧师)，32(禁用牛头人),64(禁用巨魔)，128(禁用侏儒)，512(禁用血精灵),1024(禁用德莱尼)
#        举例  288 - (32 + 256, 禁用死亡骑士和术士)
...
#        每个账号在服务器列表中所有服务器上允许创建的最多角色数量，默认：60
...
#
#    AllFlightPaths
#        角色创建后激活所有飞行点 (注:所有飞行点,不只包括角色所在阵营)，默认：0(禁用)，1(启用)
...
#
#    InstantFlightPaths
#        飞行点将使玩家立即传送到目的地，而不用进行飞行等待，默认：0(不使用)，1(使用)
...
#        玩家可以看到NPC头上的感叹号的任务等级比自己低的级数。默认:7
#        Description: Level difference between player and quest level at which quests are
...
#        重置天赋时检查副手武器栏，默认:1 - 在重置天赋的同时检查并重置，0 - 只在更换地图，登陆时检查并重置
...
#
#    Motd
#        每日问候消息被显示在世界聊天框('@' 为换行符)默认：elcome to John\'s Server@This server runs on Trinity Core)
...
#        .lookup command 使用查找命令显示最大结果，默认：0
...
#        地下城查找器系统(包含团队副本二进制掩码). 默认：1(使用地下城查找器系统),0(使用团队查找器系统)
...
#        不允许从数据库重写存在DBC文件值储的项目属性。,默认：1(启用),0(关闭)
...
#        经过多少秒之后，客户端断开不相应连接，默认 600 (10分钟), 0 (关闭，客户端不会踢人)
...
#        经过多少时间（秒）在发送下一个客户端检查请求，默认 (30秒),0(尽快使用人工检查)
...
#        如果客户检查失败了则 0 (可以行动),1(踢出游戏),2(禁止登陆)
...
#
#    Warden.BanDuration
#        将被禁止帐户的时间（秒）默认86400(24小时)，0(永久性)
...
#
#    TalentsInspecting
#        是否允许查看不同阵营的角色天赋，不影响Game mater的角色，默认：0(不允许),1(允许)。
...
#
#    ThreatRadius
#        生物脱离战斗的半径，当角色与生物距离大约该距离时，生物脱离战斗，默认：60码
...
#
#    Rate.Creature.Aggro
#        怪物自动攻击的范围乘以的倍数或者关闭自动攻击，默认：1，1.5(150%),0(0%)
...
#        创建假冒消息，使用大量含有空格或其他聊天保护，默认：0 (关闭),1 (启用)
...
#        检查游戏内链接法术，物品，任务等聊天消息，默认：0 (禁用)，1(检查通道防止发布图片),2(检查命令被用于正确顺序),3(颜色，名称不相互矛盾，需要DBC支持)
...
#        如果消息被认为包含无效该做什么，默认: 0 (忽略消息)，1 (断开发送错误消息的玩家)。
...
#        玩家被防灌水系统禁言前，可以发送的消息数目 默认：10(启用),0(禁用)。
...
#        是否启用自定义聊天频道，如密码，自动所有权等功能，或者在数据库中设置禁止名单，默认：1 (启用)，0,(如果最后一个人离开频道，所有设置无效)
...
#        自定义频道经过多少天从数据库删除，默认：14(清理14天未使用的频道) 0 (关闭，无限的存储)
...
#
#    GM.LoginState
#        是否在登录时启用GM模式，默认：0 (关闭)，1 (启用)，2 (最后保存的模式)     
...
#
#    GM.Visible
#        是否在登录时启用GM能见模式，默认：0 (关闭)，1 (启用)，2 (最后保存的模式)   
...
#
#    GM.Chat
#        是否在登录时启用GM聊天模式，默认：0 (关闭)，1 (启用)，2 (最后保存的模式)  
...
#
#    GM.WhisperingTo
#        是否启用GM接受私聊系统, 默认：1 (启用)，0 (关闭)，2 (最后保存的模式)  
...
#
#    GM.InGMList.Level
#        启用GM模式，显示在ingame列表中最低GM级别，默认：3 (显示任何人)，0 (只显示玩家)，1 (显示客服人员)，2 (显示游戏管理员)
...
#
#    GM.LowerSecurity
#        是否允许使用较低的安全级别更高的安全级别上的命令，默认：0 (禁用) 1 (启用)
...
#        能见度更新周期的时间（毫秒）。较低的值可能有性能的影响,
#        默认: 1000 - (OnContinents 在陆地上的能见度限定) 1000 
#       (在副本上的能见度限定)  1000 - (InBGArenas 在战争竞技场中的能见度限定)
...
#    Rate.Drop.Money
#        粗糙普通灰色、普通普通白色、优秀绿色、精良蓝色、史诗紫色、传说橙色、神器金色、任务物品、金币爆率倍率
...
#                     Flags:
#                         0 - None
#                         1 - Prefix Timestamp to the text		文本前添加前缀
#                         2 - Prefix Log Level to the text		文本前添加日志等级
#                         4 - Prefix Log Filter type to the text	文本前添加日志筛选器类型
#                         8 - Append timestamp to the log file name. Format: YYYY-MM-DD_HH-MM-SS
#                             (Only used with Type = 2)		为日志文件名附加时间戳.格式:年-月-天_时_分_秒(j仅当使用类型2<控制台>时)
#                        16 - Make a backup of existing file before overwrite
#                             (Only used with Mode = w)		在重写已存在日志前做个备份(仅模式为w时使用)
#
...
Appender.Console=1,3,1,"1 9 3 6 5 8"
Appender.Server=2,5,1,Server.log,w
Appender.GM=2,6,1,GM.log
Appender.DBErrors=2,3,1,DBErrors.log
...
#
#	NPC8N0:8000000
#	GM使用方法 用有GM权限的账号 输入 .npc add 8000000
#	就可以看到天使姐姐了
...
#是否开启单人进随机地下城查找器,1开启(默认,不设置为0即开启)
SoloLFG.Enable = 1
...
#技能
Spell.Craft.Dungeon = 5

#急速是否启用,关闭后,以猎人为例:恢复集中值非常慢
Leech.Craft.Enable = 1

#急速倍率
Leech.Craft.Dungeon = 5

#地下城战斗划分等级是否开启公告
Combat.Rating.Craft.Announce = 0
#怀疑是公告,发现公告还存在
...
#地下城战斗划分等级
Combat.Rating.Craft.Dungeon = 2
#此处改为1,20级装备乱七八糟的野兽猎刷法力陵墓眼镜蛇射击1833(左右),接近秒
#设为5,伤害2300左右
#改为0.5,伤害还是2300,估计不能用小数.也可能是每次修改后,角色需要小退.(因为发现把Combat.Rating.Craft.Enable设为0后,还有加成).
#结论:改Combat.Rating.Craft.Dungeon的数值可能立即生效
#小退也没用,角色大退发现生效了
#开启后,除了伤害加强,防御也增加了.地下城的怪基本打不死角色(多怪谨慎).
