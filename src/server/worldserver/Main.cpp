...
void ClearOnlineAccounts()
{
    ...
    LoginDatabase.DirectPExecute("UPDATE users_user SET online = 0 WHERE online > 0 AND id IN (SELECT acctid FROM realmcharacters WHERE realmid = %d)", realm.Id.Realm);
