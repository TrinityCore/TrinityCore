...
variables_map GetConsoleArguments(int argc, char** argv, std::string& cfg_file);
...
variables_map GetConsoleArguments(int argc, char** argv, std::string& cfg_file)
{
    ...
    all.add_options()
        ...
        ("config,c", value<std::string>(&cfg_file)->default_value(_TRINITY_REALM_CONFIG), "use <arg> as configuration file")
        ;
    variables_map vm;
    try
    {
