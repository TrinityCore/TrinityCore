...
extern int main(int argc, char** argv)
{
    ...
    // socialserver PID file creation
...
#ifdef _WIN32 // Windows
    {
        ...
        if (affinity > 0)
        {
            ...
            if (GetProcessAffinityMask(hProcess, &appAff, &sysAff))
            {
                ...
                if (!currentAffinity)
                    TC_LOG_ERROR(LOG_FILTER_SOCIALSERVER, "Processors marked in UseProcessors bitmask (hex) %x are not accessible for the socialserver. Accessible processors bitmask (hex): %x", affinity, appAff);
        ...
        if (highPriority)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                TC_LOG_INFO(LOG_FILTER_SOCIALSERVER, "socialserver process priority class set to HIGH");
            else
                TC_LOG_ERROR(LOG_FILTER_SOCIALSERVER, "Can't set socialserver process priority class.");
        }
    }
    ...
    if (highPriority)
    {
        if (setpriority(PRIO_PROCESS, 0, PROCESS_HIGH_PRIORITY))
            TC_LOG_ERROR(LOG_FILTER_SOCIALSERVER, "Can't set socialserver process priority class, error: %s", strerror(errno));
        else
            TC_LOG_INFO(LOG_FILTER_SOCIALSERVER, "socialserver process priority class set to %i", getpriority(PRIO_PROCESS, 0));
    }
#endif

    zmqpp::context* ctx = new zmqpp::context();
    zmqpp::socket* inproc = new zmqpp::socket(*ctx, zmqpp::socket_type::pub);
    inproc->set(zmqpp::socket_option::linger, 0);
    ...
    {
        stopEvent = true;
        TC_LOG_ERROR(LOG_FILTER_SOCIALSERVER, "An exception has occurred during initialization: %s", ex.what());
        // If exception was thrown in open(void*) then it will never run HandleClose by itself
        if (!broker.thr_count())
            broker.close();
        if (!puller.thr_count())
            puller.close();
    ...
    TC_LOG_INFO(LOG_FILTER_SOCIALSERVER, "%s (socialserver) ready...", _FULLVERSION);
...
bool StartDB()
{
    ...
    if (dbstring.empty())
    {
        TC_LOG_ERROR(LOG_FILTER_SOCIALSERVER, "Database not specified");
        return false;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SOCIALSERVER, "Improper value specified for LoginDatabase.WorkerThreads, defaulting to 1.");
        worker_threads = 1;
    }
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SOCIALSERVER, "Improper value specified for LoginDatabase.SynchThreads, defaulting to 1.");
        synch_threads = 1;
    }

    // NOTE: While socialserver is singlethreaded you should keep synch_threads == 1. Increasing it is just silly since only 1 will be used ever.
    if (!LoginDatabase.Open(dbstring.c_str(), uint8(worker_threads), uint8(synch_threads)))
    {
        TC_LOG_ERROR(LOG_FILTER_SOCIALSERVER, "Cannot connect to database");
        return false;
    }
