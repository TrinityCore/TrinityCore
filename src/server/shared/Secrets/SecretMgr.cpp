...
void SecretMgr::AttemptLoad(Secrets i, LogLevel errorLevel, std::unique_lock<std::mutex> const&)
{
    ...
    if (
        ...
        (oldDigest && !Kitron::Crypto::Argon2::Verify(currentValue->AsHexStr(), *oldDigest)) // there is an old digest, and the current secret does not match it
        )
    {
        ...
        if (oldDigest && info.oldKey) // there is an old digest, so there might be an old secret (if possible)
        {
            ...
            if (oldSecret && !Kitron::Crypto::Argon2::Verify(oldSecret->AsHexStr(), *oldDigest))
            {
...
Optional<std::string> SecretMgr::AttemptTransition(Secrets i, Optional<BigNumber> const& newSecret, Optional<BigNumber> const& oldSecret, bool hadOldSecret) const
{
    ...
    switch (i)
    {
        case SECRET_TOTP_MASTER_KEY:
        {
            ...
            if (result) do
            {
                ...
                {
                    if (!oldSecret)
                        return Kitron::StringFormat("Cannot decrypt old TOTP tokens - add config key '%s' to authserver.conf!", secret_info[i].oldKey);

                    bool success = Kitron::Crypto::AEDecrypt<Kitron::Crypto::AES>(totpSecret, oldSecret->ToByteArray<Kitron::Crypto::AES::KEY_SIZE_BYTES>());
                    if (!success)
                        return Kitron::StringFormat("Cannot decrypt old TOTP tokens - value of '%s' is incorrect for some users!", secret_info[i].oldKey);
                }

                if (newSecret)
                    Kitron::Crypto::AEEncryptWithRandomIV<Kitron::Crypto::AES>(totpSecret, newSecret->ToByteArray<Kitron::Crypto::AES::KEY_SIZE_BYTES>());
    ...
    if (newSecret)
    {
        BigNumber salt;
        salt.SetRand(128);
        Optional<std::string> hash = Kitron::Crypto::Argon2::Hash(newSecret->AsHexStr(), salt);
        if (!hash)
