...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_SESSIONKEY, "SELECT a.sessionkey, a.id, aa.permissionId  FROM account a LEFT JOIN rbac_account_permissions aa ON (a.id = aa.accountId) WHERE username = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_SEL_LOGONCHALLENGE, "SELECT a.sha_pass_hash, a.id, a.locked, a.lock_country, a.last_ip, aa.permissionId, a.v, a.s, a.token_key FROM account a LEFT JOIN rbac_account_permissions aa ON (a.id = aa.accountId) WHERE a.username = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_DEL_RBAC_PERMISSION_LEVEL, "DELETE FROM rbac_account_permissions WHERE permissionId = ?", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_DEL_RBAC_PERMISSION_LEVEL_BY_REALM, "DELETE FROM rbac_account_permissions WHERE permissionId = ? AND (realmId = ? OR realmId = -1)", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_INS_RBAC_PERMISSION_LEVEL, "INSERT INTO rbac_account_permissions (accountId, permissionId, granted, realmId) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_GET_ACCOUNT_ID_BY_USERNAME, "SELECT id FROM account WHERE username = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_GET_RBAC_PERMISSION_LEVEL, "SELECT permissionId FROM rbac_account_permissions WHERE accountId = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_GET_RBAC_PERMISSION_LEVEL_BY_REALMID, "SELECT permissionId FROM rbac_account_permissions WHERE accountId = ? AND (realmId = ? OR realmId = -1)", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_SEL_RBAC_PERMISSION_LEVEL_ACCOUNTS, "SELECT a.username, aa.permissionId FROM account a, rbac_account_permissions aa WHERE a.id=aa.accountId AND aa.permissionId >= ? AND (aa.realmid = -1 OR aa.realmid = ?)", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO, "SELECT a.username, a.last_ip, aa.permissionId, a.expansion FROM account a LEFT JOIN rbac_account_permissions aa ON (a.id = aa.accountId) WHERE a.id = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_RBAC_PERMISSION_LEVEL_TEST, "SELECT 1 FROM rbac_account_permissions WHERE accountId = ? AND permissionId > ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_RBAC_PERMISSION_LEVEL, "SELECT a.id, aa.permissionId, aa.realmId FROM account a LEFT JOIN rbac_account_permissions aa ON (a.id = aa.accountId) WHERE a.username = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_SEL_RBAC_PERMISSION_LEVEL_BY_ID, "SELECT permissionId, RealmID FROM rbac_account_permissions WHERE accountId = ? and (realmId = ? OR realmId = -1) ORDER BY permissionId desc", CONNECTION_SYNCH);
