...
void ObjectMgr::LoadSingleItemTemplate(std::string entry)
{
    ...
    QueryResult result = WorldDatabase.PQuery("SELECT entry, class, subclass, SoundOverrideSubclass, name, displayid, Quality, Flags, FlagsExtra, BuyCount, BuyPrice, SellPrice, InventoryType, "
        ...
        "GemProperties, RequiredDisenchantSkill, ArmorDamageModifier, duration, ItemLimitCategory, HolidayId, ScriptName, DisenchantID, "
        //                                           134        135            136
        "FoodType, minMoneyLoot, maxMoneyLoot, flagsCustom FROM item_template WHERE entry = %s", entry);

    //QueryResult resultNum = WorldDatabase.Query("SELECT entry FROM item_template");
...
void ObjectMgr::LoadCustomItemTemplates()
{
    ...
    do
    {
        ...
        itemTemplate.StatsCount = fields[1].GetUInt32();
        for (uint8 i = 0; i < itemTemplate.StatsCount; ++i)
        {
            itemTemplate.ItemStat[i].ItemStatType = fields[2 + (i * 2)].GetUInt32();
            itemTemplate.ItemStat[i].ItemStatValue = fields[3 + (i * 2)].GetUInt32();
        }
    } while (result->NextRow());
}
