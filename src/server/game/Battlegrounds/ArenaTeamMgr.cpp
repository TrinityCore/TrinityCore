...
ArenaTeamMgr::~ArenaTeamMgr()
{
    for (std::pair<uint32 const, ArenaTeam*>& teampair : ArenaTeamStore)
...
ArenaTeam* ArenaTeamMgr::GetArenaTeamByName(const std::string& arenaTeamName) const
{
    ...
    for (std::pair<uint32 const, ArenaTeam*> const& teampair : ArenaTeamStore)
    {
        std::string teamName = teampair.second->GetName();
        std::transform(teamName.begin(), teamName.end(), teamName.begin(), ::toupper);
        if (search == teamName)
            return teampair.second;
    }
    return nullptr;
}
...
{
    for (std::pair<uint32 const, ArenaTeam*> const& teampair : ArenaTeamStore)
        if (teampair.second->GetCaptain() == guid)
            return teampair.second;
