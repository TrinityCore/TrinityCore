...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/)
{
    ...
    if (!go->Create(sObjectMgr->GuidGenerator.CreatureGuid.Generate(), entry, GetBgMap(),
...
Creature* Battleground::AddCreature(uint32 entry, uint32 type, float x, float y, float z, float o, TeamId /*teamId = TEAM_NEUTRAL*/, uint32 respawntime /*= 0*/)
{
    ...
    if (!creature->Create(sObjectMgr->GuidGenerator.CreatureGuid.Generate(), map, PHASEMASK_NORMAL, entry, x, y, z, o))
    {
