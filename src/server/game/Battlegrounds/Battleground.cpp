...
inline void Battleground::_CheckSafePositions(uint32 diff)
{
    ...
    if (m_ValidStartPositionTimer >= CHECK_PLAYER_POSITION_INVERVAL)
    {
        ...
        for (std::pair<ObjectGuid, BattlegroundPlayer> const& playerpair : GetPlayers())
        {
            if (Player* player = ObjectAccessor::FindPlayer(playerpair.first))
            {
...
inline void Battleground::_ProcessResurrect(uint32 diff)
{
    ...
    {
        for (ObjectGuid guid : m_ResurrectQueue)
        {
...
void Battleground::Reset()
{
    ...
    for (std::pair<uint32 const, BattlegroundScore*>& itr : PlayerScores)
        delete itr.second;
    PlayerScores.clear();

    for (ArenaTeamScore& arenaTeamScore : _arenaTeamScores)
...
void Battleground::BuildPvPLogDataPacket(WorldPacket& data)
{
    ...
    {
        for (ArenaTeamScore& arenaTeamScore : _arenaTeamScores)
            arenaTeamScore.BuildRatingInfoBlock(data);

        for (ArenaTeamScore& arenaTeamScore : _arenaTeamScores)
            arenaTeamScore.BuildTeamInfoBlock(data);
    }
    ...
    for (std::pair<uint32, BattlegroundScore*> const& score : PlayerScores)
...
{
    int count = 0;
    for (std::pair<ObjectGuid, BattlegroundPlayer> const& playerpair : m_Players)
    {
        if (playerpair.second.Team == Team)
        {
            Player* player = ObjectAccessor::FindPlayer(playerpair.first);
