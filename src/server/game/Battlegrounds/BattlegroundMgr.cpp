...
{
    for (std::pair<BattlegroundTypeId const, BattlegroundData>& itr1 : bgDataStore)
    {
...
Battleground* BattlegroundMgr::GetBattlegroundThroughClientInstance(uint32 instanceId, BattlegroundTypeId bgTypeId)
{
    ...
    for (std::pair<uint32 const, Battleground*> const& battleground : it->second.m_Battlegrounds)
    {
        if (battleground.second->GetClientInstanceID() == instanceId)
            return battleground.second;
    }
