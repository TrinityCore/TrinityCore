...
DumpReturn PlayerDumpReader::LoadDump(std::string const& file, uint32 account, std::string name, uint32 guid)
{
    ...
    if (guid && guid < sObjectMgr->GetGenerator<HighGuid::Player>().GetNextAfterMaxUsed())
    {
        ...
        if (result)
            guid = sObjectMgr->sObjectMgr->GetGenerator<HighGuid::Player>().GetNextAfterMaxUsed();                     // use first free if exists
        else incHighest = false;
    }
    else
        guid = sObjectMgr->sObjectMgr->GetGenerator<HighGuid::Player>().GetNextAfterMaxUsed();
