        ...
        if (isEmpty || method == INSTANCE_RESET_GROUP_DISBAND || method == INSTANCE_RESET_CHANGE_DIFFICULTY)
        {
            ...
            if (isEmpty && instanceSave->CanReset())
            {
                if (map && map->IsDungeon() && SendMsgTo)
                {
                    ...
                    {
                        WorldSafeLocsEntry const * graveyardLocation = sObjectMgr->GetClosestGraveyard(instanceEntrance->target_X, instanceEntrance->target_Y, instanceEntrance->target_Z, instanceEntrance->target_mapId, SendMsgTo->GetTeam(), SendMsgTo->GetGUID(), SendMsgTo->GetName());
