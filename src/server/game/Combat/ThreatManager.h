...
class TC_GAME_API ThreatManager
{
    public:
        ...
        void ModifyThreatByPercent(Unit* target, float percent) { if (percent) ScaleThreat(target, 0.01f * (100.0f + percent)); }
    ...
    private:
        ...
        std::vector<std::pair<ObjectGuid, float>> _redirectInfo; // current redirection targets and percentages (updated from registry in ThreatManager::UpdateRedirectInfo)
        std::unordered_map<uint32, std::unordered_map<ObjectGuid, float>> _redirectRegistry; // spellid -> (victim -> pct); all redirection effects on us (removal individually managed by spell scripts because blizzard is dumb)
...
class TC_GAME_API ThreatReference
{
    public:
        ...
        float GetThreat() const { return std::max<float>(_baseAmount + _tempModifier, 0.0f); }
    ...
    protected:
        ...
        explicit ThreatReference(ThreatManager* mgr, Unit* victim) :
            ...
            _baseAmount(0.0f), _tempModifier(0.0f), _taunted(TAUNT_STATE_NONE)
        {
        ...
        OnlineState _online;
        float _baseAmount;
        float _tempModifier; // Temporary effects (auras with SPELL_AURA_MOD_TOTAL_THREAT) - set from victim's threatmanager in ThreatManager::UpdateMyTempModifiers
        TauntState _taunted;
