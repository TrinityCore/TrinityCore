...
class TC_GAME_API AuraEffect
{
    ...
    public:
        ...
        void ChangeAmount(int32 newAmount, bool mark = true, bool onStackOrReapply = false, AuraEffect const* src = nullptr);
        void RecalculateAmount(AuraEffect const* src = nullptr) { RecalculateAmount(GetCaster(), src); }
        void RecalculateAmount(Unit* caster, AuraEffect const* src = nullptr);
        bool CanBeRecalculated() const { return m_canBeRecalculated; }
        void SetCanBeRecalculated(bool val) { m_canBeRecalculated = val; }
        void HandleEffect(AuraApplication * aurApp, uint8 mode, bool apply, AuraEffect const* src = nullptr);
        void HandleEffect(Unit* target, uint8 mode, bool apply, AuraEffect const* src = nullptr);
        void ApplySpellMod(Unit* target, bool apply, AuraEffect const* src = nullptr);
