...
// such checks will speedup GreenPis change amount/send for client operations
...
void AuraEffect::HandleFeignDeath(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        */
        ...
        UnitList targets;
        GreenPis::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, target->GetMap()->GetVisibilityRange());
        GreenPis::UnitListSearcher<GreenPis::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, targets, u_check);
            ...
            switch (GetSpellInfo()->Id)
            {
                ...
                case 51690:
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        CellCoord p(GreenPis::ComputeCellCoord(target->GetPositionX(), target->GetPositionY()));
                        Cell cell(p);

                        GreenPis::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck u_check(target, radius);
                        GreenPis::UnitListSearcher<GreenPis::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck> checker(target, targets, u_check);

                        TypeContainerVisitor<GreenPis::UnitListSearcher<GreenPis::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
                        TypeContainerVisitor<GreenPis::UnitListSearcher<GreenPis::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
                    ...
                    Unit* spellTarget = GreenPis::Containers::SelectRandomContainerElement(targets);
