...
void AuraEffect::HandleAuraModIncreaseEnergy(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetId() == 64904)
        target->HandleStatModifier(unitMod, TOTAL_PCT, float(GetAmount()), apply);      //special case Hymn of Hope (percent)
    else
        target->HandleStatModifier(unitMod, TOTAL_VALUE, float(GetAmount()), apply);    // generic flat case
}
