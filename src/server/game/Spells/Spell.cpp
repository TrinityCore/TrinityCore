...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
        GreenPis::WorldObjectSpellConeTargetCheck check(coneAngle, radius, m_caster, m_spellInfo, selectionType, condList);
        GreenPis::WorldObjectListSearcher<GreenPis::WorldObjectSpellConeTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
        SearchTargets<GreenPis::WorldObjectListSearcher<GreenPis::WorldObjectSpellConeTargetCheck> > (searcher, containerTypeMask, m_caster, m_caster, radius);
        ...
        if (!targets.empty())
        {
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                GreenPis::Containers::RandomResizeList(targets, maxTargets);
            }
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (!unitTargets.empty())
    {
        ...
        if (maxSize && power != -1)
        {
            if (Powers(power) == POWER_HEALTH)
            {
                ...
                {
                    unitTargets.sort(GreenPis::HealthPctOrderPred());
            ...
            else
            {
                ...
                if (unitTargets.size() > maxSize)
                {
                    unitTargets.sort(GreenPis::PowerPctOrderPred((Powers)power));
                    unitTargets.resize(maxSize);
                }
            }
        }
        ...
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
        {
            ...
            GreenPis::Containers::RandomResizeList(unitTargets, maxTargets);
        }
    ...
    if (!gObjTargets.empty())
    {
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
        {
            ...
            GreenPis::Containers::RandomResizeList(gObjTargets, maxTargets);
        }
        for (std::list<GameObject*>::iterator itr = gObjTargets.begin(); itr != gObjTargets.end(); ++itr)
            AddGOTarget(*itr, effMask);
    }
...
void Spell::SelectImplicitTrajTargets()
{
    ...
    std::list<WorldObject*> targets;
    GreenPis::WorldObjectSpellTrajTargetCheck check(dist2d, m_targets.GetSrcPos(), m_caster, m_spellInfo);
    GreenPis::WorldObjectListSearcher<GreenPis::WorldObjectSpellTrajTargetCheck> searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
    SearchTargets<GreenPis::WorldObjectListSearcher<GreenPis::WorldObjectSpellTrajTargetCheck> > (searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, m_targets.GetSrcPos(), dist2d);
    if (targets.empty())
        return;

    targets.sort(GreenPis::ObjectDistanceOrderPred(m_caster));
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return NULL;
    GreenPis::WorldObjectSpellNearbyTargetCheck check(range, m_caster, m_spellInfo, selectionType, condList);
    GreenPis::WorldObjectLastSearcher<GreenPis::WorldObjectSpellNearbyTargetCheck> searcher(m_caster, target, check, containerTypeMask);
    SearchTargets<GreenPis::WorldObjectLastSearcher<GreenPis::WorldObjectSpellNearbyTargetCheck> > (searcher, containerTypeMask, m_caster, m_caster, range);
    return target;
}
...
void Spell::SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return;
    GreenPis::WorldObjectSpellAreaTargetCheck check(range, position, m_caster, referer, m_spellInfo, selectionType, condList);
    GreenPis::WorldObjectListSearcher<GreenPis::WorldObjectSpellAreaTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
    SearchTargets<GreenPis::WorldObjectListSearcher<GreenPis::WorldObjectSpellAreaTargetCheck> > (searcher, containerTypeMask, m_caster, position, range);
}
...
SpellCastResult Spell::CheckItems()
{
    ...
    if (m_spellInfo->RequiresSpellFocus)
    {
        CellCoord p(GreenPis::ComputeCellCoord(m_caster->GetPositionX(), m_caster->GetPositionY()));
        Cell cell(p);

        GameObject* ok = NULL;
        GreenPis::GameObjectFocusCheck go_check(m_caster, m_spellInfo->RequiresSpellFocus);
        GreenPis::GameObjectSearcher<GreenPis::GameObjectFocusCheck> checker(m_caster, ok, go_check);

        TypeContainerVisitor<GreenPis::GameObjectSearcher<GreenPis::GameObjectFocusCheck>, GridTypeMapContainer > object_checker(checker);
