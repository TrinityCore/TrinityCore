...
void RobotAI::InitializeCharacter()
{
	if (sourcePlayer->newPlayer)
	{
		...
		if (sourcePlayer->GetClass() == Classes::CLASS_HUNTER)
		{
			...
			uint32 guid = sourcePlayer->GetMap()->GenerateLocalLowGuid(HIGHGUID_PET);
			uint32 pet_number = sObjectMgr->GeneratePetNumber();
			CreatureCreatePos pos(sourcePlayer, sourcePlayer->GetOrientation());
			...
			pet->SetOwnerGuid(sourcePlayer->GetObjectGuid());
			...
			pet->SetLoyaltyLevel(LoyaltyLevel::BEST_FRIEND);
			// add to world
			pet->GetMap()->Add((Creature*)pet);

			// caster have pet now
			sourcePlayer->SetPet(pet);

			pet->SavePetToDB(PET_SAVE_AS_CURRENT, sourcePlayer);
		...
		for (std::unordered_map<uint32, PetSpell>::iterator it = petSpellMap.begin(); it != petSpellMap.end(); it++)
		{
			if (it->second.active == ACT_DISABLED)
			{
				...
				if (pST)
				{
					...
					checkPet->ToggleAutocast(it->first, true);
				}
			}
		}
	}
...
void RobotAI::Refresh()
{
	...
	sourcePlayer->SetHealthPercent(100);
	...
	for (std::set<uint32>::iterator it = spellIDSet.begin(); it != spellIDSet.end(); it++)
	{
		if (pmOnlyMyAura)
		{
			if (target->HasAura((*it)))
			{
				...
				{
					if (aIT->second->GetCasterGuid() == sourcePlayer->GetObjectGuid())
					{
						return true;
					}
				}
...
{
	if (sourcePlayer->IsNonMeleeSpellCasted(true))
	{
		return true;
	}
	if (pmCheckAura)
	...
	if (target->GetObjectGuid() != sourcePlayer->GetObjectGuid())
	{
		...
		else
		{
			...
			sourcePlayer->SetSelectionGuid(target->GetObjectGuid());
		}
	}
	...
	if (target->IsImmuneToSpell(pST, false, SpellEffectIndexMask::EFFECT_MASK_ALL))
	{
		return false;
	}
	if (IsChanneledSpell(pST))
	{
		sourcePlayer->StopMoving();
	}
	else
	{
		if (GetSpellCastTime(pST, sourcePlayer) > 0)
		{
...
{
	if (sourcePlayer->hasUnitState(UnitState::UNIT_STAT_CAN_NOT_MOVE))
	{
		return;
	}
	if (sourcePlayer->hasUnitState(UnitState::UNIT_STAT_ROAMING_MOVE))
	{
		return;
	}
	...
	if (!sourcePlayer->hasUnitState(UnitState::UNIT_STAT_RUNNING))
	{
		sourcePlayer->addUnitState(UnitState::UNIT_STAT_RUNNING);
	}
	sourcePlayer->SetSelectionGuid(pmTarget->GetObjectGuid());
	if (pmMelee)
	{
		MoveMelee(pmTarget);
		if (pmAttack)
...
void RobotAI::MoveMelee(Unit * pmTarget)
{
	if (sourcePlayer->GetDistance(pmTarget) > MELEE_MAX_DISTANCE)
	{
		...
		float targetY = 0;
		float targetZ = 0;
		pmTarget->GetClosePoint(targetX, targetY, targetZ, pmTarget->GetObjectBoundingRadius() + sourcePlayer->GetObjectBoundingRadius(), MELEE_COMBAT_DISTANCE);
		sourcePlayer->GetMotionMaster()->MovePoint(0, targetX, targetY, targetZ);
	}
	else
	{
		if (!sourcePlayer->HasInArc(pmTarget, M_PI_F / 4))
		{
...
void RobotAI::MoveCLose(Unit * pmTarget, float pmDistance)
{
	...
	if (currentDistance > pmDistance + MELEE_MAX_DISTANCE)
	{
		...
		float targetY = 0;
		float targetZ = 0;
		pmTarget->GetClosePoint(targetX, targetY, targetZ, pmTarget->GetObjectBoundingRadius() + sourcePlayer->GetObjectBoundingRadius(), pmDistance);
		sourcePlayer->GetMotionMaster()->MovePoint(0, targetX, targetY, targetZ);
	}
...
{
	sourcePlayer->Whisper(pmContent, pmLanguage, pmTarget->GetObjectGuid());
}
...
void RobotAI::HandlePacket(WorldPacket pmPacket)
{
	...
	case SMSG_GROUP_SET_LEADER:
	{
		...
		//if (newLeader)
		//{
		//    if (newLeader->GetObjectGuid() == sourcePlayer->GetObjectGuid())
		//    {
		//        WorldPacket data(CMSG_GROUP_SET_LEADER, 8);
		//        data << masterPlayer->GetObjectGuid().WriteAsPacked();
	...
	case SMSG_RESURRECT_REQUEST:
	{
		...
		sourcePlayer->ResurrectUsingRequestDataInit();
		return;
	}
...
bool RobotAI::EquipItem(std::string pmEquipName)
{
	for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
	{
		if (Item * pItem = sourcePlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
		{
			if (pItem)
			{
				if (pItem->GetProto()->Name1 == pmEquipName)
				{
					uint16 eDest;
					InventoryResult equipResult = sourcePlayer->CanEquipItem(NULL_SLOT, eDest, pItem, false);
					if (equipResult == EQUIP_ERR_OK)
...
void RobotAI::HandleChatCommand(std::string pmCommand, Player * pmSender)
{
	...
	if (commandName == "role")
	{
		...
		if (pmSender->GetObjectGuid() != masterPlayer->GetObjectGuid())
		{
			WhisperTo("You are not my master", Language::LANG_UNIVERSAL, pmSender);
			return;
		}
	...
	else if (commandName == "attack")
	{
		...
		if (st_Group_Normal->Attack(senderTarget))
		{
			...
			st_Group_Normal->staying = false;
			sourcePlayer->SetSelectionGuid(senderTarget->GetObjectGuid());
...
bool RobotAI::SpellValid(uint32 pmSpellID)
{
	...
	if (sourcePlayer->IsSpellReady(pmSpellID))
	{
		return false;
	}
	return true;
