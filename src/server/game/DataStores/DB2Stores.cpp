...
namespace
{
    ...
    std::unordered_map<uint32, std::vector<TransmogSetEntry const*>> _transmogItemSets;
    std::unordered_map<uint32, std::vector<TransmogSetItemEntry const*>> _transmogSetItems;
...
{
    auto i = _transmogSetItems.find(transmogSetID);
    if (i != _transmogSetItems.end())
        return &i->second;
