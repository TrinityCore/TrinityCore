...
void InstanceSaveManager::Unload()
{
    ...
    for (InstanceSaveHashMap::iterator itr = m_instanceSaveById.begin(); itr != m_instanceSaveById.end(); ++itr)
    {
        ...
        {
            ++next;
//            (*itr2)->UnbindInstance(save->GetMapId(), save->GetDifficultyID(), true);//暂时解决不了
        }
...
void InstanceSaveManager::_ResetSave(InstanceSaveHashMap::iterator &itr)
{
    ...
    {
        //if (InstancePlayerBind* bind = player->GetBoundInstance(itr->second->GetMapId(), itr->second->GetDifficultyID()))//暂时解决不了
        //{
...
void InstanceSaveManager::_ResetInstance(uint32 mapid, uint32 instanceId)
{
    ...
    if (iMap)
    {
        ...
        //Map::DeleteRespawnTimesInDB(mapid, instanceId);//暂时解决不了
