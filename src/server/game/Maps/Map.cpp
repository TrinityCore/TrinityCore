...
std::unordered_set<std::tuple<uint32, int, int>> missingTilesToIgnore =
{
// mapid  gx  gy
    {602, 28, 30},
    {602, 29, 30},
...
void Map::LoadMMap(int gx, int gy)
{
    ...
    else if (ShouldLogMissingTile(GetId(), gx, gy))
        TC_LOG_ERROR("mmaps.tiles", "Could not load MMAP name:%s, id:%d, x:%d, y:%d (mmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
    else
        TC_LOG_DEBUG("mmaps.tiles", "Could not load MMAP name:%s, id:%d, x:%d, y:%d (mmap rep.: x:%d, y:%d) because this tile is not generated by mmaps_generator", GetMapName(), GetId(), gx, gy, gx, gy);
}
...
bool Map::ShouldLogMissingTile(uint32 mapid, int gx, int gy)
{
    return missingTilesToIgnore.find({mapid, gx, gy}) == missingTilesToIgnore.cend();
}
