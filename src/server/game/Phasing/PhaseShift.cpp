...
{
    Flags &= EnumFlag<PhaseShiftFlags>(PhaseShiftFlags::AlwaysVisible) | PhaseShiftFlags::Inverse;
    Phases.clear();
...
void PhaseShift::ModifyPhasesReferences(PhaseContainer::iterator itr, int32 references)
{
    ...
    if (!IsDbPhaseShift)
    {
        ...
        if (CosmeticReferences)
            ...
            Flags &= ~EnumFlag<PhaseShiftFlags>(PhaseShiftFlags::NoCosmetic);
...
void PhaseShift::UpdateUnphasedFlag()
{
    ...
    Flags &= ~EnumFlag<PhaseShiftFlags>(!Flags.HasFlag(PhaseShiftFlags::Inverse) ? PhaseShiftFlags::InverseUnphased : PhaseShiftFlags::Unphased);
