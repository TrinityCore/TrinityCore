...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_RAID_WARNING:
        {
            ...
            if (!group || (group->isRaidGroup() && !(group->IsLeader(GetPlayer()->GetGUID()) || group->IsAssistant(GetPlayer()->GetGUID()))) || group->isBGGroup() || !sWorld->getIntConfig(CONFIG_CHAT_PARTY_RAID_WARNINGS))
                return;
