...
void WorldSession::HandlePlayerLogin(LoginQueryHolder const& holder)
{
        ...
        do
        {
            ...
            switch (type)
            {
                ...
                case TRANSMOG_TYPE_ENCHANT:
                    break;
                default:
                    TC_LOG_ERROR("custom.transmog", "Account %u has transmog with unknown type %u in custom_account_transmog, ignoring", GetAccountId(), type);
                    continue;
            }
            pCurrChar->transmogrification_appearances[type].insert(entry);
    ...
    if (auto result = holder.GetPreparedResult(PLAYER_LOGIN_QUERY_LOAD_TRANSMOG_SETS) )
    {
        do
        {
            ...
            pCurrChar->presetMap[PresetID].name = SetName;
            ...
            while (SetData >> slot >> entry >> type)
            {
                ...
                {
                    TC_LOG_ERROR("custom.transmog", "Set has invalid slot %u (Owner: %u, PresetID: %u), ignoring.", slot, pCurrChar->GetGUID().GetCounter(), uint32(PresetID));
                    continue;
                }
                switch (type)
                {
                    case TRANSMOG_TYPE_ITEM:
                        ...
                        {
                            TC_LOG_ERROR("custom.transmog", "Set has invalid item entry %u (Owner: %u, PresetID: %u), ignoring.", entry, pCurrChar->GetGUID().GetCounter(), uint32(PresetID));
                            continue;
                        }
                        break;
                    case TRANSMOG_TYPE_ENCHANT:
                        ...
                        {
                            TC_LOG_ERROR("custom.transmog", "Set has invalid enchant entry %u (Owner: %u, PresetID: %u), ignoring.", entry, pCurrChar->GetGUID().GetCounter(), uint32(PresetID));
                            continue;
                        }
                        break;
                    default:
                        TC_LOG_ERROR("custom.transmog", "Set has invalid transmog type %u (Owner: %u, PresetID: %u), ignoring.", type, pCurrChar->GetGUID().GetCounter(), uint32(PresetID));
                        continue;
                }
                pCurrChar->presetMap[PresetID].data.push_back(std::make_tuple(static_cast<uint8>(slot), static_cast<uint32>(entry), static_cast<AppearanceType>(type)));
            }
        } while (result->NextRow());
    }
