...
void WorldSession::HandleBattlefieldStatusOpcode(WorldPacket & /*recvData*/)
{
    ...
    for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
    {
        ...
        if (ginfo.IsInvitedToBGInstanceGUID)
        {
            ...
            if (!bg)
                continue;
            uint32 remainingTime = getMSTimeDiff(sGameTime->GetGameTimeMS(), ginfo.RemoveInviteTime);
        ...
        else
        {
            ...
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, i, STATUS_WAIT_QUEUE, avgTime, getMSTimeDiff(ginfo.JoinTime, sGameTime->GetGameTimeMS()), arenaType, 0);
            SendPacket(&data);
        }
    }
}
