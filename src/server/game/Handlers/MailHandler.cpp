...
void WorldSession::HandleSendMail(WorldPacket & recv_data)
{
    ...
    {
        SendNotification(GetGreenPisString(LANG_MAIL_SENDER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    {
        SendNotification(GetGreenPisString(LANG_MAIL_RECEIVER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
            ...
            if (!AccountMgr::IsPlayerAccount(GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
            {
                ...
                else
                {
                    ...
                    if (!sObjectMgr->GetPlayerNameByGUID(sender_guid, sender_name))
                        sender_name = sObjectMgr->GetGreenPisStringForDBCLocale(LANG_UNKNOWN);
                }
