...
void WorldSession::HandleItemQuerySingleOpcode(WorldPackets::Query::QueryItem& query)
{
    TC_LOG_INFO("network", "STORAGE: Item Query = %u", query.ItemID);

    WorldPackets::Query::QueryItemResponse response;
    response.ItemID = query.ItemID;

    ItemTemplate const* pProto = sObjectMgr->GetItemTemplate(query.ItemID);
    if (pProto)
    {
        response.Info = *pProto;

        std::string Name = response.Info->Name1;
        std::string Description = response.Info->Description;

        LocaleConstant loc_idx = GetSessionDbLocaleIndex();
        if (loc_idx != DEFAULT_LOCALE)
        {
            if (ItemLocale const* il = sObjectMgr->GetItemLocale(query.ItemID))
            {
                ObjectMgr::GetLocaleString(il->Name, loc_idx, response.Info->Name1);
                ObjectMgr::GetLocaleString(il->Description, loc_idx, response.Info->Description);
            }
        }
    }
