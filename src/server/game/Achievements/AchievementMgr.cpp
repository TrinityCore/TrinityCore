...
void AchievementMgr::SaveToDB(SQLTransaction& trans)
{
    if (!m_completedAchievements.empty())
    {
        for (std::pair<uint32 const, CompletedAchievementData>& m_completedAchievement : m_completedAchievements)
        {
            ...
            stmt->setUInt16(0, m_completedAchievement.first);
            stmt->setUInt32(1, GetPlayer()->GetGUID().GetCounter());
            trans->Append(stmt);

            stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_ACHIEVEMENT);
            stmt->setUInt32(0, GetPlayer()->GetGUID().GetCounter());
            stmt->setUInt16(1, m_completedAchievement.first);
            stmt->setUInt32(2, uint32(m_completedAchievement.second.date));
            trans->Append(stmt);
    ...
    if (!m_criteriaProgress.empty())
    {
        for (std::pair<uint32 const, CriteriaProgress>& m_criteriaProgres : m_criteriaProgress)
        {
            ...
            stmt->setUInt32(0, GetPlayer()->GetGUID().GetCounter());
            stmt->setUInt16(1, m_criteriaProgres.first);
            trans->Append(stmt);

            if (m_criteriaProgres.second.counter)
            {
                stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_ACHIEVEMENT_PROGRESS);
                stmt->setUInt32(0, GetPlayer()->GetGUID().GetCounter());
                stmt->setUInt16(1, m_criteriaProgres.first);
                stmt->setUInt32(2, m_criteriaProgres.second.counter);
                stmt->setUInt32(3, uint32(m_criteriaProgres.second.date));
                trans->Append(stmt);
            }
...
void AchievementMgr::BuildAllDataPacket(WorldPacket* data) const
{
    for (std::pair<uint32 const, CompletedAchievementData> const& m_completedAchievement : m_completedAchievements)
    {
        ...
        *data << uint32(m_completedAchievement.first);
        data->AppendPackedTime(m_completedAchievement.second.date);
    }
    *data << int32(-1);
    ...
    for (std::pair<uint32 const, CriteriaProgress> const& m_criteriaProgres : m_criteriaProgress)
    {
        ...
        *data << GetPlayer()->GetPackGUID();
        *data << uint32(0);
        data->AppendPackedTime(m_criteriaProgres.second.date);
        *data << uint32(0);
        *data << uint32(0);
    }
