...
void SmartAI::StartPath(bool run, uint32 path, bool repeat, Unit* /*invoker*/)
{
    ...
    if (!IsAIControlled())
    {
        TC_LOG_ERROR("misc", "SmartAI::StartPath: Creature entry %u wanted to start waypoint movement despite being controlled, ignoring.", me->GetEntry());
        return;
    }
...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    if (!IsAIControlled())
    {
        TC_LOG_ERROR("misc", "SmartAI::PausePath: Creature entry %u wanted to pause waypoint movement while controlled, ignoring.", me->GetEntry());
        return;
    }
...
{
    if (who && IsAIControlled() && me->Attack(who, me->IsWithinMeleeRange(who)))
    {
        if (mCanCombatMove)
...
void SmartAI::OnCharmed(bool apply)
{
    ...
    if (apply) // do this before we change charmed state, as charmed state will prevent these things from processing
    {
        if (HasEscortState(SMART_ESCORT_ESCORTING))
            EndPath(true);
        me->StopMoving();
