...
enum Opcodes
{
    ...
    CMSG_MESSAGECHAT_BATTLEGROUND_LEADER             = 0x00, // now handled by server
    ...
    CMSG_MESSAGECHAT_PARTY_LEADER                    = 0x00, // now handled by server
    CMSG_MESSAGECHAT_RAID                            = 0x60D1,
    CMSG_MESSAGECHAT_RAID_LEADER                     = 0x00, // now handled by server
    ...
    SMSG_CAMERA_SHAKE                                = 0x5CFB, // int32 shake id (from dbc's?), int32 Sound (SoundKit?)
    ...
    //SMSG_GUILD_CANCEL                              = 0x08C3, // it is what it does, but is it even used?
    ...
    SMSG_RAID_GROUP_ONLY                             = 0xDCDB, // second uint: 1(low level) 2(raid only) 3(group full) 4(group req. unmatch)
    ...
    SMSG_REPORT_PVP_AFK_RESULT                       = 0x0E56, // Unsure, but its related. (PVP_REPORT_AFK_SYSTEM_ENABLED / DISABLED)
    ...
    SMSG_SUMMON_CANCEL                               = 0x8A4E, // no data.
    ...
    SMSG_TRAINER_BUY_RESULT                          = 0x00, // could be 0xDCDE, but it seems to reference to "services" only? int64, int32, int32
