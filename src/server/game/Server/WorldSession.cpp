...
{
    char const* format = GetGreenPisString(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
const char *WorldSession::GetGreenPisString(int32 entry) const
{
    return sObjectMgr->GetGreenPisString(entry, GetSessionDbLocaleIndex());
}
