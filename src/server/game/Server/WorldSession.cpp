...
public:
    ...
    bool Initialize(uint32 accountId, uint32 battlenetAccountId)
    {
        ...
        stmt->setUInt32(0, battlenetAccountId);
        ok = SetPreparedQuery(GLOBAL_ACCOUNT_MOUNTS, stmt) && ok;
...
void WorldSession::InitializeSessionCallback(SQLQueryHolder* realmHolder, SQLQueryHolder* holder)
{
    ...
    _collectionMgr->LoadAccountMounts(holder->GetPreparedResult(AccountInfoQueryHolder::GLOBAL_ACCOUNT_MOUNTS));
