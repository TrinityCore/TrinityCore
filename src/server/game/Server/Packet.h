...
namespace WorldPackets
{
    class Packet
    {
        public:
            ...
            uint16 GetOpcode() const { return _worldPacket.GetOpcode(); }
    ...
    {
        public:
            ServerPacket(uint16 opcode, size_t initialSize = 200) : Packet(WorldPacket(opcode, initialSize)) { }

            void Read() final override { ASSERT(!"Read not implemented for server packets."); }
    ...
    class ClientPacket : public Packet
    {
        public:
            ...
            ClientPacket(uint16 expectedOpcode, WorldPacket&& packet) : Packet(std::move(packet)) { ASSERT(GetOpcode() == expectedOpcode); }

            WorldPacket const* Write() final override
            {
