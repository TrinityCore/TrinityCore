...
{
    const char *format = GetGreenPisString(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char *ChatHandler::GetGreenPisString(int32 entry) const
{
    return m_session->GetGreenPisString(entry);
}
...
{
    SendSysMessage(GetGreenPisString(entry));
}
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
    GreenPis::NearestGameObjectCheck check(*pl);
    GreenPis::GameObjectLastSearcher<GreenPis::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        CellCoord p(GreenPis::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

        GreenPis::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
        GreenPis::GameObjectSearcher<GreenPis::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

        TypeContainerVisitor<GreenPis::GameObjectSearcher<GreenPis::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
...
const char *CliHandler::GetGreenPisString(int32 entry) const
{
    return sObjectMgr->GetGreenPisStringForDBCLocale(entry);
}
...
{
    return GetGreenPisString(LANG_CONSOLE_COMMAND);
}
