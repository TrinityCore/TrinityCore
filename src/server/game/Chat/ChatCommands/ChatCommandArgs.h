...
namespace Trinity::Impl::ChatCommands
{
    ...
    |* - TryConsume: T&, std::string_view -> ChatCommandResult                              *|
    ...
    {
        static ChatCommandResult TryConsume(T& val, std::string_view args)
        {
            ...
            if (Optional<T> v = StringTo<T>(token, 0))
                ...
                return Trinity::StringFormat("Value '" STRING_VIEW_FMT "' is not valid for type %s", STRING_VIEW_FMT_ARG(token), boost::core::demangle(typeid(T).name()).c_str());
    ...
    */
    ...
    {
        static ChatCommandResult TryConsume(char const*& arg, std::string_view args) { arg = args.data(); return std::string_view(); }
    };
    ...
    {
        static ChatCommandResult TryConsume(std::string_view& val, std::string_view args)
        {
    ...
    {
        static ChatCommandResult TryConsume(std::string& val, std::string_view args)
        {
            std::string_view view;
            ChatCommandResult next = ArgInfo<std::string_view>::TryConsume(view, args);
            if (next)
                val.assign(view);
            return next;
        }
    ...
    {
        static ChatCommandResult TryConsume(std::wstring& val, std::string_view args)
        {
            std::string_view utf8view;
            ChatCommandResult next = ArgInfo<std::string_view>::TryConsume(utf8view, args);
    ...
    struct ArgInfo<T, std::enable_if_t<std::is_enum_v<T>>>
    {
        ...
        static ChatCommandResult TryConsume(T& val, std::string_view args)
        {
            ...
            ChatCommandResult next1 = ArgInfo<std::string_view>::TryConsume(strVal, args);
            if (next1)
            {
            ...
            U uVal = 0;
            if (ChatCommandResult next2 = ArgInfo<U>::TryConsume(uVal, args))
            {
                if (EnumUtils::IsValid<T>(uVal))
                {
                    ...
                    return Trinity::StringFormat("Value %s is not valid for type %s", Trinity::ToString(uVal).c_str(), boost::core::demangle(typeid(T).name()).c_str());
            }

            if (next1)
                return Trinity::StringFormat("Value '" STRING_VIEW_FMT "' is not valid for type %s", STRING_VIEW_FMT_ARG(strVal), boost::core::demangle(typeid(T).name()).c_str());
            else
                return std::nullopt;
        }
    };
    ...
    {
        static ChatCommandResult TryConsume(T& tag, std::string_view args)
        {
    ...
    {
        static ChatCommandResult TryConsume(std::vector<T>& val, std::string_view args)
        {
            val.clear();
            ChatCommandResult next = ArgInfo<T>::TryConsume(val.emplace_back(), args);

            if (!next)
                return next;

            while (ChatCommandResult next2 = ArgInfo<T>::TryConsume(val.emplace_back(), *next))
                next = std::move(next2);
    ...
    {
        static ChatCommandResult TryConsume(std::array<T, N>& val, std::string_view args)
        {
    ...
    struct ArgInfo<Trinity::ChatCommands::Variant<Ts...>>
    {
        ...
        template <size_t I>
        static ChatCommandResult TryAtIndex(Trinity::ChatCommands::Variant<Ts...>& val, [[maybe_unused]] std::string_view args)
        {
            if constexpr (I < N)
            {
                ChatCommandResult thisResult = ArgInfo<std::variant_alternative_t<I, V>>::TryConsume(val.template emplace<I>(), args);
                if (thisResult)
                    return thisResult;
                else
                {
                    ChatCommandResult nestedResult = TryAtIndex<I + 1>(val, args);
                    if (!thisResult.HasErrorMessage())
                        return nestedResult;
                    if (!nestedResult.HasErrorMessage())
                        return thisResult;
                    return Trinity::StringFormat("%s\nOr:     %s", thisResult.GetErrorMessage().c_str(), nestedResult.GetErrorMessage().c_str());
                }
            }
            else
                return std::nullopt;
        ...
        static ChatCommandResult TryConsume(Trinity::ChatCommands::Variant<Ts...>& val, std::string_view args)
        {
    ...
    {
        static ChatCommandResult TryConsume(AchievementEntry const*&, std::string_view);
    };
    ...
    {
        static ChatCommandResult TryConsume(GameTele const*&, std::string_view);
    };
    ...
    {
        static ChatCommandResult TryConsume(ItemTemplate const*&, std::string_view);
    };
    ...
    {
        static ChatCommandResult TryConsume(SpellInfo const*&, std::string_view);
    };
