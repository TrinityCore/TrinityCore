...
ChatCommandResult Trinity::Impl::ChatCommands::ArgInfo<AchievementEntry const*>::TryConsume(AchievementEntry const*& data, std::string_view args)
{
    Variant<Hyperlink<achievement>, uint32> val;
    ChatCommandResult result = ArgInfo<decltype(val)>::TryConsume(val, args);
    if (!result || (data = val.visit(AchievementVisitor())))
        return result;
    if (uint32* id = std::get_if<uint32>(&val))
        return Trinity::StringFormat("Achievement with ID %u does not exist", *id);
    return std::nullopt;
}
...
ChatCommandResult Trinity::Impl::ChatCommands::ArgInfo<GameTele const*>::TryConsume(GameTele const*& data, std::string_view args)
{
    ...
    if (val.holds_alternative<Hyperlink<tele>>())
        return Trinity::StringFormat("There is no teleport location with ID %u", static_cast<uint32>(std::get<Hyperlink<tele>>(val)));
    else
        return Trinity::StringFormat("There is no teleport location named '" STRING_VIEW_FMT "'", STRING_VIEW_FMT_ARG(std::get<std::string_view>(val)));
}
...
ChatCommandResult Trinity::Impl::ChatCommands::ArgInfo<ItemTemplate const*>::TryConsume(ItemTemplate const*& data, std::string_view args)
{
    ...
    if (!result || (data = val.visit(ItemTemplateVisitor())))
        return result;
    if (uint32* id = std::get_if<uint32>(&val))
        return Trinity::StringFormat("There is no item with ID %u", *id);
    return std::nullopt;
}
...
ChatCommandResult Trinity::Impl::ChatCommands::ArgInfo<SpellInfo const*>::TryConsume(SpellInfo const*& data, std::string_view args)
{
    ...
    if (!result || (data = val.visit(SpellInfoVisitor())))
        return result;
    if (uint32* id = std::get_if<uint32>(&val))
        return Trinity::StringFormat("There is no spell with ID %u", *id);
    return std::nullopt;
}
