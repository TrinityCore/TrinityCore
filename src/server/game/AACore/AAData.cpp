...
void AAData::AA_REP_Item_Instance(CharacterDatabaseTransaction& trans)
{
    for (auto iter : aaCenter.aa_item_instances) {
        ...
        stmt->setUInt32(0, conf.itemEntry);
        stmt->setUInt64(0, conf.owner_guid);
        stmt->setUInt64(0, conf.creatorGuid);
        stmt->setUInt64(0, conf.giftCreatorGuid);
        stmt->setUInt32(0, conf.count);
        stmt->setUInt32(0, conf.duration);
        stmt->setString(0, conf.charges);
        stmt->setUInt32(0, conf.flags);
        stmt->setString(0, conf.enchantments);
        stmt->setUInt32(0, conf.randomPropertyId);
        stmt->setUInt32(0, conf.durability);
        stmt->setUInt32(0, conf.playedTime);
        stmt->setString(0, conf.text);
        stmt->setString(0, conf.weizhi);
        stmt->setUInt64(0, conf.update_time);
        trans->Append(stmt);
    }
}
