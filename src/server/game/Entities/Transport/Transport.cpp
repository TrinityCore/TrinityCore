...
void Transport::UpdatePlayerPositions()
{
    for (PlayerSet::iterator itr = m_passengers.begin(); itr != m_passengers.end(); ++itr)
    {
        ...
        float x, y, z, o;
        o = GetOrientation() + plr->m_movementInfo.pos.m_orientation;
        x = GetPositionX() + (plr->m_movementInfo.pos.m_positionX * cos(GetOrientation()) + plr->m_movementInfo.pos.m_positionY * sin(GetOrientation() + M_PI));
        y = GetPositionY() + (plr->m_movementInfo.pos.m_positionY * cos(GetOrientation()) + plr->m_movementInfo.pos.m_positionX * sin(GetOrientation()));
        z = GetPositionZ() + plr->m_movementInfo.pos.m_positionZ;
