...
class TC_GAME_API WorldObject : public Object, public WorldLocation
{
    ...
    public:
        ...
        GameObject* SummonGameObject(uint32 entry, Position const& pos, QuaternionData const& rot, GOSummonType summonType = GO_SUMMON_MANUAL_DESPAWN, Seconds const& respawnTime = 0s, uint32 spellId = 0);
        GameObject* SummonGameObject(uint32 entry, float x, float y, float z, float ang, QuaternionData const& rot, GOSummonType summonType = GO_SUMMON_MANUAL_DESPAWN, Seconds const& respawnTime = 0s);
