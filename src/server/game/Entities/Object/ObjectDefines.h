...
static constexpr float const SIGHT_RANGE_UNIT               = 50.0f;
...
static constexpr float const DEFAULT_VISIBILITY_BGARENAS    = 533.0f;                   // default visible distance in BG/Arenas, roughly 533 yards

static constexpr float const DEFAULT_PLAYER_BOUNDING_RADIUS = 0.388999998569489f;       // player size, also currently used (correctly?) for any non Unit world objects
static constexpr float const DEFAULT_PLAYER_COMBAT_REACH    = 1.5f;
static constexpr float const MIN_MELEE_REACH                = 2.0f;
static constexpr float const NOMINAL_MELEE_RANGE            = 5.0f;
static constexpr float const MELEE_RANGE                    = (NOMINAL_MELEE_RANGE - MIN_MELEE_REACH * 2); //center to center for players
