...
{
    return IsInMap(this) && InSamePhase(this, player) && IsAtInteractDistance(player);
}
...
SpellInfo const* GameObject::GetSpellForLock(Player const* player) const
{
    ...
    for (uint8 i = 0; i < MAX_LOCK_CASE; ++i)
    {
        ...
        if (lock->Type[i] == LOCK_KEY_SPELL)
            if (SpellInfo const* spell = sSpellMgr->GetSpellInfo(lock->Index[i], player->GetMap()->GetDifficultyID()))
                return spell;
        ...
        for (auto&& playerSpell : player->GetSpellMap())
            if (SpellInfo const* spell = sSpellMgr->GetSpellInfo(playerSpell.first, player->GetMap()->GetDifficultyID()))
