...
class GameObjectTypeControlZone : public GameObjectTypeBase
{
public:
    explicit GameObjectTypeControlZone(GameObject& owner) : GameObjectTypeBase(owner) { }
    ...
    void ActivateObject(GameObjectActions action, int32 /*param*/, WorldObject* /*spellCaster*/, uint32 /*spellId*/, int32 /*effectIndex*/) override
    {
        switch (action)
        {
            case GameObjectActions::MakeInert:
                if (ControlZone* controlZone = _owner.GetControlZone())
                    controlZone->Disable();
                break;
...
{
    if (GameObjectTypeControlZone* controlZone = dynamic_cast<GameObjectTypeControlZone*>(&type))
    {
...
bool GameObject::Create(uint32 entry, Map* map, Position const& pos, QuaternionData const& rotation, uint32 animProgress, GOState goState, uint32 artKit, bool dynamic, ObjectGuid::LowType spawnid)
{
    ...
    switch (goInfo->type)
    {
        ...
        case GAMEOBJECT_TYPE_CONTROL_ZONE:
            CreateControlZone(GetGOInfo());
            m_goTypeImpl = std::make_unique<GameObjectType::GameObjectTypeControlZone>(*this);
            setActive(true);
            break;
...
{
    if (ControlZone const* controlZone = GetControlZone())
