        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    {
                        GreenPis::AnyUnfriendlyNoTotemUnitInObjectRangeCheck checker(this, owner, radius);
                        GreenPis::UnitSearcher<GreenPis::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, ok, checker);
                    ...
                    else                                        // environmental trap
                    {
                        ...
                        // affect only players
                        Player* player = NULL;
                        GreenPis::AnyPlayerInObjectRangeCheck checker(this, radius);
                        GreenPis::PlayerSearcher<GreenPis::AnyPlayerInObjectRangeCheck> searcher(this, player, checker);
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit* target)
{
    ...
    GameObject* trapGO = NULL;
    {
        ...
        GreenPis::NearestGameObjectEntryInObjectRangeCheck go_check(*target, trapEntry, range);
        GreenPis::GameObjectLastSearcher<GreenPis::NearestGameObjectEntryInObjectRangeCheck> checker(this, trapGO, go_check);

        TypeContainerVisitor<GreenPis::GameObjectLastSearcher<GreenPis::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    CellCoord p(GreenPis::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    GreenPis::NearestGameObjectFishingHole u_check(*this, range);
    GreenPis::GameObjectSearcher<GreenPis::NearestGameObjectFishingHole> checker(this, ok, u_check);

    TypeContainerVisitor<GreenPis::GameObjectSearcher<GreenPis::NearestGameObjectFishingHole>, GridTypeMapContainer > grid_object_checker(checker);
            ...
            if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
                        if (Player* target = ObjectAccessor::GetPlayer(*this, GreenPis::Containers::SelectRandomContainerElement(m_unique_users)))
