...
bool Item::Create(ObjectGuid::LowType guidlow, uint32 itemid, Player const* owner)
{
    ...
    m_scaleLvl = itemProto->GetBaseItemLevel();
    return true;
}
...
float Item::GetScalingDamageValue(ItemTemplate const* proto, uint32 itemLevel)
{
    ...
    ItemDamageEntry const* damageEntry = nullptr;
...
uint32 Item::CalculateScalingStatGTValue(ItemTemplate const* proto, uint32 itemLevel)
{
    ...
    RandomPropertiesPointsEntry const* randProperty = sRandomPropertiesPointsStore.LookupEntry(itemLevel);
...
uint32 Item::CalculateArmorScaling(ItemTemplate const* proto, uint32 itemLevel)
{
    ...
    uint32 quality = proto->GetQuality() == ITEM_QUALITY_HEIRLOOM ? ITEM_QUALITY_RARE : proto->GetQuality();
    uint32 inventoryType = proto->GetInventoryType();
...
void Item::CalculateMinMaxDamageScaling(ItemTemplate const* proto, uint32 itemLevel, uint32& minDamage, uint32& maxDamage)
{
    ...
    else
    {
        ...
        minDamage = floor(((1.f - (proto->GetStatScalingFactor() * 0.5f)) * weaponMinDamageCalc) + 0.5f);
    }
}
