...
{
    // Renamed the parameter from 'player' to 'target' so it makes sense. This
    // function can be used by pets for creature to creature checks.
    ...
    // "The aggro radius of a mob having the same level as the target is roughly 20 yards"
    ...
    // Even though this might be a pet, we'll use config value from the player
...
{
    // Select nearest attackable unit within the given distance (regardless of threat list).
    // Will return either hostile or neutral units.
