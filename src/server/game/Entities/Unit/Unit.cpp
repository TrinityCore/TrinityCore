...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, SpellEffectInfo const* effect, uint32 stack) const
{
    ...
    if (effect->BonusCoefficientFromAP > 0.0f)
    {
        ...
        {
            ApCoeffMod *= 100.0f;
            modOwner->ApplySpellMod<SPELLMOD_BONUS_MULTIPLIER>(spellProto->Id, ApCoeffMod);
            ApCoeffMod /= 100.0f;
        }
    ...
    if (Player* modOwner = GetSpellModOwner())
    {
        ...
        switch (effect_index)
        {
            ...
            case EFFECT_2:
                ...
                break;
            case 3:
                modOwner->ApplySpellMod<SPELLMOD_EFFECT4>(spellProto->Id, value);
                break;
            case 4:
                modOwner->ApplySpellMod<SPELLMOD_EFFECT5>(spellProto->Id, value);
                break;
        }
    }
    return value;
