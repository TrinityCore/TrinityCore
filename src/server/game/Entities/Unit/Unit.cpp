...
void Unit::SetFeared(bool apply)
{
    if (apply)
    {
        ...
        GetMotionMaster()->MoveFleeing(caster, fearAuras.empty() ? sWorld->getIntConfig(CONFIG_CREATURE_FAMILY_FLEE_DELAY) : 0, fearAuras.empty());             // caster == NULL processed in MoveFleeing
    }
    else
    {
        if (IsAlive())
