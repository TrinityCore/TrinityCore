...
Unit::Unit(bool isWorldObject) :
    ...
    m_removedAurasCount(0), m_interruptMask(SpellAuraInterruptFlags::None), m_charmer(nullptr), m_charmed(nullptr),
...
/*static*/ uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    else
    {
        ...
        {
            if (victim != attacker
                && victim->GetTypeId() == TYPEID_PLAYER && (!spellProto || !(spellProto->HasAttribute(SPELL_ATTR7_NO_PUSHBACK_ON_DAMAGE) || spellProto->HasAttribute(SPELL_ATTR3_TREAT_AS_PERIODIC))))
            {
                if (Spell* spell = victim->m_currentSpells[CURRENT_GENERIC_SPELL])
                {
...
{
    return flag == SpellAuraInterruptFlags::Moving;
}
