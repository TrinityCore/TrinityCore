...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    else                                            // not exist in DB, use some default fake data
    {
        ...
        SetCreateHealth(std::max(sDB2Manager.EvaluateExpectedStat(ExpectedStatType::CreatureHealth, petlevel, creatureDifficulty->GetHealthScalingExpansion(), m_unitData->ContentTuningID, Classes(cinfo->unit_class), 0) * creatureDifficulty->HealthModifier * GetHealthMod(cinfo->rank), 1.0f));
