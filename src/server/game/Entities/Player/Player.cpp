    ...
    if (!(currFields & val))
    {
        ...
        if (areaEntry->area_level > 0)
        {
            ...
            else
            {
                ...
                XP = uint32(Trinity::XP::xp_on_weekend_rate(XP));
                GiveXP(XP, nullptr);
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    uint32 XP = rewarded && !quest->IsDFQuest() ? 0 : uint32(quest->XPValue(this) * sWorld->getRate(RATE_XP_QUEST) * Trinity::XP::xp_on_weekend_rate());
