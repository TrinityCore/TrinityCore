...
int32 Player::getMaxTimer(MirrorTimerType timer) const
{
    switch (timer)
    {
        ...
        case BREATH_TIMER:
        {
            ...
            int32 UnderWaterTime = sWorld->getIntConfig(CONFIG_BREATH_TIME) * IN_MILLISECONDS;
...
{
    uint32 base_talent = static_cast<uint32>(getLevel() < 10 ? 0 : getLevel() - 9);
