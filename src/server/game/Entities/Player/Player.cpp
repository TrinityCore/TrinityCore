...
void Player::Update(uint32 p_time)
{
    ...
    // EJ failed when gathering
    if (IsNonMeleeSpellCast(false))
    {
        if (!failChecked)
        {
            if (Spell* cgs = m_currentSpells[CURRENT_GENERIC_SPELL])
            {
                if (const SpellInfo* pS = cgs->GetSpellInfo())
                {
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                    {
                        if (pS->Effects[i].Effect == SpellEffects::SPELL_EFFECT_SKINNING)
                        {
                            if (failCheckPoint == 0)
                            {
                                failCheckPoint = urand(10, 1000);
                                break;
                            }
                            else
                            {
                                if (cgs->GetTimer() < failCheckPoint)
                                {
                                    if (Creature* creature = cgs->m_targets.GetUnitTarget()->ToCreature())
                                    {
                                        uint32 skill = creature->GetCreatureTemplate()->GetRequiredLootSkill();
                                        int32 skillValue = GetSkillValue(skill);
                                        ...
                                        {
                                            cgs->SendCastResult(SPELL_FAILED_TRY_AGAIN);
                        ...
                        else if (pS->Effects[i].Effect == SpellEffects::SPELL_EFFECT_OPEN_LOCK)
                        {
                            if (failCheckPoint == 0)
                            {
                                failCheckPoint = urand(10, 2000);
                                break;
                            }
                            else
                            {
                                ...
                                {
                                    uint32 lockId = 0;
                                    if (GameObject* go = cgs->m_targets.GetGOTarget())
                                    {
                                        lockId = go->GetGOInfo()->GetLockId();
                                    }
                                    else if (Item* itm = cgs->m_targets.GetItemTarget())
                                    {
                                    ...
                                    if (lockId > 0)
                                    {
                                        ...
                                        SpellCastResult res = cgs->CanOpenLock(i, lockId, skillId, reqSkillValue, skillValue);
                                        if (res == SPELL_CAST_OK)
                                        {
                                            if (skillId == SkillType::SKILL_HERBALISM || skillId == SkillType::SKILL_MINING)
                                            {
