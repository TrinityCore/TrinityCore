...
void KillRewarder::_RewardPlayer(Player* player, bool isDungeon)
{
    ...
    {
        uint32 XpRateBg = 1;
        if (_isBattleGround) XpRateBg = 10;
        const float rate = _group ?
            _groupRate * XpRateBg * float(player->getLevel()) / _sumLevel : // Group rate depends on summary level.
...
void Player::SetRestBonus(float rest_bonus_new)
{
    ...
    // hack/fix for the Rested/RaF exp bug
    if (GetSession()->IsARecruiter() || (GetSession()->GetRecruiterId() != 0))
    {
        // adding distance check here if within 100 yards from linked player set RAF_LINKED if not check others
        if(RaFExpCheck(true))
...
bool Player::RaFExpCheck(bool forXP)
{
