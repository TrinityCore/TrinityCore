...
#ifndef GreenPis_CELLIMPL_H
#define GreenPis_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = GreenPis::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = GreenPis::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = GreenPis::ComputeCellCoord(x + radius, y + radius).normalize();
