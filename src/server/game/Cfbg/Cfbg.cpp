...
{
if (GetCFSTeam() ==
        ALLIANCE) { // You were ALLIANCE You're now HORDE; It's for changing race. When you joined battle from other race.
        if (getClass() == CLASS_SHAMAN || getClass() == CLASS_WARRIOR || getClass() == CLASS_DRUID) {
            m_FakeRace = RACE_TAUREN;
            m_FakeMorph = getGender() == GENDER_MALE ? FAKE_M_TAUREN : FAKE_F_TAUREN;
        } else {
            m_FakeRace = RACE_BLOODELF;
            m_FakeMorph = getGender() == GENDER_MALE ? FAKE_M_BLOOD_ELF_MALE : FAKE_M_BLOOD_ELF_FEMALE;
        }
    } else { // You were HORDE; You're now ALLIANCE;
        if (getClass() == CLASS_DRUID) {
            m_FakeMorph = FAKE_M_NELF;
            m_FakeRace = RACE_NIGHTELF;
        } else if (getClass() == CLASS_SHAMAN || getClass() == CLASS_HUNTER) {
            m_FakeMorph = FAKE_F_DRAENEI; // Female Draenei
            m_FakeRace = RACE_DRAENEI;
        } else {
            m_FakeRace = RACE_HUMAN;
            m_FakeMorph = getGender() == GENDER_MALE ? FAKE_M_HUMAN_MALE : FAKE_M_HUMAN_FEMALE;
        }
    }
}
