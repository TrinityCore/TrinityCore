...
struct npc_jaina_150101 : public ScriptedAI
{
    ...
    void QuestAccept(Player* player, Quest const* quest) 
    {
        ...
        {
            //player->GetSceneMgr().PlaySceneByPackageId(SCENE_SCOUTING_THE_TIDESTONE);//tmp
    ...
    void MovementInform(uint32 type, uint32 pointID) override
    {
        ...
        if (pointID && !this->portal_prepared)
        {
            ...
            if (this->portal_prepared)
            {
                ...
                {
                    me->SummonGameObject(287164, 1292.530f, -106.669f, -330.105f, 2.41f, QuaternionData(), Seconds(0));
                    Talk(1);
                    me->SetWalk(false);
...
struct npc_chief_telemancer_oculeth : public ScriptedAI
{
    ...
    void QuestAccept(Player* player, Quest const* quest) 
    {
        if (quest->GetQuestId() == QUEST_DESCENT_HORDE)
        {
            ...
            player->CompleteQuest(QUEST_DESCENT_HORDE);
        }
    }
};
...
struct npc_generic_soldier_fortunate_souls : public ScriptedAI
{
    ...
    void Reset() override
    {
        me->AddUnitFlag((UnitFlags)UNIT_NPC_FLAG_SPELLCLICK);//AddFlag
    }
    ...
    //                me->DespawnOrUnsummon(Milliseconds(0), 15s);
    //            }
    //            break;
...
struct boss_wekemara : public BossAI
{
    ...
    {        
        ...
        {
            me->SummonCreature(NPC_WEKEMARA_HANDLER, handler_spawn_at_selector_pos, TEMPSUMMON_TIMED_DESPAWN, Milliseconds(29000));
            events.Repeat(30s);
            break;
        }
        ...
        {
            UnitList tarlist;
           // SelectTargetList(tarlist, 3, SelectTargetMethod::Random, 100.0f, true);
        ...
        //    me->DespawnCreaturesInArea(NPC_WEKEMARA_HANDLER, 125.0f);
            me->SummonCreature(NPC_WEKEMARA_HANDLER, electrified_splash_pos, TEMPSUMMON_TIMED_DESPAWN, Milliseconds(30000));
            break;
...
public:
    ...
    //{
    //    go->AddToNotify(GO_FLAG_IN_USE);
    //    go->GetScheduler().Schedule(100ms, [go, player] (TaskContext /*context*/)
    //    {            
    //        player->ModifyCurrency(1560, 26);
    //        player->ModifyCurrency(1721, 5);
    //        player->ModifyCurrency(1587, 16);
    //    });
...
struct npc_urduu : public ScriptedAI
{
    ...
    void JustDied(Unit* /*unit*/) override
    {
        ...
        for (auto& players : p_list)
        {
            ...
            {
                players->ModifyCurrency(1553, 76);
                players->ModifyCurrency(1721, 3);
            }
        }
    };
private:
    ...
    void MoveInLineOfSight(Unit* unit) override
    {
        if (unit->IsPlayer() && me->GetDistance2d(unit) < 15.0f)
        {
            if (Player* player = unit->ToPlayer())
            {
                ...
                {
                    player->CompleteQuest(QUEST_UP_AGAINST_IT_HORDE);
                ...
                {
                    player->CompleteQuest(QUEST_UP_AGAINST_IT_ALLIANCE);
                    //player->GetSceneMgr().PlaySceneByPackageId(SCENE_SEEING_THE_ARMY);
                }
                me->DespawnOrUnsummon(Milliseconds(0), 15s);
            }
...
struct npc_lorthemar_theron_151848 : public ScriptedAI
{
    ...
    void QuestAccept(Player * plr, Quest const* /*quest*/) 
    {        
...
struct npc_spiritwalker_ussoh : public ScriptedAI
{
    ...
    bool GossipSelect(Player* /*plr*/, uint32 /*sender*/, uint32 /*action*/) 
    {
        return true;
    }
};
    ...
    void MoveInLineOfSight(Unit* unit) override
    {
        if (unit->IsPlayer() && me->GetDistance2d(unit) < 15.0f)
        {
            if (Player* player = unit->ToPlayer())
            {
                ...
                {
                    player->CompleteQuest(QUEST_WHERE_THE_ROAD_LEADS_ALLIANCE);
                ...
                {
                    player->CompleteQuest(QUEST_STAY_FAST_STAY_LOW_HORDE);
...
struct npc_shandris_feathermoon : public ScriptedAI
{
    ...
    void QuestAccept(Player * plr, Quest const* /*quest*/) 
    {
        ...
        if (plr->GetQuestStatus(QUEST_SECRETS_IN_THE_RUINS) == QUEST_STATUS_INCOMPLETE) 
            plr->CompleteQuest(QUEST_SECRETS_IN_THE_RUINS);
    }
    ...
    void MoveInLineOfSight(Unit* unit) override
    {
        if (unit->IsPlayer() && me->GetDistance2d(unit) < 15.0f)
            if (Player* player = unit->ToPlayer())
                ...
                {
                    player->CompleteQuest(QUEST_WHAT_REMAINS_OF_ZIN_ASHARI_ALLIANCE);
                    Talk(0);
        ...
        if (unit->IsPlayer() && me->GetDistance2d(unit) < 15.0f)
            if (Player* player = unit->ToPlayer())
                if (player->GetQuestStatus(QUEST_HELP_THEM_SEE_THE_TRUTH) == QUEST_STATUS_INCOMPLETE)
                    player->CompleteQuest(QUEST_HELP_THEM_SEE_THE_TRUTH);
    }
};
