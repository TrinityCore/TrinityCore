...
struct boss_dunegorger_kraulok : public BossAI
{
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        events.ScheduleEvent(EVENT_SONIC_BELLOW, Milliseconds(9000));
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SHAKE_LOSE:
            {
                ...
                {
                    events.ScheduleEvent(EVENT_SHAKE_LOSE, Milliseconds(15000));
                }
                else
                {
                    events.ScheduleEvent(EVENT_SHAKE_LOSE, Milliseconds(28000));
                }
                break;
            }
            case EVENT_PRIMAL_RAGE:
            {
                ...
                events.ScheduleEvent(EVENT_PRIMAL_RAGE, Milliseconds(33000));
                break;
            }
            case EVENT_SONIC_BELLOW:
            {
                ...
                if (me->HasAura(SPELL_PRIMAL_RAGE))
                    events.ScheduleEvent(EVENT_SONIC_BELLOW, Milliseconds(urand(15000, 20000)));
                else
                {
                    events.ScheduleEvent(EVENT_SONIC_BELLOW, Milliseconds(urand(25000, 30000)));
                }
                break;
            }
            default:
