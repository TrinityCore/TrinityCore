    ...
    void sGossipSelect(Player* player, uint32 /*menuId*/, uint32 /*gossipListId*/)
    {
        if (!player->GetQuestObjectiveData(QuestObjective(QUEST_ITS_ALIVE)))
        {
            ...
            player->GetScheduler().Schedule(5s, [this, player](TaskContext context)
                {
                    me->SummonCreature(NPC_RIDE_ROBOT_VEHICLE, player->GetPosition(), TEMPSUMMON_TIMED_DESPAWN, Milliseconds(90000));
                });
        }
...
{
    npc_riding_raptor_147318(Creature* c) : ScriptedAI(c) {
        /*Vehicle* vehicle = me->GetVehicleKit();*/ //NO USE
    }
