...
public:
    ...
    static bool HandleNpcInfoCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        if (curRespawnDelay < 0)
            curRespawnDelay = 0;
        std::string curRespawnDelayStr = secsToTimeString(uint64(curRespawnDelay), TF_SHORT_TEXT);
        std::string defRespawnDelayStr = secsToTimeString(target->GetRespawnDelay(), TF_SHORT_TEXT);
