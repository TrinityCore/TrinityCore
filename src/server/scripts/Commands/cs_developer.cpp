...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "developer",      SEC_ADMINISTRATOR,  false,  &HandleDeveloperCommand,    "", NULL },
    ...
    static bool HandleDeveloperCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            if (handler->GetSession()->GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DEVELOPER))
                handler->GetSession()->SendNotification(LANG_DEVELOPER_ON);
            else
                handler->GetSession()->SendNotification(LANG_DEVELOPER_OFF);
            return true;
        }
