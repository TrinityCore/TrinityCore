...
public:
    ...
    {
        if (!MMAP::MMapManager::createOrGetMMapManager()->GetNavMesh(handler->GetSession()->GetPlayer()->GetMapId()))
        {
    ...
    static bool HandleMmapLocCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        dtNavMesh const* navmesh = MMAP::MMapManager::createOrGetMMapManager()->GetNavMesh(handler->GetSession()->GetPlayer()->GetMapId());
        dtNavMeshQuery const* navmeshquery = MMAP::MMapManager::createOrGetMMapManager()->GetNavMeshQuery(handler->GetSession()->GetPlayer()->GetMapId(), player->GetInstanceId());
    ...
    static bool HandleMmapLoadedTilesCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        dtNavMesh const* navmesh = MMAP::MMapManager::createOrGetMMapManager()->GetNavMesh(mapid);
        dtNavMeshQuery const* navmeshquery = MMAP::MMapManager::createOrGetMMapManager()->GetNavMeshQuery(mapid, handler->GetSession()->GetPlayer()->GetInstanceId());
        if (!navmesh || !navmeshquery)
        {
            handler->PSendSysMessage("NavMesh not loaded for current map.");
            return true;
    ...
    static bool HandleMmapStatsCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        MMAP::MMapManager* manager = MMAP::MMapManager::createOrGetMMapManager();
