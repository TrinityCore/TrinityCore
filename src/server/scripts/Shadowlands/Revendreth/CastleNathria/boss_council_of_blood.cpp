...
private:
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case EVENT_DANSE_MACABRE:
            me->AddAura(SPELL_DANSE_MACABRE_MOD_DAMAGE_TAKEN, me);
            if (Creature* niklaus = GetCastellanNiklaus())
                niklaus->AddAura(SPELL_DANSE_MACABRE_MOD_DAMAGE_TAKEN, niklaus);
            if (Creature* stavros = GetLordStavros())
                stavros->AddAura(SPELL_DANSE_MACABRE_MOD_DAMAGE_TAKEN, stavros);
