                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SHADOW_WORD_PAIN:
                            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true, true, -SPELL_SHADOWWORDPAIN))
                                DoCast(target, SPELL_SHADOWWORDPAIN);
                            events.ScheduleEvent(EVENT_SHADOW_WORD_PAIN, 18s, 26s);
                            break;
                        case EVENT_IMMOLATE:
                            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true, true, -SPELL_IMMOLATE))
