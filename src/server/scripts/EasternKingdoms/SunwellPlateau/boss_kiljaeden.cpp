...
public:
    ...
    struct boss_kiljaedenAI : public ScriptedAI
    {
        ...
        void CastSinisterReflection()
        {
            ...
            for (uint8 i = 0; i < 4; ++i)
            {
                if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 100, true, true, -SPELL_VENGEANCE_OF_THE_BLUE_FLIGHT))
                {
                    float x, y, z;
                    ...
                    switch (t)
                    {
                        ...
                        case TIMER_LEGION_LIGHTNING:
                            if (!me->IsNonMeleeSpellCast(false))
                            {
                                ...
                                for (uint8 z = 0; z < 6; ++z)
                                {
                                    pRandomPlayer = SelectTarget(SelectAggroTarget::Random, 0, 100, true);
                        ...
                        case TIMER_ARMAGEDDON: //Phase 4
                            ...
                            for (uint8 z = 0; z < 6; ++z)
                            {
                                target = SelectTarget(SelectAggroTarget::Random, 0, 100, true);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (uiSpawnFiendTimer <= diff)
            {
                if (Creature* pFiend = DoSpawnCreature(NPC_VOLATILE_FELFIRE_FIEND, 0, 0, 0, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 20000))
                    AddThreat(SelectTarget(SelectAggroTarget::Random, 0), 100000.0f, pFiend);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (victimClass)
            {
                ...
                case CLASS_WARLOCK:
                    ...
                    if (uiTimer[2] <= diff)
                    {
                        DoCast(SelectTarget(SelectAggroTarget::Random, 0, 100, true), SPELL_SR_CURSE_OF_AGONY, true);
