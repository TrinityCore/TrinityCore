...
public:
    ...
    struct npc_gurgthock_cataAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            me->SetNpcFlag(UNIT_NPC_FLAG_QUESTGIVER);//AddNpcFlag
            uiTimer = 0;
            uiPhase = 0;
            uiQuest = 0;
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            if (uiPhase)
            {
                ...
                if (uiTimer <= uiDiff)
                {
                    ...
                    {
                        case 1:
                            if (Creature* summon = me->SummonCreature(NPC_HURP_DERP, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, Milliseconds(5000)))
                        ...
                        case 4:
                            ...
                            {
                                if (Creature* summon = me->SummonCreature(NPC_TORG_DRAKEFLAYER, SpawnPosition[1], TEMPSUMMON_CORPSE_DESPAWN, Milliseconds(5000)))
                                    SummonGUID = summon->GetGUID();
                            }
                            else
                            if (player->GetTeamId() == TEAM_HORDE)
                            {
                                if (Creature* summon = me->SummonCreature(NPC_SULLY_KNEECAPPER, SpawnPosition[1], TEMPSUMMON_CORPSE_DESPAWN, Milliseconds(5000)))
                                    SummonGUID = summon->GetGUID();
                            }
                            uiTimer = 3000;
                            uiPhase = 0;
                            break;
                        case 6:
                            if (Creature* summon = me->SummonCreature(NPC_CADAVER_COLLAGE, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, Milliseconds(5000)))
                                SummonGUID = summon->GetGUID();
                            uiPhase = 7;
                            uiTimer = 4000;
                            break;
                         case 7:
                            ...
                            break;
                        case 8:
                            if (Creature* summon = me->SummonCreature(NPC_LORD_GEOFFERY_TULVAN, SpawnPosition[2], TEMPSUMMON_CORPSE_DESPAWN, Milliseconds(5000)))
                                SummonGUID = summon->GetGUID();
                            uiPhase = 9;
                            uiTimer = 4000;
                            break;
                         case 9:
                            ...
                            break;
                        case 10:
                            if (Creature* summon = me->SummonCreature(NPC_EMBERSCAR_THE_DEVOURER, SpawnPosition[1], TEMPSUMMON_CORPSE_DESPAWN, Milliseconds(5000)))
                                SummonGUID = summon->GetGUID();
                            uiPhase = 11;
                            uiTimer = 4000;
                            break;
...
public:
    ...
    struct npc_hurp_derpAI : public ScriptedAI
    {
        ...
        {
            events.ScheduleEvent(EVENT_INTIMIDATING_ROAR, Milliseconds(10000));
            events.ScheduleEvent(EVENT_OVERHEAD_SMASH, Milliseconds(5000));
            events.ScheduleEvent(EVENT_WHIRLWIND, Milliseconds(15000));
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_INTIMIDATING_ROAR:
                    ...
                    events.ScheduleEvent(EVENT_INTIMIDATING_ROAR, Milliseconds(urand(15 * IN_MILLISECONDS, 25 * IN_MILLISECONDS)));
                    break;
                case EVENT_OVERHEAD_SMASH:
                    DoCastVictim(SPELL_OVERHEAD_SMASH);
                    events.ScheduleEvent(EVENT_OVERHEAD_SMASH, Milliseconds(urand(20 * IN_MILLISECONDS, 30 * IN_MILLISECONDS)));
                    break;
                case EVENT_WHIRLWIND:
                     if (Unit* target = SelectTarget(SelectTargetMethod::MaxThreat, 0))//SELECT_TARGET_TOPAGGRO
                     {
                         DoCast(target, SPELL_WHIRLWIND, true);
                     }
                     events.ScheduleEvent(EVENT_WHIRLWIND, Milliseconds(urand(15 * IN_MILLISECONDS, 25 * IN_MILLISECONDS)));
                    break;
                }
            }
...
public:
    ...
    struct npc_sully_kneecapperAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) 
        {
            events.ScheduleEvent(EVENT_S_CHARGE, Milliseconds(10000));
            events.ScheduleEvent(EVENT_UPPERCUT, Milliseconds(5000));

            me->SummonCreature(NPC_GLOOMWING, SpawnPosition[3], TEMPSUMMON_CORPSE_DESPAWN, Milliseconds(5000));
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                {
                case EVENT_S_CHARGE:
                    DoCast(SPELL_CHARGE);//DoCastRandom   //, 150.0f
                    events.ScheduleEvent(EVENT_S_CHARGE, Milliseconds(urand(10 * IN_MILLISECONDS, 20 * IN_MILLISECONDS)));
                    break;
                case EVENT_UPPERCUT:
                    DoCastVictim(SPELL_UPPERCUT);
                    events.ScheduleEvent(EVENT_UPPERCUT, Milliseconds(urand(15 * IN_MILLISECONDS, 25 * IN_MILLISECONDS)));
                    break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_cadaver_collageAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) 
        {
            events.ScheduleEvent(EVENT_BELCH, Milliseconds(15000));
            events.ScheduleEvent(EVENT_POISON_CLOUD, Milliseconds(10000));
            events.ScheduleEvent(EVENT_REPULSIVE_KICK, Milliseconds(8000));
        }
        ...
        void DamageTaken(Unit* attacker, uint32& /*damage*/) override
        {
            if(!inhaled && !me->IsNonMeleeSpellCast(false) && HealthBelowPct(20))
            {
                ...
                events.ScheduleEvent(EVENT_BELOW5, Milliseconds(6000));
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_BELCH:
                    ...
                    events.ScheduleEvent(EVENT_BELCH, Milliseconds(urand(15 * IN_MILLISECONDS, 20 * IN_MILLISECONDS)));
                    break;
                case EVENT_POISON_CLOUD:
                    DoCast(SPELL_POISON_CLOUD);
                    events.ScheduleEvent(EVENT_POISON_CLOUD, Milliseconds(urand(10*IN_MILLISECONDS, 15*IN_MILLISECONDS)));
                    break;
                case EVENT_REPULSIVE_KICK:
                    DoCastVictim(SPELL_REPULSIVE_KICK);
                    events.ScheduleEvent(EVENT_REPULSIVE_KICK, Milliseconds(urand(10*IN_MILLISECONDS, 15*IN_MILLISECONDS)));
                    break;
                case EVENT_BELOW5:
                    me->SetUnitFlag2(UNIT_FLAG2_FEIGN_DEATH);//AddUnitFlag2
                    me->AddAura(SPELL_CADAVER_HOVER, me);
                    events.ScheduleEvent(EVENT_TAKEOFF, Milliseconds(5200));
                    break;
                case EVENT_TAKEOFF:
                    me->SetCanFly(true);
//                    me->SetAnimTier(UnitBytes1_Flags(UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER), true);
                    ...
                    events.ScheduleEvent(EVENT_EXPLODE, Milliseconds(5600));
                    break;
                case EVENT_EXPLODE:
...
public:
    ...
    struct npc_lord_geofferyAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) 
        {
            events.ScheduleEvent(EVENT_DEATH_BY_PEASANT, Milliseconds(15000));
            events.ScheduleEvent(EVENT_GEOF_UCUT, Milliseconds(5000));
        }
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& /*damage*/) override
        {
            if(!transformed && !me->IsNonMeleeSpellCast(false) && HealthBelowPct(20))
            {
               ...
               events.ScheduleEvent(EVENT_UNDYING_FRENZY, Milliseconds(2000));
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_DEATH_BY_PEASANT:
                    ...
                    events.ScheduleEvent(EVENT_DEATH_BY_PEASANT, Milliseconds(urand(30 * IN_MILLISECONDS, 40 * IN_MILLISECONDS)));
                    break;
                case EVENT_UNDYING_FRENZY: // execute this event at 20% (needs retail info)
                    DoCastVictim(SPELL_UNDYING_FRENZY);
                    events.ScheduleEvent(EVENT_UNDYING_FRENZY, Milliseconds(urand(10 * IN_MILLISECONDS, 15 * IN_MILLISECONDS)));
                    break;
                case EVENT_GEOF_UCUT:
                    DoCastVictim(SPELL_UPPERCUT);
                    events.ScheduleEvent(EVENT_GEOF_UCUT, Milliseconds(urand(15 * IN_MILLISECONDS, 20 * IN_MILLISECONDS)));
                    break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_emberscar_devourerAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) 
        {
            events.ScheduleEvent(EVENT_FIREBALL, Milliseconds(1000));
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_FIREBALL:
                    ...
                    events.ScheduleEvent(EVENT_FIREBALL, Milliseconds(4000));
                    break;
                }
            }
        }
...
public:
    ...
    struct npc_gloomwingAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) 
        {
            events.ScheduleEvent(EVENT_GLOOM_BALL, Milliseconds(2000));
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_GLOOM_BALL:
                    ...
                    events.ScheduleEvent(EVENT_GLOOM_BALL, Milliseconds(urand(3 * IN_MILLISECONDS, 6 * IN_MILLISECONDS)));
                    break;
                }
            }
        }
