    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            case EVENT_DIPEL_MAGIC:
                if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, HighInquisitorFairbanksDispelMagicTargetSelector(me)))
                    DoCast(target, SPELL_DISPEL_MAGIC);
                events.Repeat(30s);
                break;
            case EVENT_FEAR:
                if (Unit* target = SelectTarget(SelectAggroTarget::Random, 1, 20.f, true))
                    DoCast(target, SPELL_FEAR);
                events.Repeat(40s);
                break;
            case EVENT_SLEEP:
                if (Unit* target = SelectTarget(SelectAggroTarget::MaxThreat, 0, 30.f, true, false))
