            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            Unit* target = SelectTarget(SelectAggroTarget::MaxThreat, urand(1, 3), 0.0f, false, true, -SPELL_MARK_OF_ARLOKK);
                            if (!target)
                                target = me->GetVictim();
                            if (target)
                            {
        ...
        struct npc_zulian_prowlerAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    ...
                    {
                        case EVENT_ATTACK:
                            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0.0f, 100, false))
