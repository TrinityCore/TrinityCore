                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CAUSE_INSANITY:
                            // DoCast(SelectTarget(SelectAggroTarget::Random, 0, 100, true), SPELL_CAUSE_INSANITY);
                        ...
                        case EVENT_WILL_OF_HAKKAR:
                        {
                            ...
                            SelectTargetList(unitList, 2, SelectAggroTarget::MaxThreat, 0, 0.0f, false);
                            if (unitList.size() > 1)
                                DoCast(SelectTarget(SelectAggroTarget::MaxThreat, 0, 100, true), SPELL_WILL_OF_HAKKAR);
