...
{
    npc_zulaman_voljin(Creature* creature) : ScriptedAI(creature), _instance(creature->GetInstanceScript())
    {
        Initialize();
    }
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_BANGING_THE_GONG:
                    ...
                    if (GameObject* strangeGong = _instance->GetGameObject(DATA_STRANGE_GONG))
                ...
                case EVENT_START_DOOR_OPENING_4:
                    ...
                    if (GameObject* masiveGate = _instance->GetGameObject(DATA_MASSIVE_GATE))
                ...
                case EVENT_START_DOOR_OPENING_7:
                    if (Creature* hexLordTrigger = _instance->GetCreature(DATA_HEXLORD_MALACRASS_TRIGGER))
