...
enum AkilzonMisc
{
    ...
    AREA_ZULAMAN                = 3805,
    SOUND_EAGLE                 = 12196,
    POINT_KIDNAPPER_PLAYER      = 52638,
    PATH_KIDNAPPER_CYCLIC       = 526380
};
...
struct npc_akilzon_amani_kidnapper : public ScriptedAI
{
    using ScriptedAI::ScriptedAI;
    ...
    void IsSummonedBy(WorldObject* summoner) override
    {
        if (InstanceScript* instance = me->GetInstanceScript())
            if (Creature* akilzon = instance->GetCreature(BOSS_AKILZON))
                akilzon->AI()->JustSummoned(me);

        me->SetReactState(REACT_PASSIVE);
        me->SetCorpseDelay(5, true);
        me->GetMotionMaster()->MovePoint(POINT_KIDNAPPER_PLAYER, summoner->GetPosition(), false);
    }

    void MovementInform(uint32 type, uint32 pointId) override
    {
        if (type != POINT_MOTION_TYPE || pointId != POINT_KIDNAPPER_PLAYER)
            return;
    ...
    {
        if (apply)
            me->GetMotionMaster()->MovePath(PATH_KIDNAPPER_CYCLIC, true);
    }
};
...
class spell_akilzon_electrical_overload : public SpellScript
{
    ...
    void HandleEffect(SpellEffIndex effIndex)
    {
        ...
        SpellInfo const* damageSpellSpellInfo = sSpellMgr->AssertSpellInfo(GetEffectInfo().TriggerSpell, GetCastDifficulty());
        ...
        if (SpellInfo const* triggeringSpellSpellInfo = GetTriggeringSpell())
            if (AuraEffect const* eff = GetCaster()->GetAuraEffect(triggeringSpellSpellInfo->Id, EFFECT_1))
                GetCaster()->CastSpell(nullptr, uint32(GetEffectInfo().TriggerSpell), CastSpellExtraArgs()
                    ...
                    .AddSpellMod(SPELLVALUE_BASE_POINT0, eff->GetTickNumber() * damageSpellSpellInfo->GetEffect(EFFECT_0).CalcValue()));
    }
...
void AddSC_boss_akilzon()
{
    ...
    RegisterZulAmanCreatureAI(npc_akilzon_amani_kidnapper);
