...
Creature* m_creature = nullptr;//I WROTE THIS
...
struct mob_mawsAI : public ScriptedAI
{
    ...
    {
        const SpellInfo* SPELL_RAMPAGE = nullptr;//wait to be added
        const SpellInfo* SPELL_FRENZY = nullptr;//wait to be added
        const SpellInfo* SPELL_DARK_WATER = nullptr;//wait to be added

        
        if (InCombat)
        {
            if (!m_creature->SelectVictim() || !m_creature->GetVictim() || LeaveCombatTimer < uiDiff) // m_creature->GetThreatManager().isThreatListEmpty() ?
            {//SelectHostileTarget
                InCombat = 0;
                Reset();
                return;
            ...
            {
                if (!PhaseTwo && m_creature->GetHealthPct() < 20.0f)//GetHealthPercent
                {
                    PhaseTwo = true;
                ...
                {
                    DoCastSpell(m_creature->GetVictim(), SPELL_RAMPAGE);//DoCastSpellIfCan
                    if (!PhaseTwo)
                ...
                {
                    DoCastSpell(m_creature, SPELL_FRENZY); //DoCastSpellIfCan
                    FrenzyTimer = FrenzyTimerMax;
                }
                else
                    FrenzyTimer -= uiDiff;
                if (PhaseTwo)
                {
                    ...
                    {
                        DoCastSpell(m_creature, SPELL_DARK_WATER);
    ...
    void Reset() override
    {
        ...
        m_creature->GetThreatManager();//DeleteThreatList   //WRONG
        m_creature->CombatStop(true);
        m_creature->hasLootRecipient();//SetLootRecipient //nullptr   //WRONG    
        LeaveCombatTimer = 30000;
