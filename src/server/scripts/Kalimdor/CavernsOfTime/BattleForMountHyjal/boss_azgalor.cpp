...
public:
    ...
    struct boss_azgalorAI : public hyjal_trashAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (RainTimer <= diff)
            {
                DoCast(SelectTarget(SelectAggroTarget::Random, 0, 30, true), SPELL_RAIN_OF_FIRE);
                RainTimer = 20000 + rand32() % 15000;
            } else RainTimer -= diff;

            if (DoomTimer <= diff)
            {
                DoCast(SelectTarget(SelectAggroTarget::Random, 1, 100, true), SPELL_DOOM);//never on tank
...
public:
    ...
    struct npc_lesser_doomguardAI : public hyjal_trashAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            {
                DoCast(SelectTarget(SelectAggroTarget::Random, 0, 100, true), SPELL_CRIPPLE);
