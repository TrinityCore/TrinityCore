        ...
        void ExecuteEvent(uint32 eventId) override
        {
            switch (eventId)
            {
                ...
                case EVENT_FINGER_OF_DEATH:
                    if (!SelectTarget(SelectAggroTarget::Random, 0, 5.0f)) // Checks if there are no targets in melee range
                    {
                        DoCast(SelectTarget(SelectAggroTarget::Random, 0), SPELL_FINGER_OF_DEATH);
                ...
                case EVENT_DOOMFIRE:
                    Talk(SAY_DOOMFIRE);
                    if (Unit* temp = SelectTarget(SelectAggroTarget::Random, 1))
                        SummonDoomfire(temp);
                    else
