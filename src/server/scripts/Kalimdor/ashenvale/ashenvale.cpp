...
int npc_ruul_snowhoof_Boss_ID = 1; //Later need to change,and ID to be find
int npc_feero_ironhand_Boss_ID = 2;
...
struct npc_ruul_snowhoofAI : public BossAI
{
    Creature* m_creature;
    npc_ruul_snowhoofAI(Creature* pCreature) : BossAI(pCreature, npc_ruul_snowhoof_Boss_ID)
    {
        Reset();
    };
    ...
    void Reset()  override
    {
        m_creature->AddAura(BEAR_AURA, m_creature);
    }

    //void WaypointReached(uint32 uiPointId)
    //{
    //    m_creature->Yell("DEBUG : WayPoint reached", Language(0)); //The hell, still skipping waypoints 13/02/2015
    //    //Log::instance().outString("DEBUG : Ruul : WayPoint n° %u", uiPointId);
    //    switch (uiPointId)
    //    {
    //    case 13:
    //        m_creature->SummonCreature(NPC_T_TOTEMIC, 3449.218018f, -587.825073f, 174.978867f, 4.714445f, TEMPSUMMON_DEAD_DESPAWN, Milliseconds(60000));
    //        m_creature->SummonCreature(NPC_T_URSA, 3446.384521f, -587.830872f, 175.186279f, 4.714445f, TEMPSUMMON_DEAD_DESPAWN, Milliseconds(60000));
    //        m_creature->SummonCreature(NPC_T_PATHFINDER, 3444.218994f, -587.835327f, 175.380600f, 4.714445f, TEMPSUMMON_DEAD_DESPAWN, Milliseconds(60000));
    //        break;
    //    case 19:
    //        m_creature->SummonCreature(NPC_T_TOTEMIC, 3508.344482f, -492.024261f, 186.929031f, 4.145029f, TEMPSUMMON_DEAD_DESPAWN, Milliseconds(60000));
    //        m_creature->SummonCreature(NPC_T_URSA, 3506.265625f, -490.531006f, 186.740128f, 4.239277f, TEMPSUMMON_DEAD_DESPAWN, Milliseconds(60000));
    //        m_creature->SummonCreature(NPC_T_PATHFINDER, 3503.682373f, -489.393799f, 186.629684f, 4.349232f, TEMPSUMMON_DEAD_DESPAWN, Milliseconds(60000));
    //        break;
    //    case 25:
...
bool QuestAccept_npc_ruul_snowhoof(Player* /*pPlayer*/, Creature* pCreature, Quest const* pQuest)
{
    ...
    {
        pCreature->SetFaction(FACTION_ESCORTEE);//SetFactionTemplateId
...
struct npc_torekAI : public ScriptedAI
{
    Creature* m_creature;
    npc_torekAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
    ...
    //void WaypointReached(uint32 uiPointId)
    //{
    //    //Player* pPlayer = GetPlayerForEscort();//org
    ...
    //        m_creature->SummonCreature(NPC_DURIEL, 1776.73f, -2049.06f, 109.83f, 1.54f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, Milliseconds(25000));
    //        m_creature->SummonCreature(NPC_SILVERWING_SENTINEL, 1774.64f, -2049.41f, 109.83f, 1.40f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, Milliseconds(25000));
    //        m_creature->SummonCreature(NPC_SILVERWING_WARRIOR, 1778.73f, -2049.50f, 109.83f, 1.67f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, Milliseconds(25000));
    //        break;
    //    case 20:
    ...
    {
        if (!m_creature->SelectVictim() || !m_creature->GetVictim())//SelectHostileTarget
            return;
        SpellInfo* SPELL_REND = nullptr;
        SpellInfo* SPELL_THUNDERCLAP = nullptr;
        if (m_uiRend_Timer < uiDiff)
        {
            DoCastSpell(m_creature->GetVictim(), SPELL_REND);//DoCastSpellIfCan
        ...
        {
            DoCastSpell(m_creature, SPELL_THUNDERCLAP);//DoCastSpellIfCan
...
{
    //int TEMPFACTION_RESTORE_RESPAWN = 0;//NO USE
    if (pQuest->GetQuestId() == QUEST_TOREK_ASSULT)
    {
        ...
        pCreature->SetFaction(FACTION_ORGRIMMAR);//,TEMPFACTION_RESTORE_RESPAWN
        std::list<Creature*> lCrea;
        pCreature->GetCreatureListWithEntryInGrid(lCrea, NPC_SPLINTERTREE_RAIDER, 40.0f);
        for (const auto& it : lCrea)
            it->SetFaction(FACTION_ORGRIMMAR);//, TEMPFACTION_RESTORE_RESPAWN
...
struct npc_feero_ironhandAI : public BossAI
{
    Creature* m_creature;
    npc_feero_ironhandAI(Creature* pCreature) : BossAI(pCreature, npc_feero_ironhand_Boss_ID)
    {
        Reset();
    }
    ...
    void Reset() override
    {
        if (!IsEscorted())//HasEscortState  //STATE_ESCORT_ESCORTING
        {
    ...
    void JustRespawned()
    {
        m_creature->AddUnitFlag(UnitFlags(UNIT_FIELD_FLAGS));//SetFlag    //, UNIT_FLAG_IMMUNE_TO_NPC
    ...
    //        m_creature->SummonCreature(NPC_BALIZAR_THE_UMBRAGE, aEliteSummonPositions[0][0], aEliteSummonPositions[0][1], aEliteSummonPositions[0][2], aEliteSummonPositions[0][3], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,  Milliseconds(20000));
    //        m_creature->SummonCreature(NPC_ALIGAR_THE_TORMENTOR, aEliteSummonPositions[1][0], aEliteSummonPositions[1][1], aEliteSummonPositions[1][2], aEliteSummonPositions[1][3], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,  Milliseconds(20000));
    //        m_creature->SummonCreature(NPC_CAEDAKAR_THE_VICIOUS, aEliteSummonPositions[2][0], aEliteSummonPositions[2][1], aEliteSummonPositions[2][2], aEliteSummonPositions[2][3], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,  Milliseconds(20000));
    //        break;
    //    case 30:
    //        // Complete the quest
    //       /* if (Player* pPlayer = GetPlayerForEscort())
    ...
    {
        float fX, fY, fZ;
        m_creature->GetNearPoint(m_creature, fX, fY, fZ, fDistance, fAngle);

        m_creature->SummonCreature(uiEntry, fX, fY, fZ, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, Milliseconds(20000));
    }
...
{
    int FACTION_ESCORT_A_NEUTRAL_PASSIVE = 0;
    if (pQuest->GetQuestId() == QUEST_SUPPLIES_TO_AUBERDINE)
    {
        ...
        pCreature->RemoveUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC);//RemoveFlag    //UNIT_FIELD_FLAGS,
...
struct go_foulweald_totem_moundAI : public GameObjectAI
{
    ...
    bool EventStart(uint64 /*playerGuid*/)
    {
        ...
        eventPhase = 1;
        TempSummon* foulweald = nullptr;
        for (int i = 0; i < 2; i++)
        {
            if (foulweald = me->SummonCreature(NPC_ENRAGED_FOULWEALD, foulwealdSpawnCoords[i][0], foulwealdSpawnCoords[i][1], foulwealdSpawnCoords[i][2], 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, Milliseconds(420000)))
            {
                //ObjectGuid guidCurrentEnragedFoulweald[i] = foulweald->GetGUID();
                float x, y, z;
    ...
    void EnragedFoulwealdJustDied(uint64 creatureGUID)
    {
        ...
        for (uint64& guid : guidCurrentEnragedFoulweald)
        {
            ...
            {
                pos = urand(0, 3);
                if (foulweald = me->SummonCreature(NPC_ENRAGED_FOULWEALD, foulwealdSpawnCoords[pos][0], foulwealdSpawnCoords[pos][1], foulwealdSpawnCoords[pos][2], 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, Milliseconds(420000)))
                {
    ...
    void UpdateAI(uint32 const uiDiff) override
    {
        ...
        if (phaseTimer < uiDiff)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                if (Creature* murgut = me->SummonCreature(NPC_CHIEF_MURGUT, foulwealdSpawnCoords[3][0], foulwealdSpawnCoords[3][1], foulwealdSpawnCoords[3][2], 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,Milliseconds( 120000)))
                {
                    float x, y, z;
                    float ang = 0;
                    me->GetPosition(x, y, z);
                    ...
                    if (GameObject* pGo = me->FindNearestGameObject(GO_KARANG_S_BANNER, 10))
                    {
                        ...
                        me->SummonGameObject(178207, x, y, z, ang, rot, Seconds(0),GO_SUMMON_TIMED_OR_CORPSE_DESPAWN);//, 120
                    }
                }
                phaseTimer = 120000;
                break;
...
struct npc_enraged_foulwealdAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        m_creature->AddAura(SPELL_CORRUPTED_STRENGTH,me);
        timer = 0;
    }
    ...
    void JustDied(Unit* /*pKiller*/) override
    {
        ...
        if (GameObject* gobj = m_creature->GetMap()->GetGameObject(guidMound))
        {
            if (go_foulweald_totem_moundAI* pMoundAI = dynamic_cast<go_foulweald_totem_moundAI*>(gobj->AI()))
                ...
                m_creature->GetGUID();//I WROTE THIS FOR TEMP USE
        }
    }
    ...
    bool HitBanner()
    {
        if (GameObject* pGo = m_creature->FindNearestGameObject(GO_KARANG_S_BANNER, CONTACT_DISTANCE + 1))
        {
            float x, y, z;
            pGo->GetPosition(x, y, z);
            m_creature->CastSpell(m_creature,SPELL_DESTROY_KARANG_S_BANNER_1, pGo);
    ...
    void UpdateAI(uint32 const uiDiff) override
    {
        ...
        if (!m_creature->SelectNearestHostileUnitInAggroRange() || !m_creature->GetVictim())//SelectHostileTarget
        {
            if (timer < uiDiff)
            {
                if (!HitBanner())
...
bool ProcessEventId_event_king_of_the_foulweald(uint32 /*eventId*/, Object* source, Object* target, bool /*isStart*/)
{
    ...
    else    //I WROTE TO AVOID COMPILE ERROS
        return true;
}
