...
struct boss_smashspite_the_hateful : public BossAI
{
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            {
                Position sumPos;
                //GetRandPosFromCenterInDist(&felBatSumCenter, 14.f, sumPos);//tmp
                sumPos.m_positionZ += frand(-10.f, 10.f);
                me->SummonCreature(NPC_SMASHSPITE_FELBAT, sumPos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 2000,0,ObjectGuid::Empty);
                events.Repeat(15s);
                break;
            }
        }
...
struct npc_brh_felbat : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* /*summoner*/) override
    {
        me->GetScheduler().Schedule(5s, [this](TaskContext context)
        {
            ...
            if (Unit* attackableUnit = SelectTarget(SelectTargetMethod::Random))
            {
...
void AddSC_boss_smashspite_the_hateful()
{
    ...
    //RegisterAuraScript(aura_smashspite_brutality);//tmp
