...
struct instance_black_rook_hold : public InstanceScript
{
    ...
    void Initialize() 
    {
        ...
        //SetChallengeDoorPos({ 3468.448242f, 7631.900391f, -9.821134f, 4.351523f });//tmp
    }
    ...
    void OnCreatureCreate(Creature* creature) override
    {
        ...
        if (instance->IsHeroic())
            //creature->SetBaseHealth(creature->GetMaxHealth() * 2.f);//org,below the same
            creature->SetHealth(creature->GetMaxHealth() * 2.f);
        if (instance->IsMythic())
            creature->SetHealth(creature->GetMaxHealth() * 1.33f);
            ...
            if (m_illysannaPreEventLowerMobs.size() == 0)
            {
                ...
                for (ObjectGuid guid : m_illysannaPreEventUpperMobs)
                {
                    if (Creature* creature = ObjectAccessor::GetCreature(*unit, guid))
                    {
                        ...
                        //GetPositionWithDistInFront(creature, 15.f, jumpPos);//tmp
                        ...
                        {
                            if (Player* player = GetContextCreature()->SelectNearestPlayer(100))//org,null,no 100
                            {
    ...
    void Update(uint32 diff) override
    {
        ...
        switch (events.ExecuteEvent())
        {
            case DATA_STAIRS_BOULDER_1:
            {
                ...
                if (TempSummon* boulder = instance->SummonCreature(NPC_BOULDER, firstBoulderPositions[boulderSide][0]))
...
{
    //RegisterInstanceScript(instance_black_rook_hold, 1501);//tmp
}
