...
struct boss_amalgam_of_souls : public BossAI
{
    ...
    //void DamageTaken(Unit* /*attacker*/, uint32& damage) override //org
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    {
        if (IsHeroic())
        {
            //if (me->HealthWillBeBelowPctDamaged(50, damage))  //org
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            {
                //if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1)) //org
            ...
            case SPELL_CALL_SOULS:
            {
                ...
                {
                    Position pos;
                    //GetRandPosFromCenterInDist(me, 30.f, pos);//tmp
    ...
    CreatureAI* GetSummonerAI()
    {
        if (TempSummon* meTempSummon = me->ToTempSummon())
            if (WorldObject* summoner = meTempSummon->GetSummoner())
                //if (summoner->IsCreature() && summoner->IsAIEnabled())    //org
