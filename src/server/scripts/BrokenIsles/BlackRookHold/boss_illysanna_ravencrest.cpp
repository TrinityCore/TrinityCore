...
struct boss_illysanna_ravencrest : public BossAI
{
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            {
                Position summonPos;
                //GetRandPosFromCenterInDist(&centerPos, 14.f, summonPos);//tmp
                me->SummonCreature(NPC_SOUL_TORN_VANGUARD, summonPos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 2000, 0, ObjectGuid::Empty);
                events.Repeat(30s);
                break;
            }
            case NPC_RISEN_ARCANIST:
            {
                Position summonPos;
                //GetRandPosFromCenterInDist(&centerPos, 14.f, summonPos);//tmp
                me->SummonCreature(NPC_RISEN_ARCANIST, summonPos, TEMPSUMMON_CORPSE_TIMED_DESPAWN,2000,0,ObjectGuid::Empty);
                events.Repeat(45s);
                break;
            }
            default:
