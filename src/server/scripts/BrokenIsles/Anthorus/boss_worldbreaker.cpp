...
struct boss_garothi_worldbreaker : public BossAI
{
    ...
    void RespawnCreatures()
    {
        ...
        if (creatures.empty())
        {
            if (Creature* npc = me->SummonCreature(NPC_ANNIHILATOR, pos1, TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(WEEK), 5430, false))
        ...
        if (creatures.empty())
        {
            if (TempSummon* npc = me->SummonCreature(NPC_DECIMATOR, pos2, TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(WEEK), 5430, false))
            {
