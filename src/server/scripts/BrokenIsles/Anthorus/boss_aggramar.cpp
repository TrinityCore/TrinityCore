...
struct boss_aggramar : public BossAI
{
    ...
    void LoadNPC(uint32 event, const CustomSpawnData* data)
    {
        ...
        {
            if (data->event == event)
                me->SummonCreature(data->npcId, Position(data->X, data->Y, data->Z, data->orientation), TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(WEEK));
            ++data;
        }
    }
    ...
    {
        if (me->HealthBelowPctDamaged(80, damage))//HealthWillBeBelowPctDamaged
        {
            PhaseStatus = PHASE_2;
            events.Reset();
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            case SPELL_FLARE:
            {
                ...
                if (Unit* target2 = SelectTarget(SelectTargetMethod::Random, 0.0, 0.0, true))
                    me->CastSpell(target2, SPELL_FLARE, false);

                if (Unit* target3 = SelectTarget(SelectTargetMethod::Random, 0.0, 0.0, true))
