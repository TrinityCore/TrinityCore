...
struct boss_admiral_svirax : public BossAI
{
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case NPC_CHIEF_ENGINEER_ISHKAR:
        {
            ...
            me->SummonCreature(NPC_CHIEF_ENGINEER_ISHKAR, phase2pos, TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(Milliseconds(WEEK)));
            break;
        }
        case NPC_GENERAL_ERODUS:
        {
            ...
            events.ScheduleEvent(SPELL_ENTROPIC_MINE, 5s);
            me->SummonCreature(NPC_GENERAL_ERODUS, phase2pos, TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(Milliseconds(WEEK)));
            break;
        }
        ...
        case SPELL_SUMMON_REINFORCEMENTS:
        {
            ...
            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 1, 0.f, true))//SELECT_TARGET_RANDOM
                me->SummonCreature(NPC_FANATICAL_PYROMANCER, target->GetRandomNearPosition(15.0f), TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(WEEK));
            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 1, 0.f, true))
                me->SummonCreature(NPC_CHIEF_ENGINEER_ISHKAR, target->GetRandomNearPosition(15.0f), TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(WEEK));
            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 1, 0.f, true))
                me->SummonCreature(NPC_FELBLADE_SHOCKTROOPER, target->GetRandomNearPosition(15.0f), TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(WEEK));
            events.Repeat(8s, 9s);
            break;
        }
        default:
