...
struct boss_argus_the_unmaker : public BossAI
{
    ...
    //    {
    //        if (data->event == event)
    //            if (TempSummon* personalCreature = me->SummonCreature(data->npcId, Position(data->X, data->Y, data->Z, data->orientation), TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(WEEK)))
    //            {
    //                if (data->spellId > 0)
    ...
    void SwitchPhase(uint32 /*phase*/)
    {
        ...
        //        me->SetHealth(_health);
        //        if (Creature* life = me->FindNearestCreature(NPC_EONAR, 150.0f, true))
        //            life->SummonCreature(NPC_GIFT_OF_THE_LIFEBINDER, Position(2893.73f, -4545.36f, 292.028f, 3.77829f), TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(WEEK));
        //        //LoadNPC(EVENT_5, spawnData);
        //        //instance->DoResurrectPlayers(1.0f);//org
        //        instance->UseCombatResurrection();//DoResurrectPlayers
        //    });
    ...
    //{
    //    if (me->HealthBelowPctDamaged(HP_70, damage))//HealthWillBeBelowPctDamaged
    //        if (PhaseStatus == PHASE_01)
    //            SwitchPhase(PHASE_02);

    //    if (me->HealthBelowPctDamaged(HP_40, damage))
    //        if (PhaseStatus == PHASE_02)
    //            SwitchPhase(PHASE_03);

    //    if (me->HealthBelowPctDamaged(HP_30, damage))
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        {
            if (Unit* target1 = SelectTarget(SelectTargetMethod::Random, 0.0, 0.0, true))
    ...
    //uint8 PhaseStatus;//org
  
    enum  PhaseStatus : uint8
    {
        PHASE_01 ,
        PHASE_02 ,
        PHASE_03 ,
