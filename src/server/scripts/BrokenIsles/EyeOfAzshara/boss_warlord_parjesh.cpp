		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_THROW_SPEAR:
				{
					...
					SelectTargetList(targets, 5, SELECT_TARGET_RANDOM, 500.0f, true, false); // 5 players, random targets, alive > non-tank spec selector Nontankselector selects the current target
