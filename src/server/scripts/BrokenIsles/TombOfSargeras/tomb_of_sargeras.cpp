...
struct npc_tos_breach : ScriptedAI
{
    ...
    void DoAction(int32 const actionID) override
    {
        ...
        case 1:
            for (uint8 i = 0; i < 3; ++i)
            {
                AddDelayedEvent(100 + 600 * i, [this, i]() -> void
                {
...
struct npc_tos_kadghar_1 : ScriptedAI
{
    ...
    void DoAction(int32 const actionID) override
    {
        ...
        AddDelayedEvent(8500, [this]() -> void
        {
        ...
        AddDelayedEvent(25000, [this]() -> void
        {
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        if (!IntroDone && instance->GetBossState(DATA_GOROTH) == DONE)
        {
            ...
            AddDelayedEvent(1000, [this] {me->SetFacingTo(6.25f); });
            ...
            for (uint8 i = 0; i < 4; ++i)
                if (auto add = me->GetMap()->GetCreature(instance->GetGuidData(introEntries[i])))
                {
                    ...
                    AddDelayedEvent(1000, [add] {add->SetFacingTo(6.25f); });
                }
        ...
        if (me->GetDistance(who) < 100.0f && !IntroDone)
        {
            ...
            AddDelayedEvent(7000, [this]() -> void
            {
                if (auto boss = me->GetMap()->GetCreature(instance->GetGuidData(DATA_GOROTH)))
                {
                    ...
                    boss->AI()->Talk(10);
                    AddDelayedEvent(4000, [boss]() -> void
                    {
                        boss->AI()->Talk(0);
...
public:
    ...
    class spell_vantus_rune_tomb_of_sargeras_AuraScript : public AuraScript
    {
        ...
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_vantus_rune_tomb_of_sargeras_AuraScript::OnUpdate, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
        }
    };
