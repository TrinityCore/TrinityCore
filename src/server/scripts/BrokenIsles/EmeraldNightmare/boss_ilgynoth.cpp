...
struct boss_ilgynoth : public BossAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SUMMON_7:
                SummomRandPos(1, NPC_DEATHGLARE_TENTACLE);
                events.RescheduleEvent(EVENT_SUMMON_6, 30000); //Next: Sem informação. Vamos repetir?
                break;
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (rupturingRoarTimer)
        {
            if (rupturingRoarTimer <= diff)
            {
                ...
                if (auto target = SelectTarget(SELECT_TARGET_MINDISTANCE, 0, 100.0f, true, false))
                {
