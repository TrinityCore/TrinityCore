...
public:
    ...
    struct boss_flotsamAI : public WorldBossAI
    {
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_JETSAM, Milliseconds(10000));
            me->AI()->Talk(88247.f,nullptr); //On aggro text
            me->SetPower(POWER_MANA, 0);
            getsamCount = 0;
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (events.Empty())
            {
                events.ScheduleEvent(EVENT_JETSAM, Milliseconds(1000));
            }
            ...
            if (me->IsInCombat())
            {
                ...
                if (manaTimer >= 1000)
                {
                    if (me->GetPower(POWER_MANA) < 100)
                        ...
                        events.ScheduleEvent(EVENT_GETSAM, Milliseconds(200));
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_JETSAM:
                {
                    if (me->IsInCombat())
                    {
                        ...
                        getsamCount++;
                        events.ScheduleEvent(EVENT_BREAKSAM, Milliseconds(5000));
                    }
                    break;
                }
                case EVENT_BREAKSAM:
                {
                    ...
                    if (getsamCount >= 4)
                    {
                        events.ScheduleEvent(EVENT_YAKSAM, Milliseconds(5000));
                        getsamCount = 0;
                    } else
                        events.ScheduleEvent(EVENT_JETSAM, Milliseconds(10000));
...
public:
    ...
    class spell_flotsam_jetsam_target_SpellScript : public SpellScript
    {
        ...
        void HandleHitTarget(SpellEffIndex /*effIndex*/)
        {
            ...
            if (TempSummon* tempSumm = caster->SummonCreature(NPC_JETSAM, hit->GetPositionX(), hit->GetPositionY(), hit->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 120000,ObjectGuid::Empty))
            {
                //tempSumm->SetSummonerGUID(caster->GetGUID());//org
                tempSumm->SetCreatorGUID(caster->GetGUID());//I wrote,may crash
