        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_PHASE_FIRE:
                    ...
                    events.ScheduleEvent(EVENT_CHANGE_PHASE, Milliseconds(20000));
                    events.ScheduleEvent(EVENT_BURNING_BOMB, Milliseconds(200));
                    events.ScheduleEvent(EVENT_WRATHFUL_FLAMES, Milliseconds(urand(7000, 8000)));
                    lastPhase = PHASE_FIRE;
                    break;
                case EVENT_PHASE_FROST:
                    me->CastSpell(me, SPELL_AR_FROST, true);
                    events.ScheduleEvent(EVENT_HOWLING_GALE, Milliseconds(200));
                    events.ScheduleEvent(EVENT_ICY_COMET, Milliseconds(8000));
                    events.ScheduleEvent(EVENT_CHANGE_PHASE, Milliseconds(20000));
                ...
                case EVENT_PHASE_ARCANE:
                    ...
                    events.ScheduleEvent(EVENT_CHANGE_PHASE, Milliseconds(20000));
                    events.ScheduleEvent(EVENT_ARCANE_DESOLATION, Milliseconds(200));
                ...
                case EVENT_BURNING_BOMB:
                {
                    if (me->IsInCombat())
                        if (me->HasAura(SPELL_AR_FIRE))
                        {
                            ...
                            events.ScheduleEvent(EVENT_BURNING_BOMB, Milliseconds(16000));
                        }
                    break;
                }
                ...
                case EVENT_ARCANE_DESOLATION:
                {
                    if (me->IsInCombat())
                        ...
                        {
                            events.ScheduleEvent(EVENT_ARCANE_DESOLATION, Milliseconds(12000));
                            me->CastSpell(me, SPELL_A_AD_TARGET, false);
                            events.ScheduleEvent(EVENT_ARCANOPULSE, Milliseconds(1000));
                        }
                    break;
                }
            ...
            case PHASE_NEUTRAL:
                if (lastPhase == 0)
                {
                    events.ScheduleEvent(EVENT_PHASE_FIRE, Milliseconds(5000));
                    events.SetPhase(PHASE_FIRE);
                    lastPhase = PHASE_FIRE;
                }
                else if (lastPhase == PHASE_FIRE)
                {
                    events.ScheduleEvent(EVENT_PHASE_FROST, Milliseconds(5000));
                    events.SetPhase(PHASE_FROST);
                    lastPhase = PHASE_FROST;
                }
                else if (lastPhase == PHASE_FROST)
                {
                    events.ScheduleEvent(EVENT_PHASE_ARCANE, Milliseconds(5000));
