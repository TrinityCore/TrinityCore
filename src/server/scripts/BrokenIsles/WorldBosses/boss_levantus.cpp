...
public:
    ...
    struct boss_levantusAI : ScriptedAI
    {
        ...
        void EnterCombat(Unit* /*who*/) 
        {
            ...
            _events.ScheduleEvent(4, Milliseconds(35000));
            _events.ScheduleEvent(5, Milliseconds(15000));
            _events.ScheduleEvent(6, Milliseconds(25000));
        }
            ...
            if (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case 1:
                {
                    for (uint32 i = 1; i < 7; i++) // +1
                    {
                        ...
                        //Below two are wrote by me,may crash
                        Waypointspawn[i].GetPositionX() + urand(0, 25), Waypointspawn[i].GetPositionY() + urand(0, 25), Waypointspawn[i].GetPositionZ();//What?This can also do?
                        //C++ is so free,awsome!
                        me->CastSpell(CastSpellTargetArg(Waypointspawn[i]), 217212);
                    }
                ...
                case 2:
                    ...
                    _events.ScheduleEvent(2, Milliseconds(5000));
                    break;
                case 3:
                    //if (Unit* target = SelectTarget(SELECT_TARGET_FARTHEST))  //org
                    if (Unit* target = SelectTarget(SelectTargetMethod::MaxDistance))
                        me->CastSpell(target, 217229, true);
                    break;
                case 4:
                    //if (Unit* target = SelectTarget(SELECT_TARGET_NEAREST))   //org
                    if (Unit* target = SelectTarget(SelectTargetMethod::MinDistance))
                    {
                        if (target->GetDistance(me) < 30)
                            ...
                            _events.ScheduleEvent(3, Milliseconds(1000));
                    }
                    _events.ScheduleEvent(4, Milliseconds(60000));
                    break;
                case 5:
                {
                    DoCast(217344);
                ...
                {
                    DoCast(217235);
                    _events.ScheduleEvent(6, Milliseconds(30000));
                    break;
                }
                default:
                    break;
