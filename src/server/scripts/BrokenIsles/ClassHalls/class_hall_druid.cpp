...
struct npc_nightmare_blight_103246 : public ScriptedAI
{
    ...
    void JustDied(Unit* killer) override
    {
        if (killer->IsPlayer())
        {
            ...
            killer->ToPlayer()->SummonCreature(NPC_KEEPER_REMULOS_103488, Position(1720.639f, 1347.214f, 2.08268f, 4.830286f), TEMPSUMMON_TIMED_DESPAWN, 60000);
...
struct npc_thisalee_crow_106299 : public ScriptedAI
{
    ...
    void QuestAccept(Player* player, Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_DEFENSE_OF_AVIANA)
            player->ForceCompleteQuest(QUEST_DEFENSE_OF_AVIANA);

        if (quest->GetQuestId() == QUEST_YOU_CANT_TAKE_THE_SKY_FROM_ME)
        {
            player->ForceCompleteQuest(QUEST_DEFENSE_OF_AVIANA);
