...
struct npc_apata_highmountain_104382 : public ScriptedAI
 {
    npc_apata_highmountain_104382(Creature * creature) : ScriptedAI(creature) { }
        ...
        if (quest->GetQuestId() == QUEST_THE_EAGLE_SPIRIT_S_BLESSING)
             {
            if (Creature* bird = player->SummonCreature(100967, Position(-861.094f, 4276.1f, 746.284f, 0.365586f), TEMPSUMMON_MANUAL_DESPAWN, 300000, 0, player->GetGUID()))
                 bird->AddNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
            player->SummonCreature(100990, Position(-861.094f, 4276.1f, 746.284f, 0.365586f), TEMPSUMMON_MANUAL_DESPAWN, 300000, 0, player->GetGUID());
            }
         }
...
struct npc_apata_highmountain_100990 : public ScriptedAI
 {
    npc_apata_highmountain_100990(Creature * creature) : ScriptedAI(creature) {  }
        ...
        void Reset() override
         {
        me->Say(114943);
        if (Player* player = me->SelectNearestPlayer(90.0f))
             {
