        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_TRANSITION_1:
                    me->SetReactState(REACT_PASSIVE);
                    me->SetArmor(VOIDWALKER_ARMOR, 0);
...
class spell_astromancer_wrath_of_the_astromancer : public AuraScript
{
    ...
    {
        return ValidateSpellEffect({ { spellInfo->Id, EFFECT_1 } }) && ValidateSpellInfo({ uint32(spellInfo->GetEffect(EFFECT_1).CalcValueAsInt()) });
    }
    ...
    void AfterRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        GetTarget()->CastSpell(GetTarget(), uint32(GetEffectInfo(EFFECT_1).CalcValueAsInt()), true);
    }
