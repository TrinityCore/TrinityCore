    ...
    void DamageTaken(Unit* /*who*/, uint32& damage, DamageEffectType /*damageType*/, SpellInfo const* /*spellInfo = nullptr*/) override
    {
        ...
        if (damage >= me->GetHealth())
        {
            ...
            if (!IsFeigningDeath)
            {
                ...
                Scheduler.Schedule(0s, [this](TaskContext const& /*task*/)
                {
                    Scheduler.CancelAll();
            ...
            case ACTION_ADVISOR_DEFEATED:
            {
                ...
                switch (_defeatedAdvisorCounter)
                {
                    case ADVISOR_THALADRED:
                        ...
                        _scheduler.Schedule(12500ms, [this](TaskContext const& /*task*/)
                        {
                    ...
                    case ADVISOR_SANGUINAR:
                        ...
                        _scheduler.Schedule(7s, [this](TaskContext const& /*task*/)
                        {
                    ...
                    case ADVISOR_CAPERNIAN:
                        ...
                        _scheduler.Schedule(8s, [this](TaskContext const& /*task*/)
                        {
...
class spell_kaelthas_remove_weapons : public SpellScript
{
    ...
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        ...
        for (uint32 spell : RemoveWeaponsSpells)
            target->CastSpell(target, spell, TRIGGERED_FULL_MASK & ~TRIGGERED_IGNORE_REAGENT_COST);
    }
...
class spell_kaelthas_gaining_power : public AuraScript
{
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_NETHERBEAM_1, SPELL_NETHERBEAM_2, SPELL_NETHERBEAM_3 }) &&
            sSoundKitStore.LookupEntry(SOUND_PRECAST_NATURE_LOW) &&
            sSoundKitStore.LookupEntry(SOUND_PRECAST_FROST_HIGH);
    }
...
class spell_kaelthas_explodes : public SpellScript
{
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_KAEL_GAINING_POWER, SPELL_NETHERBEAM_3 }) &&
            sSoundKitStore.LookupEntry(SOUND_EXORCISM) &&
            sSoundKitStore.LookupEntry(SOUND_FIRE_WARD_TARGET) &&
            sSoundKitStore.LookupEntry(SOUND_ARCANE_EXPLOSION);
    }
...
class spell_kaelthas_pure_nether_beam : public SpellScript
{
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_PURE_NETHER_BEAM_1, SPELL_PURE_NETHER_BEAM_2, SPELL_PURE_NETHER_BEAM_3 }) &&
            sSoundKitStore.LookupEntry(SOUND_GHOSTLY_STRIKE_IMPACT) &&
            sSoundKitStore.LookupEntry(SOUND_HOLY_BOLT);
    }
...
class spell_kaelthas_pure_nether_beam_visual : public SpellScript
{
    ...
    {
        GetHitUnit()->SendPlaySpellVisualKit(SPELL_VISUAL_KIT_1, 0, 0);
        GetHitUnit()->SendPlaySpellVisualKit(SPELL_VISUAL_KIT_2, 0, 0);
    }
...
class spell_kaelthas_nether_vapor_lightning : public AuraScript
{
    ...
    void OnPeriodic(AuraEffect const* aurEff)
    {
        GetTarget()->CastSpell(GetTarget()->GetNearPosition(10.0f, frand(0.0f, 2.0f * float(M_PI))), aurEff->GetAmountAsInt());
    }
