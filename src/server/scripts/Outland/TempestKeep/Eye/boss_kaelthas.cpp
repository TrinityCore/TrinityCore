                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_GRAVITY_LAPSE:
                            ...
                            events.CancelEventGroup(EVENT_GROUP_COMBAT);
                            events.ScheduleEvent(EVENT_NETHER_BEAM, 5000);
                            events.ScheduleEvent(EVENT_SHOCK_BARRIER, 1000);
                            events.ScheduleEvent(EVENT_GRAVITY_LAPSE, 20000);
                        case EVENT_NETHER_BEAM:
                            if (Unit* unit = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                DoCast(unit, SPELL_NETHER_BEAM);

                            events.ScheduleEvent(EVENT_NETHER_BEAM, 4000);
                            break;
