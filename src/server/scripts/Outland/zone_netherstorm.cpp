...
/*######
## Quest 10439: Dimensius the All-Devouring
######*/
...
{
    npc_saeed_follower(Creature* creature) : ScriptedAI(creature), leaderGuid(), index(0), total(0), followLeaderCalled(false) { }

    ObjectGuid leaderGuid;
    uint32 index;
    uint32 total;
    bool followLeaderCalled;
    static ObjectGuid talkingFollowerGuid;

    void Reset() override
    {
        followLeaderCalled = false;
        me->setActive(true);
    ...
    void FollowLeader(Unit* who = nullptr)
    {
        if (Creature* leader = ObjectAccessor::GetCreature(*me, leaderGuid))
        {
            if (!who)
            {
                ...
                float angle = 2.0f * M_PI * float(index) / float(total);
    ...
    void UpdateAI(uint32 /*diff*/) override
    {
        ...
        Creature* leader = ObjectAccessor::GetCreature(*me, leaderGuid);
        ...
        {
            if (!followLeaderCalled)
            {
                FollowLeader();
                followLeaderCalled = true;
            }
        }
    }
};

// Initialisiere die statische Variable
...
{
    npc_captain_saeed(Creature* creature) : EscortAI(creature), playerAwayTime(0), escortPaused(false), pausedByPlayer(false), pausedByEvent(false), attackScheduled(false) { }

    std::list<Creature*> followers;
    std::vector<ObjectGuid> followerGUIDs;
    uint32 lastReachedWaypoint = 0;
    std::set<ObjectGuid> playersWithKillCredit;
    ObjectGuid questStarterGUID;
    uint32 playerAwayTime;
    uint32 currentGossipState = 0;
    bool escortPaused;
    bool pausedByPlayer;
    bool pausedByEvent;
    bool attackScheduled;
    ...
    bool OnGossipHello(Player* player) override
    {
        ...
        if (questStarterGUID.IsEmpty() && player->GetQuestStatus(QUEST_DIMENSIONS_THE_ALL_DEVOURING) == QUEST_STATUS_INCOMPLETE)
            questStarterGUID = player->GetGUID();

        uint32 gossipText = GetData(DATA_GOSSIP_STATE) == 1 ? TEXT_NPC_SAEED_START_FIGHT : TEXT_NPC_SAEED_DEFAULT;

        if (player->GetGUID() != questStarterGUID)
        {
        ...
        if (playersWithKillCredit.count(player->GetGUID()) == 0)
    ...
    bool OnGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
    {
        ...
        case GOSSIP_ACTION_INFO_DEF:
        {
                ...
                for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
                {
                    if (Player* member = ref->GetSource())
                    {
                        if (member->GetQuestStatus(QUEST_DIMENSIONS_THE_ALL_DEVOURING) == QUEST_STATUS_INCOMPLETE)
                        {
                            ...
                            playersWithKillCredit.insert(member->GetGUID());
                        }
                    }
                }
            }
            else
            {
                ...
                playersWithKillCredit.insert(player->GetGUID());
            }
    ...
    void SetData(uint32 id, uint32 value) override
    {
        if (id == DATA_GOSSIP_STATE)
            currentGossipState = value;
    }

    uint32 GetData(uint32 id) const override
    {
        if (id == DATA_GOSSIP_STATE)
            return currentGossipState;
        return 0;
    }

    void StartEscortFollowers()
    {
        const uint32 followerEntries[] = { NPC_SAEED_FOLLOWER_1, NPC_SAEED_FOLLOWER_2, NPC_SAEED_FOLLOWER_3 };
        followers.clear();
        ...
        for (Creature* cr : followers)
            if (cr)
                cr->HandleEmoteCommand(EMOTE_ONESHOT_ROAR);

        events.ScheduleEvent(EVENT_START_EMOTE_FOLLOW, 3s);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        events.Update(diff);
        while (uint32 ev = events.ExecuteEvent())
        {
            switch (ev)
            {
            ...
            {
                EscortDespawn();
                me->CastSpell(me, EVENT_CAST_VISUAL_TELEPORT_SPELL, false);
        ...
        if (questStarterGUID.IsEmpty())
            questStarterGUID = player->GetGUID();

        if (lastReachedWaypoint < 18)
        {
            if (me->GetDistance(player) > 20.0f)
            {
                ...
                {
                    playerAwayTime += diff;
                    if (playerAwayTime >= 20000)
                        me->DespawnOrUnsummon(1s, 5s);
                }
            }
            else
            {
                if (escortPaused && !pausedByPlayer && !pausedByEvent)
                {
                    SetEscortPaused(false);
                    escortPaused = false;
                }
    ...
    void WaypointReached(uint32 waypointId, uint32 /*pathId*/) override
    {
        ...
        lastReachedWaypoint = waypointId;
        ...
        case 16:
        {
            if (Player* player = ObjectAccessor::GetPlayer(*me, questStarterGUID))
                Talk(NPC_SAEED_SAY_4, player);
            else
                Talk(NPC_SAEED_SAY_4);

            SetData(DATA_GOSSIP_STATE, 1);
            SetEscortPaused(true);
            escortPaused = true;
            pausedByPlayer = true;
            pausedByEvent = false;
        ...
        case 18:
        {
            ...
            events.ScheduleEvent(EVENT_ATTACK_DIMENSIONS, 10s);
            break;
        }
        ...
        case 19:
        {
            ...
            SetEscortPaused(true);
            escortPaused = true;
            events.ScheduleEvent(EVENT_DESPAWN_WP19, 5s);
            break;
        }
    ...
    {
        for (auto guid : playersWithKillCredit)
        {
...
class spell_37425_logic_tick : public SpellScript
{
    ...
    {
        uint32 spells[5] = { 37396, 37397, 37399, 37405, 37409 }; /*Handle the 5 Shadow Rain Spells*/
        uint32 spellToCast = spells[urand(0, 4)];
