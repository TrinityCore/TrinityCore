        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_MARK_OF_KAZZAK:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, [](Unit const* unit) { return unit && unit->IsPlayer() && unit->GetPowerIndex(POWER_MANA) < MAX_POWERS_PER_CLASS && !unit->HasAura(SPELL_MARK_OF_KAZZAK); }))
