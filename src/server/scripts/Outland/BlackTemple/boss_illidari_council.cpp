    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            case EVENT_HAMMER_OF_JUSTICE:
                if (Unit* target = SelectTarget(SelectAggroTarget::Random, 1, HammerTargetSelector(me)))
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            ...
            case EVENT_ARCANE_EXPLOSION:
                if (_canUseArcaneExplosion && SelectTarget(SelectAggroTarget::Random, 0, 10.0f))
                {
...
class spell_illidari_council_deadly_strike : public AuraScript
{
    ...
    void OnTrigger(AuraEffect const* aurEff)
    {
        ...
        if (Unit* victim = GetTarget()->GetAI()->SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true, true))
