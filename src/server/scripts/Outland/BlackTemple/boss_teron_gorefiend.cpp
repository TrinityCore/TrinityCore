        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_SHADOW_DEATH:
                    if (Unit* target = SelectTarget(SelectAggroTarget::Random, 1, 100.0f, true, true, -SPELL_SPIRITUAL_VENGEANCE))
...
struct npc_shadowy_construct : public ScriptedAI
{
    ...
    void SelectNewTarget()
    {
        if (Creature* teron = _instance->GetCreature(DATA_TERON_GOREFIEND))
        {
            Unit* target = teron->AI()->SelectTarget(SelectAggroTarget::Random, 0, 100.0f, true, true, -SPELL_SPIRITUAL_VENGEANCE);
            // He should target Vengeful Spirits only if has no other player available
            if (!target)
                target = teron->AI()->SelectTarget(SelectAggroTarget::Random, 0);
