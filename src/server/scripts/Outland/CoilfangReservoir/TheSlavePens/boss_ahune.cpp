    ...
    void DoAction(int32 action) override
    {
        switch (action)
        {
            ...
            case ACTION_START_VISUALS:
                ...
                if (me->HasStringId("EarthenRingFlamecaller1"))
                {
                    _scheduler.Schedule(45s, 55s, [this](TaskContext const& /*task*/)
                    {
                ...
                {
                    _scheduler.Schedule(75s, 80s, [this](TaskContext const& /*task*/)
                    {
                        me->RemoveAurasDueToSpell(SPELL_FIND_OPENING_CHANNEL);
                ...
                {
                    _scheduler.Schedule(20s, 25s, [this](TaskContext const& /*task*/)
                    {
                        me->RemoveAurasDueToSpell(SPELL_FIND_OPENING_CHANNEL);
...
class spell_ahune_summoning_rhyme_aura : public AuraScript
{
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_FORCE_WHISP_FLIGHT_MISSILE, SPELL_SUMMONING_RHYME_BONFIRE }) &&
            sBroadcastTextStore.HasRecord(TEXT_INTRO_1) && sBroadcastTextStore.HasRecord(TEXT_INTRO_2) && sBroadcastTextStore.HasRecord(TEXT_INTRO_3);
    }
