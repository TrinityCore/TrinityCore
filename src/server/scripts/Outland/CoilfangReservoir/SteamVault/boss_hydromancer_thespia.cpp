            ...
            void ExecuteEvent(uint32 eventId) override
            {
                switch (eventId)
                {
                    case EVENT_LIGHTNING_CLOUD:
                        if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 30.0f, true))
                    ...
                    case EVENT_LUNG_BURST:
                        if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 40.0f, true))
                            DoCast(target, SPELL_LUNG_BURST);
                        events.ScheduleEvent(EVENT_LUNG_BURST, 7s, 12s);
                        break;
                    case EVENT_ENVELOPING_WINDS:
                        if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 35.0f, true))
