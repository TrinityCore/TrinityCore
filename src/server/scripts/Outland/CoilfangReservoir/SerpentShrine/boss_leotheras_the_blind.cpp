...
class spell_leotheras_the_blind_whirlwind_effect : public SpellScript
{
    void HandleScript(SpellEffIndex /*effIndex*/)
    {
        ...
        {
            if (eff->GetTickNumber() != eff->GetSpellEffectInfo().GetPeriodicTickCount())
            {
                if (roll_chance(50))
                {
                    caster->GetThreatManager().ResetAllThreat();
...
class spell_leotheras_the_blind_chaos_blast : public SpellScript
{
    ...
    {
        return ValidateSpellEffect({ { SPELL_CHAOS_BLAST_DAMAGE, EFFECT_1 } });
    }

    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_CHAOS_BLAST_DAMAGE, GetCastDifficulty());
