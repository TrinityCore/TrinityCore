...
enum Spells
{
    SPELL_EARTHQUAKE        = 161457,
    SPELL_SUNDER_ARMOR      = 153002,
    SPELL_CHAIN_LIGHTNING   = 153006,
    SPELL_OVERRUN           = 157465,
    SPELL_ENRAGE            = 157173,
...
class boss_Viryx : public CreatureScript
{
    public:
        boss_Viryx() : CreatureScript("boss_Viryx") { }

        struct boss_ViryxAI : public ScriptedAI
        {
            boss_ViryxAI(Creature* creature) : ScriptedAI(creature)
            {
                Initialize();
            }
        ...
        {
            return new boss_ViryxAI(creature);
        }
};
