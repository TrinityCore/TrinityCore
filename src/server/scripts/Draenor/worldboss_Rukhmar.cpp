...
enum Spells
{
    SPELL_EARTHQUAKE        = 154477,
    SPELL_SUNDER_ARMOR      = 155327,
    SPELL_CHAIN_LIGHTNING   = 153994,
    SPELL_OVERRUN           = 153991,
    SPELL_ENRAGE            = 157173,
...
class worldboss_Rukhmar : public CreatureScript
{
    public:
        worldboss_Rukhmar() : CreatureScript("worldboss_Rukhmar") { }

        struct worldboss_RukhmarAI : public ScriptedAI
        {
            worldboss_RukhmarAI(Creature* creature) : ScriptedAI(creature)
            {
                Initialize();
            }
        ...
        {
            return new worldboss_RukhmarAI(creature);
        }
};

void AddSC_worldboss_Rukhmar()
{
    new worldboss_Rukhmar();
}
