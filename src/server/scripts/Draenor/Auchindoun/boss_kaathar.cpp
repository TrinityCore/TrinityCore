...
struct boss_kaathar : public BossAI
{
    ...
    void JustDied(Unit* /*p_Killer*/) override
    {
        ...
        if (instance)
        {
            //instance->DoPlayScenePackageIdOnPlayers(SpellAuchindounSceneTeronogorSpawn);//tmp
            instance->DoRemoveAurasDueToSpellOnPlayers(eKaatharSpells::SpellSanctifiedGroundAura);
        }
        ...
        events.ScheduleEvent(Events::EVENT_POST_KAATHAR_1, Milliseconds(2 * TimeConstants::IN_MILLISECONDS));
        me->SummonCreature(uint32(NPC_NYAMI), Position(1649.33f, 2964.08f, 35.3197f, 2.65524f), TEMPSUMMON_MANUAL_DESPAWN, Milliseconds(WEEK));
    }
    ...
    void DoAction(int32 p_Action)
    {
        ...
        case eAuchindounActions::ActionCountPre1StBossKill:
        {
            ...
            if (m_Counting >= 12)
            {
                ...
                m_IntroDone = true;
                events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_1, Milliseconds(5 * TimeConstants::IN_MILLISECONDS));
            }
            break;
        ...
        if (instance)
        {
            if (Creature* l_AssinatingGuard = instance->GetCreature(eAuchindounDatas::DataAssinatingGuard))
                if (Creature* l_AssinatedGuard = instance->GetCreature(eAuchindounDatas::DataAssinatedGuard))
                {
                    ...
                    l_AssinatedGuard->Kill(l_AssinatedGuard, nullptr, true, false);//I CHANGED THIS,ADD ",nullptr,true,false"
                }
        }
    ...
    void ScheduleTasks() override
    {
        ...
        if (me->GetMap() && me->GetMap()->IsHeroic())
            events.ScheduleEvent(eKaatharEvents::EventFate, Milliseconds(25 * TimeConstants::IN_MILLISECONDS));

        events.ScheduleEvent(eKaatharEvents::EventHallowedGround, Milliseconds(urand(12 * TimeConstants::IN_MILLISECONDS, 17 * TimeConstants::IN_MILLISECONDS)));
        events.ScheduleEvent(eKaatharEvents::EventSanctifiedStrike, Milliseconds(8 * TimeConstants::IN_MILLISECONDS));
        events.ScheduleEvent(eKaatharEvents::EventHolyShield, Milliseconds(30 * TimeConstants::IN_MILLISECONDS));
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        case Events::EVENT_POST_KAATHAR_1:
        {
            ...
            {
                if (TempSummon* l_Defender = me->SummonCreature(eAuchindounCreatures::CreatureAucheniDefender, g_PositionFourMagesThatSpawnAfterKaatharIsKaaput[l_I], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                {
            ...
            events.ScheduleEvent(Events::EVENT_POST_KAATHAR_2, Milliseconds(7 * TimeConstants::IN_MILLISECONDS));
            events.ScheduleEvent(Events::EVENT_POST_KAATHAR_3, Milliseconds(55 * TimeConstants::IN_MILLISECONDS));
            break;
        }
        case Events::EVENT_POST_KAATHAR_2:
        {
            if (Creature* l_Tuulani = me->FindNearestCreature(CreatureSoulBinderTuulani, 1000.0f))
            {
                ...
                l_Tuulani->SummonGameObject(eAuchindounObjects::GameobjectHolyWall, g_PositionWallInMiddleFromNyami.GetPositionX(),
                    g_PositionWallInMiddleFromNyami.GetPositionY(), g_PositionWallInMiddleFromNyami.GetPositionZ(),
                    g_PositionWallInMiddleFromNyami.GetOrientation(), QuaternionData(0, 0, 0, 0), Seconds(0));
                /// Holy Wall, Object Behind
                l_Tuulani->SummonGameObject(eAuchindounObjects::GameobjectHolyWall, g_PositionWallInBackFromNyami.GetPositionX(),
                    g_PositionWallInBackFromNyami.GetPositionY(), g_PositionWallInBackFromNyami.GetPositionZ(),
                    g_PositionWallInBackFromNyami.GetOrientation(), QuaternionData(0, 0, 0, 0), Seconds(0));
                ...
                for (int8 l_I = 0; l_I < 3; l_I++)
                {
                    if (Creature* l_Prisoners = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniSoulPriest, g_PositionThreePrisoners[l_I], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                    {
                        ...
                        l_Prisoners->AddUnitFlag(UnitFlags(UnitFlags2::UNIT_FLAG2_DISABLE_PRED_STATS));//l_Prisoners->AddUnitFlag(UnitFlags(UnitFlags2::UNIT_FLAG2_DISABLE_TURN));//org
                        //l_Prisoners->AddDynamicFlag(UnitDynFlags::UNIT_DYNFLAG_DEAD);//tmp
                    }
                }
                ...
                for (int8 l_I = 0; l_I < 11; l_I++)
                {
                    if (Creature* l_Prisoners = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniDefender, g_PositionCorpsesNearNyomi[l_I], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                    {
                        ...
                        l_Prisoners->AddUnitFlag(UnitFlags(UNIT_FLAG_DISALLOWED | UnitFlags2::UNIT_FLAG2_FEIGN_DEATH));//UNIT_FLAG2_DISABLE_TURN,MOST POSSIBLE TO BE WRONG
                    }
                }
                ...
                for (int8 l_I = 0; l_I < 11; l_I++)
                {
                    if (Creature* l_Prisoner = l_Tuulani->SummonCreature(eAuchindounCreatures::CreatureAucheniDefender, g_PositionCorpsesNearNyomi[l_I], TempSummonType::TEMPSUMMON_DEAD_DESPAWN))
                    {
                        ...
                        l_Prisoner->AddUnitFlag(UnitFlags(UNIT_FLAG_DISALLOWED | UnitFlags2::UNIT_FLAG2_FEIGN_DEATH));//UNIT_FLAG2_DISABLE_TURN
                    }
                }
            ...
            /// Magus
           /* if (Creature* l_Magus = me->GetSummonedCreatureByEntry(eAuchindounCreatures::CreatureAucheniMagus))   //tmp
            {
                l_Magus->CastSpell(l_Magus, eAuchindounSpells::SpellArcaneChanneling);
                l_Magus->SummonGameObject(eAuchindounObjects::GameobjectTaladorPortal, g_PositionTuulaniGobjectPortalSpawn.GetPositionX(),
                    g_PositionTuulaniGobjectPortalSpawn.GetPositionY(), g_PositionTuulaniGobjectPortalSpawn.GetPositionZ(),
                    g_PositionTuulaniGobjectPortalSpawn.GetOrientation(), QuaternionData(0, 0, 0, 0), Seconds(0));
            }     */
            break;
        }
        ...
        case Events::EVENT_NYAMI_ESCAPE_1:
        {
            if (Creature* l_Nyami = me->FindNearestCreature(CreatureSoulBinderNyami, 1000.0f))
            {
                ...
                events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_2, Milliseconds(3 * TimeConstants::IN_MILLISECONDS));
            }
            break;
        }
            ...
            if (Creature* l_Nyami = me->FindNearestCreature(CreatureSoulBinderNyami, 1000.0f))
            {
                ...
                if (Unit* l_Caster = l_Nyami->FindNearestCreature(eAuchindounCreatures::CreatureLeftCrystalTrigger, 1000.0f))
                {
                    for (uint8 l_I = 0; l_I < 20; l_I++)
                    {
                        ...
                        l_Caster->SendPlaySpellVisual(Position(1911.741f, 3183.639f, 56.50413f, 0.0f), 0.f,
                            eAuchindounSpellVisualKit::SpellVisualKitBlackOrbFallingDownInSpiral, 0, 0,false, 8.0f);
                    }
                }
                events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_3, Milliseconds(6 * TimeConstants::IN_MILLISECONDS));
            }
            ...
            if (Creature* l_Nyami = me->FindNearestCreature(CreatureSoulBinderNyami, 1000.0f))
            {
                ...
                if (Unit* l_Caster = l_Nyami->FindNearestCreature(eAuchindounCreatures::CreatureLeftCrystalTrigger, 1000.0f, true))
                {
                    for (uint8 l_I = 0; l_I < 20; l_I++)
                    {
                        ...
                        l_Caster->SendPlaySpellVisual(Position(l_Caster->GetPositionX(), l_Caster->GetPositionY(), l_Caster->GetPositionZ(), 0.f), 0.0f, eAuchindounSpellVisualKit::SpellVisualKitBlackOrbFallingDownInSpiral, 0, 0,false, 8.0f);
                    }
                }
                events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_4, Milliseconds(4 * TimeConstants::IN_MILLISECONDS));
            }
            break;
        }
        case Events::EVENT_NYAMI_ESCAPE_4:
        {
            if (Creature* l_Nyami = me->FindNearestCreature(CreatureSoulBinderNyami, 1000.0f))
            {
                ...
                events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_5, Milliseconds(5 * TimeConstants::IN_MILLISECONDS));
            }
        ...
        case Events::EVENT_NYAMI_ESCAPE_5:
        {
            ...
            events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_6, Milliseconds(5 * TimeConstants::IN_MILLISECONDS));
            break;
        }
        case Events::EVENT_NYAMI_ESCAPE_6:
        {
            if (Creature* l_Nyami = me->FindNearestCreature(CreatureSoulBinderNyami, 1000.0f))
            {
                ...
                events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_7, Milliseconds(4 * TimeConstants::IN_MILLISECONDS));
            }
            break;
        }
        ...
        {
            if (Creature* l_Nyami = me->FindNearestCreature(CreatureSoulBinderNyami, 1000.0f))
                l_Nyami->DespawnOrUnsummon(Milliseconds(5 * TimeConstants::IN_MILLISECONDS));
            if (Creature* l_Tuulani = me->FindNearestCreature(CreatureSoulBinderTuulani, 1000.0f))
                l_Tuulani->AI()->Talk(eAuchindounTalks::TUULANITALK10);
            events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_8, Milliseconds(5 * TimeConstants::IN_MILLISECONDS));
            break;
        }
        case Events::EVENT_NYAMI_ESCAPE_8:
        {
            ...
            events.ScheduleEvent(Events::EVENT_NYAMI_ESCAPE_9, Milliseconds(5 * TimeConstants::IN_MILLISECONDS));
            break;
        }
        ...
        case eKaatharEvents::EventFate:
        {
            ...
            events.ScheduleEvent(eKaatharEvents::EventFate, Milliseconds(37 * TimeConstants::IN_MILLISECONDS));
            break;
        }
        case eKaatharEvents::EventHallowedGround:
        {
            ...
            me->SummonCreature(eKaatharCreatures::TriggerHallowedGround, l_Position, TempSummonType::TEMPSUMMON_TIMED_DESPAWN, Milliseconds(30 * TimeConstants::IN_MILLISECONDS));
            events.ScheduleEvent(eKaatharEvents::EventHallowedGround, Milliseconds(8 * TimeConstants::IN_MILLISECONDS));
            break;
        }
        case eKaatharEvents::EventHolyShield:
        {
            ...
            {
                //float l_X = l_Target->m_positionX + 10 /** cos(l_Target->m_orientation)*/;    //DID N'T USE
                //float l_Y = l_Target->m_positionY + 10/* * sin(l_Target->m_orientation)*/;    //DID N'T USE
                ...
                events.ScheduleEvent(eKaatharEvents::EventConsecratedLight, Milliseconds(4 * TimeConstants::IN_MILLISECONDS));
                events.ScheduleEvent(eKaatharEvents::EventHolyShieldReturn, Milliseconds(14 * TimeConstants::IN_MILLISECONDS));
            }
            events.ScheduleEvent(eKaatharEvents::EventHolyShield, Milliseconds(47 * TimeConstants::IN_MILLISECONDS));
            break;
        }
        case eKaatharEvents::EventHolyShieldReturn:
        {
            ...
            {
                me->SetCurrentEquipmentId(1); /// Equipment Id
                l_Shield->DespawnOrUnsummon(Milliseconds(3 * TimeConstants::IN_MILLISECONDS));
        ...
        case eKaatharEvents::EventSanctifiedStrike:
        {
            ...
            for (uint8 l_I = 0; l_I <= 10; l_I++)
            {
                ...
                me->SummonCreature(eKaatharCreatures::TriggerFissureSummoner, l_PosX, l_PosY, me->GetPositionZ(), me->GetOrientation(), TempSummonType::TEMPSUMMON_TIMED_DESPAWN, Milliseconds(15 * TimeConstants::IN_MILLISECONDS));
            }
            events.ScheduleEvent(eKaatharEvents::EventSanctifiedStrike, Milliseconds(8 * TimeConstants::IN_MILLISECONDS));
            break;
        }
...
struct auchindoun_kaathar_mob_hallowed_ground : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        events.ScheduleEvent(eKaatharEvents::EventCheckPlayer, Milliseconds(4 * TimeConstants::IN_MILLISECONDS)); // Takes 4 seconds to charge
    ...
    void DoAction(int32 p_Action)
    {
        ...
        case eKaatharActions::ActionFateHallowedGround:
            ...
            me->DespawnOrUnsummon(Milliseconds(1 * TimeConstants::IN_MILLISECONDS));
            break;
        default:
            break;
        }
    ...
    void UpdateAI(uint32 p_Diff)
    {
        ...
        case eKaatharEvents::EventCheckPlayer:
            ...
            events.ScheduleEvent(eKaatharEvents::EventCheckPlayer, Milliseconds(1 * TimeConstants::IN_MILLISECONDS));
            break;
        default:
            break;
        }
...
struct auchindoun_kaathar_mob_teleport_players : public ScriptedAI
{
    ...
    {
        me->SetReactState(ReactStates::REACT_PASSIVE);
        me->AddUnitFlag(UnitFlags(UnitFlags2::UNIT_FLAG2_DISABLE_PRED_STATS));//UNIT_FLAG2_DISABLE_TURN
...
struct auchindoun_kaathar_mob_holy_shield : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        me->AddUnitFlag(UnitFlags(UnitFlags2::UNIT_FLAG2_DISABLE_PRED_STATS));//UNIT_FLAG2_DISABLE_TURN//org
        //me->AddDynamicFlag(UnitDynFlags::UNIT_DYNFLAG_DEAD);//tmp
