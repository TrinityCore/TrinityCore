...
struct boss_azaakel : public BossAI
{
    ...
    void DoAction(int32 p_Action)
    {
        ...
        case eAzzakelActions::ActionFelSpark:
            events.ScheduleEvent(eAzzakelEvents::EventFelSpark, Milliseconds(1 * TimeConstants::IN_MILLISECONDS));
            break;
        case eAzzakelActions::ActionRenewEvents:
        {
            ...
            events.ScheduleEvent(eAzzakelEvents::EventFelLash, Milliseconds(8 * TimeConstants::IN_MILLISECONDS));
            events.ScheduleEvent(eAzzakelEvents::EventClawsOfArgus, Milliseconds(45 * TimeConstants::IN_MILLISECONDS));
            events.ScheduleEvent(eAzzakelEvents::EventCurtainOfFlame, Milliseconds(14 * TimeConstants::IN_MILLISECONDS));
            events.ScheduleEvent(eAzzakelEvents::EventMalevolentCrush01, Milliseconds(20 * TimeConstants::IN_MILLISECONDS));
            //if (Unit* l_Target = SelectTarget(SelectAggroTarget::SELECT_TARGET_TOPAGGRO, 0, 100.0f, true))    //org
            if (Unit* l_Target = SelectTarget(SelectTargetMethod::MaxThreat, 0, 100.0f, true))
                AttackStart(l_Target);
            break;
        }
        case eAzzakelActions::ActionMalevolentCrash:
            events.ScheduleEvent(eAzzakelEvents::EventMalevolentCrush01, Milliseconds(1 * TimeConstants::IN_MILLISECONDS));
            break;
        default:
            break;
        }
    ...
    void JustSummoned(Creature* summon) override
    {
        if (summon)
        {
            ...
            case eAzzakelCreatures::CreatureFelguard:
                ...
                //summon->SetInCombatWithZone();//tmp
                break;
            default:
                break;
            }
    ...
    void ScheduleTasks() override
    {
        ...
        me->SummonGameObject(uint32(eAuchindounObjects::GameobjectFelBarrier), 1911.01f, 2722.89f, 30.799f,
            g_PositionAzzakel_Blackgate.GetOrientation(), QuaternionData(0, 0, 0, 0), Seconds(0), GO_SUMMON_TIMED_OR_CORPSE_DESPAWN);
        //DAMN IT,IT WORKS..
    ...
    void JustDied(Unit* /*p_Killer*/)
    {
        ...
        if (instance)
        {
            ...
            if (m_Achievement)
            {
                ...
                {
                    //m_Instance->DoCompleteAchievement(eAuchindounAchievements::AchievementDemonSouls);    //tmp
                }
        ...
        void UpdateAI(uint32 p_Diff) override
        {
            ...
            case eAzzakelEvents::EventFelLash:
                ...
                events.Repeat(Milliseconds(8 * TimeConstants::IN_MILLISECONDS), Milliseconds(12 * TimeConstants::IN_MILLISECONDS));
                break;
            case eAzzakelEvents::EventClawsOfArgus:
            {
                ...
                events.Repeat(Milliseconds(45 * TimeConstants::IN_MILLISECONDS));
                break;
            }
            case eAzzakelEvents::EventCurtainOfFlame:
            {
                if (Unit* l_Target = SelectTarget(SelectTargetMethod::Random,0, 50.0f, true))//SelectAggroTarget::SELECT_TARGET_RANDOM  //org
                {
                    Talk(eAzzakelTalks::AzzakelSpell03);
                    me->AddAura(eAzzakelSpells::SpellCurtainOfFlameAura, l_Target);
                }
    
                events.Repeat(Milliseconds(8 * TimeConstants::IN_MILLISECONDS), Milliseconds(15 * TimeConstants::IN_MILLISECONDS));
                break;
            }
            case eAzzakelEvents::EventMalevolentCrush01:
            {
                ...
                events.Repeat(Milliseconds(20 * TimeConstants::IN_MILLISECONDS));
                break;
            }
            default:
                break;
...
struct auchindoun_azzakel_mob_controller : public ScriptedAI
{
    ...
    void DoAction(int32 p_Action)
    {
        ...
        case eAuchindounActions::ActionCountPre3StBossKill:
        {
            ...
            if (m_Counting >= 6 && !m_Summoned)
            {
                ...
                events.ScheduleEvent(EventSummonAzzakel01, Milliseconds(2 * TimeConstants::IN_MILLISECONDS));
            }
            break;
        }
        default:
    ...
    void UpdateAI(uint32 p_p_Diff) override
    {
        ...
        {
            if (TempSummon* TempAzzakael = me->SummonCreature(NPC_AZAAKEL, *me, TempSummonType::TEMPSUMMON_MANUAL_DESPAWN))
            {
                if (GameObject* l_Portal = me->SummonGameObject(uint32(eAuchindounObjects::GameobjectDemonicPortal), *me, QuaternionData(0, 0, 0, 0),Seconds(0),GO_SUMMON_TIMED_OR_CORPSE_DESPAWN))
                {
                    m_Azzakel = TempAzzakael->GetGUID();
                    l_Portal->AddToNotify(GameObjectFlags(GO_FLAG_NOT_SELECTABLE | GameObjectFlags::GO_FLAG_NODESPAWN | GO_FLAG_INTERACT_COND));//l_Portal->AddFlag(GameObjectFlags(GO_FLAG_NOT_SELECTABLE | GameObjectFlags::GO_FLAG_NODESPAWN | GO_FLAG_INTERACT_COND));//org
            ...
            events.ScheduleEvent(eAzzakelEvents::EventSummonAzzakel02, Milliseconds(1 * TimeConstants::IN_MILLISECONDS));
            break;
        }
    ...
    struct auchindoun_azzakel_mob_fel_spark_triggerAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            for (uint8 l_I = 0; l_I < 20; ++l_I)
            {
                ...
                /// 326528
                if (Creature* l_FelSparkNullAITrigger = me->SummonCreature(uint32(eFelSparkCreatures::CreatureFelSparkNullAITrigger), l_X, l_Y, me->GetPositionZ(), me->GetOrientation(), TempSummonType::TEMPSUMMON_TIMED_DESPAWN, Milliseconds(7 * TimeConstants::IN_MILLISECONDS)))
                {
            ...
            me->DespawnOrUnsummon(Milliseconds(7 * TimeConstants::IN_MILLISECONDS));
        }
    };
            ...
            if (InstanceScript* l_Instance = GetCaster()->GetInstanceScript())
            {
                if (Creature* l_Azzakel = l_Instance->instance->GetCreature(l_Instance->GetGuidData(eAuchindounDatas::DataBossAzzakael)))
                {
                    if (l_Azzakel->IsAIEnabled())
                    {
                        ...
                        {
                            if (Unit* l_Target = GetCaster()->GetAI()->SelectTarget(SelectTargetMethod::MaxThreat, 0, 100.0f, true))
                            {
