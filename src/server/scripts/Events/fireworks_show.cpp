                ...
                switch (eventId)
                {
                    ...
                    case EVENT_FIRE:
                    {
                        if (std::vector<Position> const* positions = Trinity::Containers::MapGetValuePtr(PositionsByZoneMap, me->GetZoneId()))
                        {
                            ...
                            if (_big)
                            {
                                if (GameObject* firework = me->SummonGameObject(FireworksBIGOnlyPicker(), rndpos, QuaternionData(0.f, 0.f, rndrot, rndrot2), GO_SUMMON_TIMED_DESPAWN, 6min))
                                {
                            ...
                            {
                                if (GameObject* firework = me->SummonGameObject(FireworksPicker(), rndpos, QuaternionData(0.f, 0.f, rndrot, rndrot2), GO_SUMMON_TIMED_DESPAWN, 6min))
                                {
                                    firework->SetRespawnTime(0);
                                    firework->Delete();
                                }
