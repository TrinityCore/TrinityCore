                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SUMMON_GNOLL:
                        {
                            ...
                            events.ScheduleEvent(EVENT_SUMMON_GNOLL, Milliseconds((GNOLL_TIMER)));
                            break;
                        }
                        case EVENT_SUMMON_DOLL:
                        {
                            ...
                            events.ScheduleEvent(EVENT_SUMMON_DOLL, Milliseconds(DOLL_TIMER));
                            break;
                        }
                        case EVENT_SUMMON_BOSS:
                        {
                            ...
                            events.ScheduleEvent(EVENT_SUMMON_BOSS, Milliseconds(BOSS_TIMER));
                            break;
                        }
                        case EVENT_CHECK_PLAYER_POS:
                        {
                            ...
                            events.ScheduleEvent(EVENT_CHECK_PLAYER_POS, Milliseconds(1000));
                            break;
                        }
                    }
                }
...
class AreaTrigger_at_whack_a_gnoll_entrance : public AreaTriggerScript
{
    public:
        ...
        //bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/, bool /*entered*/) //tmp
        //{
        //    if (!player->HasAura(SPELL_ENABLE_POWERBAR))
        //        player->CastSpell(player, SPELL_FORBIDDEN_ZONE, true);
