    ...
    public:
        ...
        struct boss_commander_rimokAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_FRENZIED_ASSAULT,Milliseconds(urand(5000, 10000)));
                events.ScheduleEvent(EVENT_VISCOUS_FLUID, Milliseconds(urand(10000, 15000)));
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch(events.ExecuteEvent())
                {
                    case EVENT_FRENZIED_ASSAULT:
                        ...
                        events.ScheduleEvent(EVENT_FRENZIED_ASSAULT, Milliseconds(urand(10000, 15000)));
                        break;
                    case EVENT_VISCOUS_FLUID:
                        me->CastSpell(me, 107077, true);
                        events.ScheduleEvent(EVENT_VISCOUS_FLUID, Milliseconds(urand(5000, 10000)));
                        break;
                }
    ...
    public:
        ...
        struct npc_viscous_fluidAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                if (checkTimer <= diff)
                {
                    if (Player* player = me->SelectNearestPlayer(100))//null before
                    {
                ...
                if (Unit* caster = GetCaster())
                {
                    if (InstanceScript* pInstance = caster->GetInstanceScript())
                    {
                        ...
                        if (Player* player = it->GetSource())
                            caster->CastSpell(player, GetSpellInfo()->GetEffect(SpellEffIndex(0)).TriggerSpell,true);////caster->CastSpell(player, GetSpellInfo()->GetEffect(SpellEffIndex(0))->TriggerSpell);//org
