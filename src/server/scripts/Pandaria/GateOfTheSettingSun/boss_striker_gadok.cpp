    ...
    public:
        ...
        struct boss_striker_gadokAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_PREY_TIME, Milliseconds(10000), PHASE_MAIN);
                events.ScheduleEvent(EVENT_IMPALING_STRIKE, Milliseconds(19000), PHASE_MAIN);
            }
            ...
            void DoStrafingEvent()
            {
                switch (strafingEventProgress)
                {
                    ...
                    case 12: // POINT_END, End Strafing Event, go back to fight
                        ...
                        events.ScheduleEvent(EVENT_PREY_TIME, Milliseconds(5000), PHASE_MAIN);
                        events.ScheduleEvent(EVENT_IMPALING_STRIKE, Milliseconds(9000), PHASE_MAIN);
                        break;
                    default:
                        break;
                }
...
struct npc_krikthik : public ScriptedAI
{
    ...
    {
        nextMovementTimer = 0;
        actualAngle = me->GetFollowAngle();//GetAngle(CenterPos.GetPositionX(), CenterPos.GetPositionY())
        direction = urand(0, 1);
...
public:
    ...
    struct npc_krikthik_disruptorAI : public npc_krikthik
    {
        ...
        void Reset() override
        {
            ...
            events.ScheduleEvent(EVENT_DISRUPTOR_BOMBARD, Milliseconds(urand(5000, 20000)));
        }
...
public:
    ...
    struct npc_flak_cannonAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* /*caster*/, SpellInfo const* spellInfo) override//This seems right
        {
            if (!pInstance)
                return;
            ...
            if (spellInfo->Id == 116554) // Fire Flak Cannon
            {
                for (uint8 i = 0; i < 5; ++i)
                {
                    if (Creature* bombarder = pInstance->instance->GetCreature(pInstance->GetGuidData(DATA_RANDOM_BOMBARDER)))
                    {
                        ...
                        bombarder->DespawnOrUnsummon(Milliseconds(2000));
                    }
                }
            }
        }
