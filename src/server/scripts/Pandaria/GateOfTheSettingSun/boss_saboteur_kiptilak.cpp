    ...
    public:
        ...
        struct boss_saboteur_kiptilakAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                events.ScheduleEvent(EVENT_EXPLOSIVES, Milliseconds(urand(7500,  10000)));
                events.ScheduleEvent(EVENT_SABOTAGE, Milliseconds(urand(22500, 30000)));
            ...
            void DoWorldInFlamesEvent()
            {
                ...
                for (auto itr: munitionList)
                {
                    ...
                    itr->DespawnOrUnsummon(Milliseconds(2000));
                }
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch(events.ExecuteEvent())
                {
                    case EVENT_EXPLOSIVES:
                        ...
                        {
                            //me->CastSpell(frand(702, 740), frand(2292, 2320), 388.5f, SPELL_PLANT_EXPLOSIVE, true);//tmp
                        }
                        events.ScheduleEvent(EVENT_EXPLOSIVES, Milliseconds(urand(7500, 12500)));
                        break;
                    case EVENT_SABOTAGE:
...
public:
    ...
    struct npc_munition_explosion_bunnyAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            float x = 0.0f;
            float y = 0.0f;
            //me->GetPositionWithDistInOrientation(40.0f, orientation, x, y);//org
            me->GetRandomNearPosition(40.0f);//I WROTE
        ...
        //void JustEngagedWith(Unit* who) override {    //TRY,FAILED
        //    _JustEngagedWith(who);
        //}
            ...
            if (checkTimer <= diff)
            {
                ...
                if (Creature* munition = GetClosestCreatureWithEntry(me, NPC_STABLE_MUNITION, 2.0f, true))
                {
                    if (munition->HasAura(SPELL_MUNITION_STABLE))
                    {
                        ...
                        munition->DespawnOrUnsummon(Milliseconds(2000));
                    }
                }
            }
