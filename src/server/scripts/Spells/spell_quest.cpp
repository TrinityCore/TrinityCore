...
public:
    ...
    class spell_q10930_fumpingSpellScript : public SpellScript
    {
        PrepareSpellScript(spell_q10930_fumpingSpellScript);

        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
            {
                caster->CastSpell(caster, SPELL_SUMMON_HAISHULUD, true);
            }
        }

        void Register()
        {
            OnEffectHit += SpellEffectFn(spell_q10930_fumpingSpellScript::HandleDummy, SpellEffIndex::EFFECT_0, SPELL_EFFECT_ANY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_q10930_fumpingSpellScript();
    }
};
