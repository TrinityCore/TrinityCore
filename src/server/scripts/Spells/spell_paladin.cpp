...
class spell_pal_righteous_defense_trigger : public SpellScriptLoader
{
    public:
        spell_pal_righteous_defense_trigger() : SpellScriptLoader("spell_pal_righteous_defense_trigger") { }

        class spell_pal_righteous_defense_trigger_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_pal_righteous_defense_trigger_SpellScript);
            ...
            void HandleTargets(std::list<Unit*>& targetList)
            {
                ...
                if (targetList.size() > 3)
                {
                    ...
                    for (uint32 i = 0;i < 3;)
                    {
                        Unit* tempTarget = SelectRandomContainerElement(targetList);
                        if (!tempTarget->IsFriendlyTo(GetCaster()))
                        {
        ...
        SpellScript* GetSpellScript() const
        {
            return new spell_pal_righteous_defense_trigger_SpellScript();
        }
};

class spell_pal_righteous_defense_target : public SpellScriptLoader
{
    public:
        spell_pal_righteous_defense_target() : SpellScriptLoader("spell_pal_righteous_defense_target") { }

        class spell_pal_righteous_defense_target_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_pal_righteous_defense_target_SpellScript);
            ...
            void Register()
            {
                OnUnitTargetSelect += SpellUnitTargetFn(spell_pal_righteous_defense_target_SpellScript::FindTarget, EFFECT_0, TARGET_UNIT_TARGET_ALLY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_pal_righteous_defense_target_SpellScript();
        }
};
...
void AddSC_paladin_spell_scripts()
{
    ...
    new spell_pal_righteous_defense_trigger();
    new spell_pal_righteous_defense_target();
}
