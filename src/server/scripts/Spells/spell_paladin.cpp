...
enum RighteousDefense
{
    SPELL_RIGHTEOUS_DEFENSE_TAUNT = 31790,
};

class spell_pal_rightous_defense_trigger : public SpellScriptLoader
{
    public:
        spell_pal_rightous_defense_trigger() : SpellScriptLoader("spell_pal_rightous_defense_trigger") { }

        class spell_pal_rightous_defense_trigger_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_pal_rightous_defense_trigger_SpellScript);
            ...
            void HandleTargets(std::list<Unit*>& targetList)
            {
                if (targetList.size() > 2)
                {
                    ...
                    for (uint32 i = 0;i < 3; i++)
                        newTargets.push_back(SelectRandomContainerElement(targetList));
            ...
            void Register()
            {
                OnUnitTargetSelect += SpellUnitTargetFn(spell_pal_rightous_defense_trigger_SpellScript::HandleTargets, EFFECT_0, TARGET_UNIT_CASTER_AREA_RAID);
                OnEffectHitTarget += SpellEffectFn(spell_pal_rightous_defense_trigger_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_pal_rightous_defense_trigger_SpellScript();
        }
};

class spell_pal_rightous_defense_target : public SpellScriptLoader
{
    public:
        spell_pal_rightous_defense_target() : SpellScriptLoader("spell_pal_rightous_defense_target") { }

        class spell_pal_rightous_defense_target_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_pal_rightous_defense_target_SpellScript);
                    ...
                    if (Unit* target = *itr)
                        if (!target->IsFriendlyTo(GetCaster()))
                            if (Unit* newTarget = target->getVictim())
                                if (newTarget->IsFriendlyTo(GetCaster()))
                                {
                                    targetList.clear();
            ...
            void Register()
            {
                OnUnitTargetSelect += SpellUnitTargetFn(spell_pal_rightous_defense_target_SpellScript::FindTarget, EFFECT_0, TARGET_UNIT_TARGET_ALLY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_pal_rightous_defense_target_SpellScript();
        }
};
...
void AddSC_paladin_spell_scripts()
{
    ...
    new spell_pal_rightous_defense_trigger();
    new spell_pal_rightous_defense_target();
}
