...
public:
    ...
    class spell_hun_piercing_shots_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            {
                int32 duration = sSpellMgr->GetSpellInfo(SPELL_HUNTER_PIERCING_SHOTS)->GetMaxDuration();
                uint32 amplitude = sSpellMgr->GetSpellInfo(SPELL_HUNTER_PIERCING_SHOTS)->Effects[0].Amplitude;
...
public:
    ...
    class spell_hun_t9_4p_bonus_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            PreventDefaultAction();
            Unit* caster = eventInfo.GetActor();
            Pet* target = caster->ToPlayer()->GetPet();

            caster->CastSpell(target, SPELL_HUNTER_T9_4P_GREATNESS, true, nullptr, aurEff);
