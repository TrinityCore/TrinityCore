        ...
        struct go_wg_vehicle_teleporterAI : public GameObjectAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                if (_checkTimer <= diff)
                {
                    ...
                    {
                        if (Creature* vehicle = go->FindNearestCreature(vehiclesList[i], 3.0f, true))
                        {
                            if (!vehicle->HasAura(SPELL_VEHICLE_TELEPORT))
                            {
                                if (vehicle->GetVehicle())
                                {
                                    ...
                                    {
                                        if (go->GetUInt32Value(GAMEOBJECT_FACTION) == player->getFaction())
                                        {
                                            vehicle->CastSpell(vehicle, SPELL_VEHICLE_TELEPORT, true);
                                            if (Creature* targetTeleport = vehicle->FindNearestCreature(NPC_WORLD_TRIGGER_LARGE_AOI_NOT_IMMUNE_PC_NPC, 100.0f, true))
                                            {
                                                float x, y, z, o;
