...
public:
    ...
    struct boss_flame_leviathanAI : public BossAI
    {
        ...
        void DoAction(int32 const uiAction)
        {
            // Start encounter
            if (uiAction == 10)
            {
            ...
            case ACTION_TOWER_OF_LIFE_DESTROYED: // Tower of Nature destroyed
                towerOfLife = false;
                break;
            case ACTION_UNBROKEN_ACHIEVEMENT:
                Unbroken = false;
                break;
            }
        }
...
public:
    ...
    struct npc_mechanoliftAI : public PassiveAI
    {
        ...
        void MovementInform(uint32 /*type*/, uint32 id)
        {
            if (id == 1)
            {
                Creature* container = me->FindNearestCreature(NPC_CONTAINER, 5, true);
                if (container)
        ...
        if(Creature* vehicle = player->GetVehicleCreatureBase())
        {
            if(!vehicle->HasAura(SPELL_AUTO_REPAIR))
            {
                ...
                if (Creature* leviathan = ObjectAccessor::GetCreature(*player, instance ? instance->GetData64(TYPE_LEVIATHAN) : 0))
                    leviathan->AI()->DoAction(ACTION_UNBROKEN_ACHIEVEMENT); // set bool to false thats checked in leviathan getdata
            }
        }
        return true;
    }
