...
struct boss_hodir : public BossAI
{
    ...
    void JustAppeared() override
    {
        ...
        std::span<uint32 const> entries = instance->instance->GetTeamIdInInstance() == TEAM_HORDE
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        instance->TriggerGameEvent(EVENT_ID_ENGAGE_HODIR);
    }
    ...
    void DoAction(int32 action) override
    {
        switch (action)
        {
            ...
            case ACTION_COOLEST_FRIEND_DIES:
                instance->DoUpdateWorldState(WORLD_STATE_HODIR_COOLEST_FRIENDS_FAILED, 1);
                break;
            case ACTION_BITING_COLD_TOO_MUCH_STACKS:
                instance->DoUpdateWorldState(WORLD_STATE_HODIR_GETTING_COLD_FAILED, 1);
                break;
            case ACTION_PLAYER_IS_FROZEN:
                instance->DoUpdateWorldState(WORLD_STATE_HODIR_CHEESE_THE_FREEZE_FAILED, 1);
                break;
            case ACTION_CACHE_SHATTERED:
                Talk(EMOTE_SHATTER);
                instance->DoUpdateWorldState(WORLD_STATE_HODIR_THIS_CACHE_WAS_RARE_FAILED, 1);
                break;
    ...
    void OnSpellStart(SpellInfo const* spellInfo) override
    {
        if (spellInfo->Id == SPELL_FLASH_FREEZE)
        {
            ...
            DoCastSelf(SPELL_ICICLE_FORCE_CAST, true);
        }
    }

    void OnSpellCast(SpellInfo const* spellInfo) override
    {
        if (spellInfo->Id == SPELL_FROZEN_BLOWS)
            Talk(EMOTE_BLOWS);
            ...
            switch (eventId)
            {
                ...
                case EVENT_FLASH_FREEZE_FINISHED_2:
                {
                    ...
                    for (GameObject* fire : firesObjects)
                        fire->ActivateObject(GameObjectActions::Despawn, 0);
...
struct npc_icicle : public ScriptedAI
{
    ...
    void JustAppeared() override
    {
        _scheduler.Schedule(3500ms, [this](TaskContext const& /*task*/)
        {
            switch (me->GetEntry())
            {
                case NPC_ICICLE:
...
class spell_hodir_freeze : public SpellScript
{
    bool Validate(SpellInfo const* spellInfo) override
    {
        return ValidateSpellEffect({ { spellInfo->Id, EFFECT_1 } })
            && ValidateSpellInfo({ uint32(spellInfo->GetEffect(EFFECT_1).CalcValueAsInt()) });
    }

    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if(Trinity::UnitAuraCheck(true, uint32(GetEffectInfo(EFFECT_1).CalcValueAsInt())));
    }
...
class spell_hodir_icicle : public SpellScript
{
    ...
    {
        GetCaster()->CastSpell(GetHitUnit(), uint32(GetEffectInfo(EFFECT_0).CalcValueAsInt()), true);
    }
...
class spell_hodir_icicle_force_cast : public SpellScript
{
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        Trinity::Containers::RandomResize(targets, GetSpellInfo()->Id == SPELL_ICICLE_FORCE_CAST ? 2 : 3);
    }
...
class spell_hodir_dispel_magic : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if(Trinity::UnitAuraCheck(false, uint32(GetEffectInfo(EFFECT_1).CalcValueAsInt())));
        Trinity::Containers::RandomResize(targets, 1);
    }
...
class spell_hodir_storm_cloud_primer : public SpellScript
{
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_STORM_CLOUD_EFFECT });
    }
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        GetCaster()->CastSpell(GetHitUnit(), SPELL_STORM_CLOUD_EFFECT);
    }
...
class spell_hodir_melt_ice : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if(Trinity::UnitAuraCheck(true, uint32(GetEffectInfo(EFFECT_0).CalcValueAsInt())));
        Trinity::Containers::RandomResize(targets, 1);
    }
