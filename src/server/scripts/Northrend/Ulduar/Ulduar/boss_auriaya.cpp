        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_SAVAGE_POUNCE:
                    if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, CatsTargetSelector(me, 10.0f, 15.0f)))
                    {
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_START_COMBAT:
                    ...
                    break;
                case EVENT_RUSH:
                    if (Unit* target = SelectTarget(SelectAggroTarget::Random, 1, CatsTargetSelector(me, 10.0f, 11.0f)))
                    {
...
class spell_auriaya_agro_creator : public SpellScript
{
    ...
    void HandleDummyEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = caster->AI()->SelectTarget(SelectAggroTarget::Random, 0, CatsTargetSelector(caster, 5.0f, 10.0f)))
        {
...
class spell_auriaya_random_agro_periodic : public AuraScript
{
    ...
    void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        bool farTarget = true;
        Unit* target = owner->AI()->SelectTarget(SelectAggroTarget::Random, 0, CatsTargetSelector(owner, 15.0f, 25.0f));
        if (!target)
        {
            farTarget = false;
            target = owner->AI()->SelectTarget(SelectAggroTarget::Random, 0);
        }
