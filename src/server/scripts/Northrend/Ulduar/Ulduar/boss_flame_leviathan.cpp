        ...
        class spell_pursue_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<Unit*>& targets)
            {
                ...
                else
                {
                    ...
                    _target = GreenPis::Containers::SelectRandomContainerElement(targets);
                ...
                if (targets.HasTraj())
                    if (Vehicle* vehicle = GetCaster()->GetVehicleKit())
                        if (Unit* passenger = vehicle->GetPassenger(damage - 1))
                        {
                            ...
                            GreenPis::WorldObjectSpellAreaTargetCheck check(99, GetExplTargetDest(), GetCaster(), GetCaster(), GetSpellInfo(), TARGET_CHECK_DEFAULT, NULL);
                            GreenPis::WorldObjectListSearcher<GreenPis::WorldObjectSpellAreaTargetCheck> searcher(GetCaster(), targetList, check);
