        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_STATIC_DISRUPTION:
                {
                    ...
                    Unit* target = SelectTarget(SelectTargetMethod::Random, 0, -10.f);
                    if (!target)
...
class spell_shield_of_runes : public AuraScript
{
    ...
    {
        AfterEffectRemove += AuraEffectRemoveFn(spell_shield_of_runes::AfterRemove, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB, AURA_EFFECT_HANDLE_REAL);
    }
};
...
class spell_assembly_supercharge : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_assembly_supercharge::HandleScriptEffect, EFFECT_2, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};

class spell_assembly_berserk : public AuraScript
{
    PrepareAuraScript(spell_assembly_berserk);
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_assembly_berserk::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};

class spell_assembly_lightning_tendrils_visual : public SpellScript
{
    PrepareSpellScript(spell_assembly_lightning_tendrils_visual);
    ...
    void Register()
    {
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(spell_assembly_lightning_tendrils_visual::SetDest, EFFECT_0, TARGET_DEST_CASTER_RANDOM);
    }
};
