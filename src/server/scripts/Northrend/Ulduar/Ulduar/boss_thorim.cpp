        ...
        struct npc_thorim_arena_phaseAI : public npc_thorim_trashAI
        {
            ...
            void ExecuteEvent(uint32 eventId) override
            {
                switch (eventId)
                {
                    ...
                    {
                        Unit* referer = me;
                        if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, [referer](Unit* unit){ return unit->GetTypeId() == TYPEID_PLAYER && unit->IsInRange(referer, 8.0f, 25.0f); }))
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            Unit* referer = me;
                            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, [referer](Unit* unit){ return unit->GetTypeId() == TYPEID_PLAYER && unit->IsInRange(referer, 8.0f, 40.0f); }))
