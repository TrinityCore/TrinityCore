        ...
        struct boss_sapphironAI : public BossAI
        {
            ...
            ObjectGuid GetGUID(int32 data) const override
            {
                if (data == DATA_BLIZZARD_TARGET)
                {
                    ...
                    if (Unit* newTarget = me->AI()->SelectTarget(SelectAggroTarget::Random, 1, BlizzardTargetSelector(blizzards)))
                    ...
                    if (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            ...
                            case EVENT_LIFTOFF:
                            {
                                ...
                                SelectTargetList(targets, RAID_MODE(2, 3), SelectAggroTarget::Random, 0, 200.0f, true);
