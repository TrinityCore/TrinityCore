...
class KelThuzadCharmedPlayerAI : public SimpleCharmedPlayerAI
{
    public:
        ...
        Unit* SelectAttackTarget() const override
        {
            ...
            {
                if (Unit* target = charmer->AI()->SelectTarget(SelectAggroTarget::Random, 0, CharmedPlayerTargetSelectPred()))
                    return target;
                if (Unit* target = charmer->AI()->SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true, true, -SPELL_CHAINS))
                    return target;
            }
            return nullptr;
        }
        ...
        public:
            ...
            void SpellHit(WorldObject* /*caster*/, SpellInfo const* spellInfo) override
            {
                if (spellInfo->Id == SPELL_CHAINS_DUMMY)
                {
                    ...
                    SelectTargetList(targets, 3, SelectAggroTarget::Random, 0, 0.0f, true, false);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_DETONATE_MANA:
                        {
                            ...
                            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, pred))
