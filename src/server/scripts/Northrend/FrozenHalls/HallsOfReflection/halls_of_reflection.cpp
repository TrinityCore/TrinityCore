            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch (_events.ExecuteEvent())
                {
                    ...
                    case EVENT_CIRCLE_OF_DESTRUCTION:
                        if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 10.0f, true))
    ...
    public:
        ...
        struct npc_frostsworn_generalAI : public ScriptedAI
        {
            ...
            void SummonClones()
            {
                ...
                SelectTargetList(playerList, 5, SelectAggroTarget::MaxThreat, 0, 0.0f, true);
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch (_events.ExecuteEvent())
                {
                    case EVENT_BALEFUL_STRIKE:
                        if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 8.0f, true))
...
class HoRStartMovementEvent : public BasicEvent
{
    public:
        ...
        bool Execute(uint64 /*execTime*/, uint32 /*diff*/) override
        {
            ...
            if (Unit* target = _owner->AI()->SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true))
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                switch (_events.ExecuteEvent())
                {
                    ...
                    case EVENT_RISEN_WITCH_DOCTOR_SHADOW_BOLT:
                        if (Unit* target = SelectTarget(SelectAggroTarget::MaxThreat, 0, 20.0f, true))
                            DoCast(target, SPELL_SHADOW_BOLT);
                        _events.ScheduleEvent(EVENT_RISEN_WITCH_DOCTOR_SHADOW_BOLT, 2s, 3s);
                        break;
                    case EVENT_RISEN_WITCH_DOCTOR_SHADOW_BOLT_VOLLEY:
                        if (SelectTarget(SelectAggroTarget::Random, 0, 30.0f, true))
