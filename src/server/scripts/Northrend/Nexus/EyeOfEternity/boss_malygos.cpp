            ...
            switch (action)
            {
                case ACTION_LAND_ENCOUNTER_START:
                    ...
                    if (Creature* alexstraszaBunny = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_ALEXSTRASZA_BUNNY_GUID)))
                    {
                        ...
                        alexstraszaBunny->GetNearPoint2D(pos.m_positionX, pos.m_positionY, 30.0f + alexstraszaBunny->GetObjectSize(), alexstraszaBunny->GetAngle(me));
                    ...
                    case EVENT_FLY_OUT_OF_PLATFORM:
                        if (!_performingDestroyPlatform)
                        {
                            if (Creature* alexstraszaBunny = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_ALEXSTRASZA_BUNNY_GUID)))
                            {
                                ...
                                alexstraszaBunny->GetNearPoint2D(randomPosOnRadius.m_positionX, randomPosOnRadius.m_positionY, 120.0f + alexstraszaBunny->GetObjectSize(), alexstraszaBunny->GetAngle(me));
        ...
        class spell_malygos_random_portal_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Creature* target = GetHitCreature())
                {
                    ...
                    target->GetNearPoint2D(pos.m_positionX, pos.m_positionY, frand(29.1f, 30.0f) + target->GetObjectSize(), target->GetAngle(malygos));
