            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SURGE_OF_POWER_P_THREE:
                        if (GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
                        {
                            if (Unit* tempSurgeTarget = SelectTarget(SelectAggroTarget::Random, 0, 0.0f, false, true, SPELL_RIDE_RED_DRAGON_BUDDY))
                            {
                    ...
                    case EVENT_STATIC_FIELD:
                        if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 60.0f, false, true, SPELL_RIDE_RED_DRAGON_BUDDY))
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_ARCANE_SHOCK:
                            if (Unit* victim = SelectTarget(SelectAggroTarget::Random, 0, 5.0f, true))
