            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            bool corporealityUpdate = false;
                            int8 side = 0;
                            if (MaterialDamageTaken * 1.02f >= TwilightDamageTaken)
                            {
                                TwilightDamageTaken = 0;
                                MaterialDamageTaken = 0;
                                corporealityUpdate = true;
                                side = 1;
                            }
                            ...
                            if (corporealityUpdate)
                            {
                                if (side == 1)
                                {
                                    uint32 materialCorporeality = _instance->GetData(DATA_CORPOREALITY_MATERIAL);
                                    uint32 newMaterialVal = (materialCorporeality - 10 < 0 ? 0 : materialCorporeality - 10);
                                    uint32 newTwilightVal = 100 - newMaterialVal;
                                    instance->SetData(DATA_CORPOREALITY_MATERIAL, newMaterialVal);
                                    instance->SetData(DATA_CORPOREALITY_TWILIGHT, newTwilightVal);
                                }
                                else if (side == -1)
                                {
                                    uint32 materialCorporeality = _instance->GetData(DATA_CORPOREALITY_MATERIAL);
                                    uint32 newMaterialVal = (materialCorporeality + 10 > 100 ? 100 : materialCorporeality + 10);
                                    uint32 newTwilightVal = 100 - newMaterialVal;
                                    instance->SetData(DATA_CORPOREALITY_MATERIAL, newMaterialVal);
                                    instance->SetData(DATA_CORPOREALITY_TWILIGHT, newTwilightVal);
                                }
