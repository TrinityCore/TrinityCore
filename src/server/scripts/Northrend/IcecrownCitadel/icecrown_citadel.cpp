                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_DEATH_PLAGUE:
                            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 1, DeathPlagueTargetSelector(me)))
                            {
...
struct npc_darkfallen_noble : public DarkFallenAI
{
    ...
    void ScheduleSpells() override
    {
        ...
        Scheduler.Schedule(500ms, [this](TaskContext /*context*/)
        {
            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true, false, -SPELL_CHAINS_OF_SHADOW))
...
struct npc_darkfallen_archmage : public DarkFallenAI
{
    ...
    void ScheduleSpells() override
    {
        ...
        {
            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true, false, -SPELL_POLYMORPH))
...
struct npc_darkfallen_tactician : public DarkFallenAI
{
    ...
    void ScheduleSpells() override
    {
        ...
        {
            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true, false))
            {
