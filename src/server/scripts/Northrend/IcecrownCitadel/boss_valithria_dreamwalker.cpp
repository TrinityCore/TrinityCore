...
class ValithriaDespawner : public BasicEvent
{
    public:
        ...
        {
            GreenPis::CreatureWorker<ValithriaDespawner> worker(_creature, *this);
        ...
        struct npc_risen_archmageAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit* /*target*/)
            {
                ...
                if (me->GetDBTableGUIDLow() && _canCallEnterCombat)
                {
                    ...
                    GreenPis::CreatureListSearcher<RisenArchmageCheck> searcher(me, archmages, check);
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            Player* player = NULL;
                            GreenPis::AnyPlayerInObjectRangeCheck check(me, 5.0f);
...
class spell_dreamwalker_summoner : public SpellScriptLoader
{
    public:
        ...
        class spell_dreamwalker_summoner_SpellScript : public SpellScript
        {
            ...
            {
                targets.remove_if (GreenPis::UnitAuraCheck(true, SPELL_RECENTLY_SPAWNED));
                if (targets.empty())
                    return;

                Unit* target = GreenPis::Containers::SelectRandomContainerElement(targets);
                targets.clear();
    ...
    public:
        ...
        class spell_dreamwalker_summon_suppresser_AuraScript : public AuraScript
        {
            ...
            void PeriodicTick(AuraEffect const* /*aurEff*/)
            {
                ...
                summoners.remove_if (GreenPis::UnitAuraCheck(true, SPELL_RECENTLY_SPAWNED));
                GreenPis::Containers::RandomResizeList(summoners, 2);
                if (summoners.empty())
                    return;
