                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SLIME_PUDDLE:
                        {
                            ...
                            SelectTargetList(targets, 2, SelectAggroTarget::Random, 0, 0.0f, true);
                            if (!targets.empty())
                        ...
                        case EVENT_MALLEABLE_GOO:
                            ...
                            {
                                std::list<Unit*> targets;
                                SelectTargetList(targets, 2, SelectAggroTarget::Random, 0, -7.0f, true);
                                if (!targets.empty())
                                {
                            ...
                            {
                                if (Unit* target = SelectTarget(SelectAggroTarget::Random, 1, -7.0f, true))
                                {
                                    Talk(EMOTE_MALLEABLE_GOO);
