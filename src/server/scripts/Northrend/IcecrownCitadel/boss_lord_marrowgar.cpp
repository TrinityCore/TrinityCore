                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_BONE_STORM_MOVE:
                        {
                            ...
                            Unit* unit = SelectTarget(SelectAggroTarget::Random, 0, NonTankTargetSelector(me));
                            if (!unit)
    ...
    public:
        ...
        class spell_marrowgar_coldflame_SpellScript : public SpellScript
        {
            ...
            void SelectTarget(std::list<WorldObject*>& targets)
            {
                ...
                Unit* target = GetCaster()->GetAI()->SelectTarget(SelectAggroTarget::Random, 0, -GetCaster()->GetCombatReach(), true, false, -SPELL_IMPALED);
                if (!target)
...
class spell_marrowgar_bone_spike_graveyard : public SpellScriptLoader
{
    public:
        ...
        class spell_marrowgar_bone_spike_graveyard_SpellScript : public SpellScript
        {
            ...
            {
                return GetCaster()->GetAI()->SelectTarget(SelectAggroTarget::Random, 0, BoneSpikeTargetSelector(GetCaster()->GetAI())) ? SPELL_CAST_OK : SPELL_FAILED_NO_VALID_TARGETS;
            }
            ...
            void HandleSpikes(SpellEffIndex effIndex)
            {
                ...
                if (Creature* marrowgar = GetCaster()->ToCreature())
                {
                    ...
                    marrowgarAI->SelectTargetList(targets, boneSpikeCount, SelectAggroTarget::Random, 1, BoneSpikeTargetSelector(marrowgarAI));
                    if (targets.empty())
                        return;
