        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_IMPALING_SPEAR:
                    if (Unit* target = SelectTarget(SelectAggroTarget::Random, 1, 0.0f, true, true, -SPELL_IMPALING_SPEAR))
                    {
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_YMIRJAR_ICE_TRAP:
                    if (Unit* target = SelectTarget(SelectAggroTarget::Random, 1, 20.0f, true))
                        DoCast(target, SPELL_ICE_TRAP);
                    _events.Repeat(15s, 20s);
                    break;
                case EVENT_YMIRJAR_VOLLEY:
                    if (Unit* target = SelectTarget(SelectAggroTarget::Random, 1, 40.0f, true))
