...
class boss_prince_taldaram_icc : public CreatureScript
{
    public:
        ...
        struct boss_prince_taldaramAI : public BloodPrincesBossAI
        {
            ...
            {
                summons.Summon(summon);
                Unit* target = SelectTarget(SelectAggroTarget::Random, 1, -10.0f, true); // first try at distance
                if (!target)
                    target = SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true);     // too bad for you raiders, its going to boom
