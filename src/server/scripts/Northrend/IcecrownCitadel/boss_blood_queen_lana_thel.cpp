    ...
    public:
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_PACT_OF_THE_DARKFALLEN:
                        {
                            ...
                            if (Is25ManRaid())
                                ++targetCount;
                            GreenPis::Containers::RandomResizeList<Player*>(targets, targetCount);
                            if (targets.size() > 1)
                            {
                        ...
                        case EVENT_TWILIGHT_BLOODBOLT:
                        {
                            std::list<Player*> targets;
                            SelectRandomTarget(false, &targets);
                            GreenPis::Containers::RandomResizeList<Player*>(targets, uint32(Is25ManRaid() ? 4 : 2));
                            for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
        ...
        private:
            ...
            Player* SelectRandomTarget(bool includeOfftank, std::list<Player*>* targetList = NULL)
            {
                ...
                if (includeOfftank)
                {
                    tempTargets.sort(GreenPis::ObjectDistanceOrderPred(me->getVictim()));
                    return tempTargets.front();
                }

                return GreenPis::Containers::SelectRandomContainerElement(tempTargets);
            }
    ...
    public:
        ...
        class spell_blood_queen_bloodbolt_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<Unit*>& targets)
            {
                ...
                GreenPis::Containers::RandomResizeList(targets, targetCount);
...
class spell_blood_queen_pact_of_the_darkfallen : public SpellScriptLoader
{
    public:
        ...
        class spell_blood_queen_pact_of_the_darkfallen_SpellScript : public SpellScript
        {
            ...
            {
                unitList.remove_if (GreenPis::UnitAuraCheck(false, SPELL_PACT_OF_THE_DARKFALLEN));
...
class spell_blood_queen_pact_of_the_darkfallen_dmg_target : public SpellScriptLoader
{
    public:
        ...
        class spell_blood_queen_pact_of_the_darkfallen_dmg_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<Unit*>& unitList)
            {
                unitList.remove_if (GreenPis::UnitAuraCheck(true, SPELL_PACT_OF_THE_DARKFALLEN));
