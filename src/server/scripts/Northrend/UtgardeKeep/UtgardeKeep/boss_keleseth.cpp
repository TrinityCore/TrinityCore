                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_FROST_TOMB:
                            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 100.0f, true, true, -SPELL_FROST_TOMB))
                            {
                                Talk(SAY_FROST_TOMB);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_DECREPIFY:
                            if (Unit* target = SelectTarget(SelectAggroTarget::Random, 0, 0.0f, true, true, -SPELL_DECREPIFY))
