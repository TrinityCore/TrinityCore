...
#define SelectItemInstanceContent "ii.guid, ii.itemEntry, ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags, ii.enchantments, ii.randomBonusListId, " \
    ...
    // Garrison
    PrepareStatement(CHAR_SEL_CHARACTER_GARRISON, "SELECT garrTypeId, siteLevelId, followerActivationsRemainingToday FROM character_garrison WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CHARACTER_GARRISON, "INSERT INTO character_garrison (guid, garrTypeId, siteLevelId, followerActivationsRemainingToday) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
