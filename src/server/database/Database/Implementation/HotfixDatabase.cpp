...
void HotfixDatabaseConnection::DoPrepareStatements()
{
    ...
    // ScenarioEventEntry.db2
    PrepareStatement(HOTFIX_SEL_SCENARIO_EVENT_ENTRY, "SELECT ID, Value, Operator FROM scenario_event_entry ORDER BY ID DESC", CONNECTION_SYNCH);

    // ScenarioStep.db2
    PrepareStatement(HOTFIX_SEL_SCENARIO_STEP, "SELECT ID, Description, Name, CriteriaTreeID, ScenarioID, PreviousStepID, QuestRewardID, Step, Flags, Unk1 FROM scenario_step ORDER BY ID DESC", CONNECTION_SYNCH);
    PREPARE_LOCALE_STMT(HOTFIX_SEL_SCENARIO_STEP, "SELECT Description_lang, Name_lang FROM scenario_step_locale WHERE locale = ?", CONNECTION_SYNCH);
