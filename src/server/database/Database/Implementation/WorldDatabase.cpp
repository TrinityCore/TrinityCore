...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_CREATURE_TEMPLATE, "SELECT CreatureId, KillCredit1, KillCredit2, Name, NameAlt, Title, TitleAlt, IconName, RequiredExpansion, VignetteId, FactionTemplateId, NpcFlags, SpeeWalk, SpeedRun, Scale, Classification, DamageSchool, BaseAttackTime, RangeAttackTime, BaseVariance, RangeVariance, Class, UnitFlags, UnitFlags2, UnitFlags3, CreatureFamilyId, TrainerClass, CreatureTypeId, VehicleId, AIName, MovementType, ctm.HoverInitiallyEnabled, ctm.Chase, ctm.Random, ctm.InteractionPauseTimer, ExperienceModifier, RacialLeader, MovementInfoId, WidgetSetId, WidgetSetUnitConditionId, RegenHealth, CreatureImmunitiesId, FlagsExtra, ScriptName, StringId FROM creature_template ct LEFT JOIN creature_template_movement ctm ON ct.CreatureId = ctm.CreatureId WHERE CreatureId = ? OR 1 = ?", CONNECTION_SYNCH);
