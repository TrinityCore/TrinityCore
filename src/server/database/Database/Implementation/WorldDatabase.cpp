...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_UPD_CREATURE_FACTION, "UPDATE creature_template SET FactionTemplateID = ? WHERE CreatureID = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_CREATURE_NPCFLAG, "UPDATE creature_template SET NpcFlags = ? WHERE CreatureID = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(WORLD_SEL_CREATURE_TEMPLATE, "SELECT ct.CreatureID, ct.KillCredit1, ct.KillCredit2, ct.Name, ct.NameAlt, ct.Title, ct.TitleAlt, ct.IconName, ct.RequiredExpansion, ct.VignetteID, ct.FactionTemplateID, ct.NpcFlags, ct.SpeedWalk, ct.SpeedRun, ct.Scale, ct.Classification, ct.DamageSchool, ct.BaseAttackTime, ct.RangeAttackTime, ct.BaseVariance, ct.RangeVariance, ct.Class, ct.UnitFlags, ct.UnitFlags2, ct.UnitFlags3, ct.CreatureFamilyID, ct.TrainerClass, ct.CreatureTypeID, ct.VehicleID, ct.AIName, ct.MovementType, ctm.HoverInitiallyEnabled, ctm.Chase, ctm.Random, ctm.InteractionPauseTimer, ct.ExperienceModifier, ct.RacialLeader, ct.MovementInfoID, ct.WidgetSetID, ct.WidgetSetUnitConditionID, ct.RegenHealth, ct.CreatureImmunitiesID, ct.FlagsExtra, ct.ScriptName, ct.StringID FROM creature_template ct LEFT JOIN creature_template_movement ctm ON ct.CreatureID = ctm.CreatureId WHERE ct.CreatureID = ? OR 1 = ?", CONNECTION_SYNCH);
