...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_DEL_CREATURE_LINKED_RESPAWN, "DELETE FROM linked_respawn WHERE guid = ? AND linkType IN (0,1)", CONNECTION_ASYNC); //CREATURE_TO_CREATURE, CREATURE_TO_GO
    PrepareStatement(WORLD_DEL_CREATURE_LINKED_RESPAWN_MASTER, "DELETE FROM linked_respawn WHERE linkedGuid = ? AND linkType IN (0,3)", CONNECTION_ASYNC); //CREATURE_TO_CREATURE, GO_TO_CREATURE
    PrepareStatement(WORLD_DEL_GOB_LINKED_RESPAWN, "DELETE FROM linked_respawn WHERE guid = ? AND linkType IN (2,3)", CONNECTION_ASYNC); //GO_TO_GO, GO_TO_CREATURE
    PrepareStatement(WORLD_DEL_GOB_LINKED_RESPAWN_MASTER, "DELETE FROM linked_respawn WHERE linkedGuid = ? AND linkType IN (2,1)", CONNECTION_ASYNC);
    PrepareStatement(WORLD_REP_CREATURE_LINKED_RESPAWN, "REPLACE INTO linked_respawn (guid, linkedGuid, linkType) VALUES (?, ?, 0)", CONNECTION_ASYNC);
