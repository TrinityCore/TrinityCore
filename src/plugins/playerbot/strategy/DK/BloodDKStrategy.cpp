...
public:
    BloodDKStrategyActionNodeFactory()
    {
		...
		creators["death strike"] = &heart_strike;
...
private:
	...
	static ActionNode* blood_strike(PlayerbotAI* ai)
    {
        return new ActionNode ("blood strike",
            /*P*/ NextAction::array(0, new NextAction("frost presence"), NULL),
            /*A*/ NextAction::array(0, new NextAction("rune strike"), NULL),
            /*C*/ NULL);
    }
...
triggers.push_back(new TriggerNode(
		"low health",
		NextAction::array(0, 
			new NextAction("blood presence", ACTION_HIGH + 4),
			new NextAction("blood tap", ACTION_HIGH + 4),
			new NextAction("vampiric blood", ACTION_HIGH + 3),
			new NextAction("death strike", ACTION_HIGH + 2), NULL)));
...
void BloodDKAoeStrategy::InitTriggers(std::list<TriggerNode*> &triggers)
{
	triggers.push_back(new TriggerNode(
		"high aoe",
		NextAction::array(0, new NextAction("army of the dead", ACTION_NORMAL + 5),
			new NextAction("pestilence", ACTION_NORMAL + 4),
			new NextAction("blood boil", ACTION_NORMAL + 3), NULL)));

	triggers.push_back(new TriggerNode(
		"medium aoe",
		NextAction::array(0, new NextAction("pestilence", ACTION_NORMAL + 4),
			new NextAction("blood boil", ACTION_NORMAL + 3), NULL)));

	triggers.push_back(new TriggerNode(
		"light aoe",
		NextAction::array(0, new NextAction("pestilence", ACTION_NORMAL + 4), new NextAction("hearth strike", ACTION_NORMAL + 3), NULL)));
}
