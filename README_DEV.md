...
`world-viewer` serves a read-only Elwynn agent map at `http://<server-LAN-IP>:${WORLD_VIEWER_PORT:-8090}`. Compose publishes the port on all host interfaces. The container can start without telemetry; it shows a waiting state until a snapshot arrives. To enable live data, set a long random `WORLD_VIEWER_TELEMETRY_TOKEN` in `.env`, then set `AIWorld.TelemetryEnabled = 1` in `deploy/worldserver.conf` and restart `worldserver`. The same token is passed to both containers by Compose and is required on `POST /internal/telemetry`. Browser reads are unauthenticated, so anyone who can reach the port can see the telemetry.
